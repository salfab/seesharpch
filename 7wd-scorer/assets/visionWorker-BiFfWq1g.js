...
`)}get variablesInfo(){if(this.uniforms.length===0)return;let e=t=>[12,10,1,6][["u32","f16","f32","i32"].indexOf(t)];return this.uniforms.map(t=>[e(t.type),t.length??1])}},nu=(e,t)=>new tu(e,t)}),ru,Ni,iu,au,su,ou,ut,uu,lu,Vt=J(()=>{he(),me(),qe(),ye(),ru=(e,t)=>{if(!e||e.length!==1)throw new Error("Transpose requires 1 input.");if(t.length!==0&&t.length!==e[0].dims.length)throw new Error(`perm size ${t.length} does not match input rank ${e[0].dims.length}`)},Ni=(e,t)=>t.length!==0?t:[...new Array(e).keys()].reverse(),iu=(e,t)=>W.sortBasedOnPerm(e,Ni(e.length,t)),au=(e,t,n,r)=>{let i=`fn perm(i: ${r.type.indices}) -> ${n.type.indices} {
  ...
  }`},{name:"TransposeShared",shaderCache:{inputDependencies:["type"]},getRunData:()=>{let y=W.size(a);return{outputs:[{dims:a,dataType:e.dataType}],dispatchGroup:{x:Math.ceil(o[1]/g),y:Math.ceil(o[0]/g)},programUniforms:[{type:12,data:y},...ue(s,o)]}},getShaderSource:l}}return l=g=>{let y=K("a",n,s.length),_=ae("output",n,o.length);return`
  ${g.registerUniform("output_size","u32").declareVariables(y,_)}
...
var best_index : i32 = 0;`,`if (${r.getByIndices("input_indices")} ${t.selectLastIndex>0?">=":">"} value) {
  ...
  }`};return{name:"AttentionProbsSoftmax",shaderCache:{hint:`${l};${m};${u}`,inputDependencies:y},getShaderSource:_,getRunData:()=>({outputs:[],dispatchGroup:{x:1,y:i,z:t*n},programUniforms:p})}},tl=(e,t,n,r,i,a,s,o,u)=>{let l=s+a.kvSequenceLength,h=[a.batchSize,a.numHeads,a.sequenceLength,l],c=e>1&&r,p=a.kvNumHeads?a.kvNumHeads:a.numHeads,m=c?[a.batchSize,p,l,a.headSize]:void 0,g=a.nReps?a.nReps:1,y=a.scale===0?1/Math.sqrt(a.headSize):a.scale,_=Fe(a.headSize),$=a.headSize/_,x=12,T={x:Math.ceil(l/x),y:Math.ceil(a.sequenceLength/x),z:a.batchSize*a.numHeads},S=[{type:12,data:a.sequenceLength},{type:12,data:$},{type:12,data:l},{type:12,data:a.numHeads},{type:12,data:a.headSize},{type:1,data:y},{type:12,data:s},{type:12,data:a.kvSequenceLength},{type:12,data:g}],I=c&&r&&W.size(r.dims)>0,E=["type","type"];I&&E.push("type"),i&&E.push("type"),o&&E.push("type"),u&&E.push("type");let M=[{dims:h,dataType:t.dataType,gpuDataType:0}];c&&M.push({dims:m,dataType:t.dataType,gpuDataType:0});let v=z=>{let B=K("q",t.dataType,t.dims,_),X=K("key",n.dataType,n.dims,_),U=[B,X];if(I){let O=K("past_key",r.dataType,r.dims,_);U.push(O)}i&&U.push(K("attention_bias",i.dataType,i.dims));let H=o?K("seq_lens",o.dataType,o.dims):void 0;H&&U.push(H);let A=u?K("total_sequence_length_input",u.dataType,u.dims):void 0;A&&U.push(A);let G=ae("output",t.dataType,h),R=[G];c&&R.push(ae("present_key",t.dataType,m,_));let P=Qe(1,_),Y=[{name:"M",type:"u32"},{name:"K",type:"u32"},{name:"N",type:"u32"},{name:"num_heads",type:"u32"},{name:"head_size",type:"u32"},{name:"alpha",type:"f32"},{name:"past_sequence_length",type:"u32"},{name:"kv_sequence_length",type:"u32"},{name:"n_reps",type:"u32"}];return`
  const TILE_SIZE = ${x}u;
...
`,Yl=e=>`quick_gelu_impl(${e})`,Xl=(e,t)=>{let n=Qe(e.inputs[0].dataType);e.compute(Se(e.inputs[0],"QuickGelu",Yl,Kl(n,t.alpha),t.cacheKey,e.inputs[0].dataType))}}),Zl,Ql,Jl,f0=J(()=>{me(),ye(),qi(),Zl=e=>{if(e[0].dims.length!==3)throw new Error("input should have 3 dimensions");if(![2560,5120,10240].includes(e[0].dims[2]))throw new Error("hidden state should be 2560, 5120 or 10240");if(e[1].dims.length!==1)throw new Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw new Error("last dimension of input and bias are not the same")},Ql=e=>{let t=e[0].dims.slice();t[2]=t[2]/2;let n=K("input",e[0].dataType,e[0].dims,4),r=K("bias",e[0].dataType,[e[0].dims[2]],4),i=ae("output",e[0].dataType,t,4),a=W.size(t)/4,s=He(e[0].dataType);return{name:"BiasSplitGelu",getRunData:()=>({outputs:[{dims:t,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(a/64)}}),getShaderSource:o=>`
  ...
  ${o.mainStart()}
    ...
    ${i.setByOffset("global_idx","valueLeft * geluRight")}
          ...
          `}else $=g.setByOffset("global_idx",m(y.getByOffset("global_idx"),_.getByOffset("global_idx")));else{if(!a)throw new Error("no necessary to use scalar implementation for element-wise binary op implementation.");let x=(T,S,I="")=>{let E=`aData[indexA${S}][componentA${S}]`,M=`bData[indexB${S}][componentB${S}]`;return`
    ...
    fn pow_vector_custom(a : vec4<${t}>, b : vec4<${t}>) -> vec4<${t}> {
      ...
      `)},od=e=>{bt(e,"Sub",(t,n)=>`${t}-${n}`)},ud=e=>{bt(e,"Greater",{scalar:(t,n)=>`u32(${t}>${n})`,vector:(t,n)=>`vec4<u32>(${t}>${n})`},void 0,void 0,9)},ld=e=>{bt(e,"Less",{scalar:(t,n)=>`u32(${t}<${n})`,vector:(t,n)=>`vec4<u32>(${t}<${n})`},void 0,void 0,9)},dd=e=>{bt(e,"GreaterOrEqual",{scalar:(t,n)=>`u32(${t}>=${n})`,vector:(t,n)=>`vec4<u32>(${t}>=${n})`},void 0,void 0,9)},cd=e=>{bt(e,"LessOrEqual",{scalar:(t,n)=>`u32(${t}<=${n})`,vector:(t,n)=>`vec4<u32>(${t}<=${n})`},void 0,void 0,9)}}),hd,pd,fd,md,gd,yd,g0=J(()=>{he(),me(),qe(),ye(),hd=(e,t)=>{if(!e||e.length<1)throw new Error("too few inputs");let n=0,r=e[n],i=r.dataType,a=r.dims.length;e.forEach((s,o)=>{if(o!==n){if(s.dataType!==i)throw new Error("input tensors should be one type");if(s.dims.length!==a)throw new Error("input tensors should have the same shape");s.dims.forEach((u,l)=>{if(l!==t&&u!==r.dims[l])throw new Error("non concat dimensions must match")})}})},pd=(e,t)=>`
...
`},vd=(e,t,n,r,i=!1)=>{let[a,s,o,u]=r,l=He(r[0].type.tensor);return`
    ...
    `},vr=(e,t,n,r,i=!1,a)=>{let s=e[0].dims,o=e[1].dims,u=s.slice(0,-2),l=o.slice(0,-2),h=r?r.slice(0,-2):n.slice(0,-2),c=W.size(h),p=s[s.length-2],m=s[s.length-1],g=o[o.length-1],y=m%4===0&&g%4===0,_=p<=8?[4,1,1]:[4,4,1],$=[8,8,1],x=[Math.ceil(g/$[0]/_[0]),Math.ceil(p/$[1]/_[1]),Math.ceil(c/$[2]/_[2])],T=y?4:1,S=[...u,p,m/T],I=S.length,E=[...l,m,g/T],M=E.length,v=[c,p,g/T],z=[{type:6,data:p},{type:6,data:g},{type:6,data:m}];cn(t,z),z.push(...ue(h,S,E));let B=["rank","rank"],X=e.length>2;X&&(z.push(...ue(e[2].dims)),B.push("rank")),z.push(...ue(v));let U=H=>{let A=h.length,G=Oi("batchDims",e[0].dataType,A,1),R=He(e[0].dataType),P=K("a",e[0].dataType,I,T),Y=K("b",e[1].dataType,M,T),O=ae("result",e[0].dataType,v.length,T),Z=[P,Y];if(X){let ne=i?T:1;Z.push(K("bias",e[2].dataType,e[2].dims.length,ne))}let D=[{name:"dim_a_outer",type:"i32"},{name:"dim_b_outer",type:"i32"},{name:"dim_inner",type:"i32"}];hn(t,D);let V=He(O.type.tensor),F=dn(t,O.type.value,V),L=vd(T,X,F,[G,P,Y,O],i);return`
  ${H.registerUniforms(D).registerInternalVariables(G).declareVariables(...Z,O)}
  ${L}
  ${y?Yi(_,$,R,G):Zi(_,$,R,G)}
                   `};return{name:"MatMul",shaderCache:{hint:`${_};${t.activation};${y};${i}`,inputDependencies:B},getRunData:()=>({outputs:[{dims:a?a(n):n,dataType:e[0].dataType}],dispatchGroup:{x:x[0],y:x[1],z:x[2]},programUniforms:z}),getShaderSource:U}}}),Sd,Td,w0=J(()=>{he(),Nt(),ye(),pn(),Hi(),y0(),Qi(),Sd=(e,t,n,r,i=!1,a,s=4,o=4,u=4,l="f32")=>{let h=z=>{switch(z){case 1:return"resData = x[xIndex];";case 3:return`resData = vec3<${l}>(x[xIndex], x[xIndex + 1], x[xIndex + 2]);`;case 4:return"resData = x[xIndex / 4];";default:throw new Error(`innerElementSize ${z} is not supported.`)}},c=z=>{switch(z){case 1:return"return w[row * i32(uniforms.w_shape[3]) + colIn];";case 4:return"return w[row * i32(uniforms.w_shape[3]) / 4 + colIn];";default:throw new Error(`innerElementSize ${z} is not supported.`)}},p=e?`
    ...
    fn mm_write(batch: i32, row : i32, colIn : i32, valueIn : ${I}) {
      ...
      fn setOutputAtCoords(d0 : i32, d1 : i32, d2 : i32, d3 : i32, value : ${y?`vec4<${O}>`:O}) {
        ...
        ${y?Yi(T,x,O,void 0,!l,v):Zi(T,x,O,void 0,!l,v,!1,void 0,o)}`};return{name:"Conv2DMatMul",shaderCache:{hint:`${t.cacheKey};${I};${y};${z};${B};${X};${E};${M};${v}`,inputDependencies:A},getRunData:()=>({outputs:[{dims:u?u(n):n,dataType:e[0].dataType}],dispatchGroup:{x:S[0],y:S[1],z:S[2]},programUniforms:H}),getShaderSource:G}}}),Id,Ji,Hn,kd,ea,Ed,Md,Cd,_0=J(()=>{he(),Nt(),me(),ye(),pn(),Hi(),Id=e=>{let t=1;for(let n=0;n<e.length;n++)t*=e[n];return t},Ji=e=>typeof e=="number"?[e,e,e]:e,Hn=(e,t)=>t<=1?e:e+(e-1)*(t-1),kd=(e,t,n,r=1)=>{let i=Hn(t,r);return Math.floor((e[0]*(n-1)-n+i)/2)},ea=(e,t,n,r,i)=>{i==null&&(i=kd(e,t[0],r[0]));let a=[0,0,0,n];for(let s=0;s<3;s++)e[s]+2*i>=t[s]&&(a[s]=Math.trunc((e[s]-t[s]+2*i)/r[s]+1));return a},Ed=(e,t,n,r,i,a,s,o,u,l)=>{let h,c,p,m;if(e==="VALID"&&(e=0),typeof e=="number"){h={top:e,bottom:e,left:e,right:e,front:e,back:e};let g=ea([t,n,r,1],[o,u,l],1,[i,a,s],e);c=g[0],p=g[1],m=g[2]}else if(Array.isArray(e)){if(!e.every((y,_,$)=>y===$[0]))throw Error(`Unsupported padding parameter: ${e}`);h={top:e[0],bottom:e[1],left:e[2],right:e[3],front:e[4],back:e[5]};let g=ea([t,n,r,1],[o,u,l],1,[i,a,s],e[0]);c=g[0],p=g[1],m=g[2]}else if(e==="SAME_UPPER"){c=Math.ceil(t/i),p=Math.ceil(n/a),m=Math.ceil(r/s);let g=(c-1)*i+o-t,y=(p-1)*a+u-n,_=(m-1)*s+l-r,$=Math.floor(g/2),x=g-$,T=Math.floor(y/2),S=y-T,I=Math.floor(_/2),E=_-I;h={top:T,bottom:S,left:I,right:E,front:$,back:x}}else throw Error(`Unknown padding parameter: ${e}`);return{padInfo:h,outDepth:c,outHeight:p,outWidth:m}},Md=(e,t,n,r,i,a=!1,s="channelsLast")=>{let o,u,l,h,c;if(s==="channelsLast")[o,u,l,h,c]=e;else if(s==="channelsFirst")[o,c,u,l,h]=e;else throw new Error(`Unknown dataFormat ${s}`);let[p,,m,g,y]=t,[_,$,x]=Ji(n),[T,S,I]=Ji(r),E=Hn(m,T),M=Hn(g,S),v=Hn(y,I),{padInfo:z,outDepth:B,outHeight:X,outWidth:U}=Ed(i,u,l,h,_,$,x,E,M,v),H=a?p*c:p,A=[0,0,0,0,0];return s==="channelsFirst"?A=[o,H,B,X,U]:s==="channelsLast"&&(A=[o,B,X,U,H]),{batchSize:o,dataFormat:s,inDepth:u,inHeight:l,inWidth:h,inChannels:c,outDepth:B,outHeight:X,outWidth:U,outChannels:H,padInfo:z,strideDepth:_,strideHeight:$,strideWidth:x,filterDepth:m,filterHeight:g,filterWidth:y,effectiveFilterDepth:E,effectiveFilterHeight:M,effectiveFilterWidth:v,dilationDepth:T,dilationHeight:S,dilationWidth:I,inShape:e,outShape:A,filterShape:t}},Cd=(e,t,n,r,i,a)=>{let s=a==="channelsLast";s?e[0].dims[3]:e[0].dims[1];let o=[64,1,1],u={x:n.map((_,$)=>$)},l=[Math.ceil(Id(u.x.map(_=>n[_]))/o[0]),1,1];xe("verbose",()=>`[conv3d_naive_webgpu] dispatch = ${l}`);let h=1,c=W.size(n),p=[{type:12,data:c},{type:12,data:r},{type:12,data:i},{type:12,data:t.strides},{type:12,data:t.dilations}];cn(t,p),p.push(...ue(e[0].dims,e[1].dims));let m=["rank","rank"],g=e.length===3;g&&(p.push(...ue(e[2].dims)),m.push("rank")),p.push(...ue(n));let y=_=>{let $=[{name:"output_size",type:"u32"},{name:"filter_dims",type:"u32",length:r.length},{name:"pads",type:"u32",length:i.length},{name:"strides",type:"u32",length:t.strides.length},{name:"dilations",type:"u32",length:t.dilations.length}];hn(t,$);let x=1,T=He(e[0].dataType),S=K("x",e[0].dataType,e[0].dims.length,h),I=K("W",e[1].dataType,e[1].dims.length,x),E=[S,I],M=ae("result",e[0].dataType,n.length,x),v="";if(g){let X=K("bias",e[2].dataType,e[2].dims.length,x);E.push(X),v+=`
        fn getBiasByOutputCoords(coords : array<u32, 5>) -> ${T} {
          ...
          ${_.registerUniforms($).declareVariables(...E,M)}
          ${_.mainStart()}
  ...
  }`};return{name:"GroupedConv",shaderCache:{hint:`${t.cacheKey}_${c}`,inputDependencies:g},getRunData:()=>({outputs:[{dims:r?r(n):n,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(p/64)},programUniforms:m}),getShaderSource:y}},Rd=(e,t,n,r)=>{let i=e.length>2,a=Fe(n[3]),s=Fe(n[2]),o=W.size(n)/a/s,u=[e[0].dims[0],e[0].dims[1],e[0].dims[2],e[0].dims[3]/a],l=[e[1].dims[0],e[1].dims[1],e[1].dims[2],e[1].dims[3]/a],h=[n[0],n[1],n[2],n[3]/a],c=[{type:12,data:o},{type:6,data:[t.strides[0],t.strides[1]]},{type:6,data:[t.pads[0],t.pads[1]]}];cn(t,c),c.push(...ue(u,l,h));let p=(s-1)*t.strides[1]+l[1],m=g=>{let y=ae("output",e[0].dataType,h.length,a),_=He(y.type.tensor),$=dn(t,y.type.value,_),x=K("x",e[0].dataType,u.length,a),T=K("w",e[1].dataType,l.length,a),S=[x,T];i&&S.push(K("b",e[2].dataType,e[2].dims,a));let I=i?"value += b[output_channel];":"",E=[{name:"output_size",type:"u32"},{name:"strides",type:"i32",length:2},{name:"pads",type:"i32",length:2}];return hn(t,E),`
...
`)};
    ...
    ${g}`},h=[{type:12,data:u},...ue(t,r)];return{name:"Expand",shaderCache:{hint:`${r.length};${s}${o}`,inputDependencies:["rank"]},getShaderSource:l,getRunData:()=>({outputs:[{dims:r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(u/64)},programUniforms:h})}},lc=e=>{sc(e.inputs),e.compute(uc(e.inputs),{inputs:[0]})}}),dc,cc,E0=J(()=>{he(),me(),ye(),qi(),dc=e=>{let t=e[0].dataType,n=W.size(e[0].dims),r=W.size(e[1].dims),i=r%4===0,a=s=>{let o=K("x",t,[1],4),u=K("bias",t,[1],4),l=ae("y",t,[1],4),h=[{name:"output_vec_size",type:"u32"},{name:"bias_size",type:"u32"}],c=m=>`
    ...
    }`};return{name:"FastGeluWithBias",shaderCache:{hint:`${i}`,inputDependencies:["type","type"]},getShaderSource:a,getRunData:s=>({outputs:[{dims:s[0].dims,dataType:s[0].dataType}],programUniforms:[{type:12,data:Math.ceil(n/4)},{type:12,data:r}],dispatchGroup:{x:Math.ceil(n/In/4)}})}},cc=e=>{e.inputs.length<2||W.size(e.inputs[1].dims)===0?Vl(e):e.compute(dc(e.inputs))}}),hc,pc,fc,mc,M0=J(()=>{he(),me(),qe(),ye(),hc=e=>{if(!e||e.length!==2)throw new Error("Gather requires 2 inputs.")},pc=(e,t)=>{let n=e[0].dims,r=e[1].dims,i=n.length,a=W.normalizeAxis(t.axis,i),s=n.slice(0);s.splice(a,1,...r);let o=n[a],u=e[0].dataType===9?4:1,l=Math.ceil(W.size(s)/u),h=[{type:12,data:l},{type:6,data:o},{type:12,data:a},...ue(e[0].dims,e[1].dims,s)],c=p=>{let m=K("data",e[0].dataType,e[0].dims.length,u),g=K("inputIndices",e[1].dataType,e[1].dims.length),y=ae("output",e[0].dataType,s.length,u),_=x=>{let T=r.length,S=`var indicesIndices${x}  = ${g.type.indices}(0);`;for(let I=0;I<T;I++)S+=`${T>1?`indicesIndices${x}[${I}]`:`indicesIndices${x}`} = ${s.length>1?`outputIndices${x}[uniforms.axis + ${I}]`:`outputIndices${x}`};`;S+=`
...
`)}
  ...
  }`};return e.compute({name:"InstanceNormComputeChannelScaleShift",shaderCache:{hint:`${u};${o};${p}`,inputDependencies:y},getRunData:()=>({outputs:[{dims:g,dataType:1}],dispatchGroup:{x:c},programUniforms:_}),getShaderSource:$},{inputs:[t,n,r],outputs:[-1]})[0]},sh=(e,t,n)=>{let r=t[0].dims,i=r,a=2,s=r[0],o=r[1],u=W.sizeFromDimension(r,a),l=Fe(u),h=W.size(i)/l,c=ha(e,t[0],t[1],t[2],s,u,o,n.epsilon),p=[s,o,u/l],m=[s,o],g=["type","none"],y=_=>{let $=K("x",t[0].dataType,p.length,l),x=K("scale_shift",1,m.length,2),T=ae("output",t[0].dataType,p.length,l),S=[$,x,T];return`
  ...
  ${I.mainStart()}
    ...
    ${x?"inv_std_output[global_idx] = inv_std_dev":""};
        ...
        }`};return{name:"MatMulNBits",shaderCache:{hint:`${t.blockSize};${t.bits};${c};${p};${m};${y};${$}`,inputDependencies:Array(e.length).fill("rank")},getRunData:()=>({outputs:[{dims:g,dataType:h}],dispatchGroup:{x:_},programUniforms:x}),getShaderSource:E}},gh=(e,t)=>{let n=e[0].dims,r=n.length,i=n[r-2],a=t.k,s=t.n,o=n.slice(0,r-2),u=W.size(o),l=e[1].dims[2]/4,h=e[0].dataType,c=Fe(t.k),p=Fe(l),m=o.concat([i,s]),g=128,y=s%8===0?8:s%4===0?4:1,_=g/y,$=Math.floor(32/t.bits),x=_*p*$,T=x/c,S=x/t.blockSize,I=W.size(m)/y,E=[],M=[u,i,a/c],v=W.convertShape(e[1].dims).slice();v.splice(-1,1,l/p),E.push(...ue(M)),E.push(...ue(v)),E.push(...ue(e[2].dims)),e.length===4&&E.push(...ue(W.convertShape(e[3].dims)));let z=[u,i,s];E.push(...ue(z));let B=X=>{let U=M.length,H=K("a",e[0].dataType,U,c),A=K("b",12,v.length,p),G=K("scales",e[2].dataType,e[2].dims.length),R=[H,A,G],P=e.length===4?K("zero_points",12,e[3].dims.length):void 0;P&&R.push(P);let Y=z.length,O=ae("output",e[0].dataType,Y),Z=He(e[0].dataType),D=()=>{switch(c){case 1:return`
        ...
        }`};return{name:"BlockwiseMatMulNBits32",shaderCache:{hint:`${t.blockSize};${c};${p};${_};${y}`,inputDependencies:Array(e.length).fill("rank")},getRunData:()=>({outputs:[{dims:m,dataType:h}],dispatchGroup:{x:I},programUniforms:E}),getShaderSource:B}},yh=(e,t)=>{fh(e.inputs,t),t.blockSize===32&&e.adapterInfo.isVendor("intel")&&e.adapterInfo.isArchitecture("gen-12lp")?e.compute(gh(e.inputs,t)):e.compute(mh(e.inputs,t))},wh=e=>Ie(e)}),_h,bh,$h,xh,vh,Sh,Th,Ih,kh,L0=J(()=>{he(),me(),ye(),_h=e=>{if(!e||e.length<1)throw new Error("Too few inputs");if(e[0].dataType!==1&&e[0].dataType!==10)throw new Error("Input type must be float or float16.");if(e.length>=2){let t=e[0].dims.length*2===e[1].dims[0];if(e.length===4&&(t=e[3].dims[0]*2===e[1].dims[0]),!t)throw new Error("The pads should be a 1D tensor of shape [2 * input_rank] or [2 * num_axes].")}},bh=(e,t,n)=>{let r="";for(let i=t-1;i>=0;--i)r+=`
  ...
  for (var i = 0u; i < uniforms.num_updates_elements; i++) {
    ...
    ${Vh(t.reduction,"output[data_offset + i]","value",g.type.value)}
      ...
      }`};return{name:"ScatterND",shaderCache:{hint:`${t.cacheKey}_${t.reduction}`,inputDependencies:["rank","rank"]},getRunData:()=>({outputs:[{dims:i,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(s/64)},programUniforms:l}),getShaderSource:h}},jh=e=>Ie({reduction:e.reduction}),Kh=(e,t)=>{e.compute(Hh(e.inputs,t),{inputs:[e.inputs[1],e.inputs[2]],outputs:[]})}}),Yh,Xh,Zh,$a,Qh,Jh,ep,tp,np,rp,ip,ap,xa,sp,op,up,lp,dp,cp,hp,V0=J(()=>{he(),me(),qe(),ye(),Yh=(e,t)=>{if(e.every(n=>n>0||(()=>{throw new Error("Resize requires scales input values to be positive")})),e.length>0){if(t.mode==="linear"){if(!(e.length===2||e.length===3||e.length===4&&e[0]===1&&e[1]===1||e.length===4&&e[0]===1&&e[3]===1||e.length===5&&e[0]===1&&e[1]===1))throw new Error(`For linear mode, Resize requires scales to be 2D, 3D, 4D with either two outermost or one innermost and
...
`:"",sp=(e,t,n,r,i)=>{let[a,s,o,u]=n.length===2?[-1,0,1,-1]:[0,2,3,1],l=e.type.value;return`
    ...
    }`},op=(e,t,n,r,i,a,s,o,u,l)=>{let h=n.length===2,[c,p]=h?[0,1]:[2,3],m=e.type.value,g=y=>{let _=y===c?"row":"col";return`
  ...
  fn bicubicInterpolation(output_indices: ${t.type.indices}) -> ${m} {
    ...
    }`},lp=(e,t,n,r,i,a)=>{let s=e.dims,o=ep(a,t.axes,s.length),u=tp(s,r,i,t.axes),l=r.slice();r.length===0&&(l=s.map((x,T)=>x===0?1:u[T]/x),t.keepAspectRatioPolicy!=="stretch"&&(u=np(s,l,t)));let h=ae("output",e.dataType,u.length),c=K("input",e.dataType,s.length),p=W.size(u),m=s.length===u.length&&s.every((x,T)=>x===u[T]),g=t.coordinateTransformMode==="tf_crop_and_resize",y=t.extrapolationValue,_=c.type.value,$=x=>`
      ${m?"":`
      ${Qh(t.coordinateTransformMode,_)};
      ${(()=>{switch(t.mode){case"nearest":return`
              ${ap(c,s)};
              ${Jh(t.nearestMode,n,_)};
...
`}
      ...
      }`},S=[{dims:o,dataType:e[0].dataType}];return n>1&&S.push({dims:h,dataType:1}),n>2&&S.push({dims:h,dataType:1}),n>3&&S.push({dims:a,dataType:e[0].dataType}),{name:"SkipLayerNormalization",shaderCache:{hint:`${$};${m};${g};${y}`,inputDependencies:e.map((I,E)=>"type")},getShaderSource:T,getRunData:()=>({outputs:S,dispatchGroup:{x:Math.ceil(u/l)},programUniforms:x})}},mp=(e,t)=>{pp(e.inputs);let n=[0];e.outputCount>1&&n.push(-3),e.outputCount>2&&n.push(-3),e.outputCount>3&&n.push(3),e.compute(fp(e.inputs,t,e.outputCount,!1),{outputs:n})}}),gp,Xn,yp,va,wp,_p,bp,$p,j0=J(()=>{he(),me(),qe(),ye(),gp=(e,t)=>{if(!e||e.length<1)throw new Error("too few inputs");if(t.axes.length!==0){if(t.axes.length!==t.starts.length||t.axes.length!==t.ends.length)throw new Error("axes, starts and ends must have the same length")}else if(t.starts.length!==t.ends.length)throw new Error("starts and ends must have the same length");e.slice(1).forEach((n,r)=>{if(e[r+1].dataType!==6&&e[r+1].dataType!==7)throw new Error(`Input ${r} must be an array of int32 or int64`)})},Xn=(e,t)=>{let n=[];if(e.length>t)if(e[t].dataType===7)e[t].getBigInt64Array().forEach(r=>n.push(Number(r)));else if(e[t].dataType===6)e[t].getInt32Array().forEach(r=>n.push(Number(r)));else throw new Error(`Input ${t} must be an array of int32 or int64`);return n},yp=(e,t)=>{if(e.length>1){let n=Xn(e,1),r=Xn(e,2),i=Xn(e,3);return i.length===0&&(i=[...Array(e[0].dims.length).keys()]),Ie({starts:n,ends:r,axes:i})}else return t},va=(e,t,n,r,i)=>{let a=e;return e<0&&(a+=n[r[t]]),i[t]<0?Math.max(0,Math.min(a,n[r[t]]-1)):Math.max(0,Math.min(a,n[r[t]]))},wp=(e,t,n)=>`fn calculateInputIndices(output_indices: ${t.type.indices}) -> ${e.type.indices} {
          var input_indices: ${e.type.indices};
          var carry = 0u;
      ...
      }`;return{name:"Slice",shaderCache:{hint:`${u.length}_${s.length}_${a.length}`,inputDependencies:["rank"]},getShaderSource:_,getRunData:()=>({outputs:[h],dispatchGroup:{x:Math.ceil(r/64)},programUniforms:y})}},bp=(e,t)=>{gp(e.inputs,t);let n=yp(e.inputs,t);e.compute(_p(e.inputs,n),{inputs:[0]})},$p=e=>{let t=e.starts,n=e.ends,r=e.axes;return Ie({starts:t,ends:n,axes:r})}}),xp,vp,Sp,Tp,K0=J(()=>{he(),me(),qe(),Vt(),ye(),xp=e=>{if(!e||e.length!==1)throw new Error("Softmax op requires 1 input.")},vp=(e,t)=>{let n=e.inputs[0],r=n.dims,i=W.size(r),a=r.length,s=W.normalizeAxis(t.axis,a),o=s<r.length-1,u,l=[];o?(l=Array.from({length:a},(M,v)=>v),l[s]=a-1,l[a-1]=s,u=e.compute(ut(n,l),{inputs:[n],outputs:[-1]})[0]):u=n;let h=u.dims,c=h[a-1],p=i/c,m=Fe(c),g=c/m,y=64;p===1&&(y=256);let _=(M,v)=>v===4?`max(max(${M}.x, ${M}.y), max(${M}.z, ${M}.w))`:v===2?`max(${M}.x, ${M}.y)`:v===3?`max(max(${M}.x, ${M}.y), ${M}.z)`:M,$=K("x",u.dataType,u.dims,m),x=ae("result",u.dataType,u.dims,m),T=$.type.value,S=He(u.dataType)==="f32"?`var threadMax = ${T}(-3.4028234663852886e+38f);`:`var threadMax = ${T}(-65504.0h);`,I=M=>`
    ...
    }`;return{name:"Tile",shaderCache:{hint:`${r}`,inputDependencies:["rank"]},getRunData:()=>({outputs:[{dims:i,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(a/64)},programUniforms:[{type:12,data:a},...ue(e[0].dims,i)]}),getShaderSource:l}},Mp=e=>{Ip(e.inputs),e.compute(Ep(e.inputs),{inputs:[0]})}}),Cp,Ap,Rp,X0=J(()=>{he(),me(),ye(),Cp=(e,t,n,r,i)=>{let a=ae("output_data",i,n.length,4),s=K("a_data",t[1].dataType,t[1].dims.length,4),o=K("b_data",t[2].dataType,t[2].dims.length,4),u=K("c_data",t[0].dataType,t[0].dims.length,4),l,h=(c,p,m)=>`select(${p}, ${c}, ${m})`;if(!r)l=a.setByOffset("global_idx",h(s.getByOffset("global_idx"),o.getByOffset("global_idx"),u.getByOffset("global_idx")));else{let c=(p,m,g="")=>{let y=`a_data[index_a${m}][component_a${m}]`,_=`b_data[index_b${m}][component_b${m}]`,$=`bool(c_data[index_c${m}] & (0xffu << (component_c${m} * 8)))`;return`
 ...
 * limitations under the License.
 * =============================================================================
 */const iy=114;function ay(e,t,n){const r=Math.min(n/e,n/t),i=Math.round(e*r),a=Math.round(t*r);return{scale:r,padX:Math.floor((n-i)/2),padY:Math.floor((n-a)/2),resizedWidth:i,resizedHeight:a}}function Kt(e,t,n){const{width:r,height:i,channels:a,data:s}=e,o=new Uint8Array(t*n*3),u=r/t,l=i/n;for(let h=0;h<n;h++){const c=(h+.5)*l-.5,p=Math.max(0,Math.min(i-1,Math.floor(c))),m=Math.min(i-1,p+1),g=Math.max(0,Math.min(1,c-p));for(let y=0;y<t;y++){const _=(y+.5)*u-.5,$=Math.max(0,Math.min(r-1,Math.floor(_))),x=Math.min(r-1,$+1),T=Math.max(0,Math.min(1,_-$)),S=(p*r+$)*a,I=(p*r+x)*a,E=(m*r+$)*a,M=(m*r+x)*a,v=(h*t+y)*3;for(let z=0;z<3;z++){const B=s[S+z]*(1-T)+s[I+z]*T,X=s[E+z]*(1-T)+s[M+z]*T;o[v+z]=Math.min(255,Math.max(0,Math.round(B*(1-g)+X*g)))}}}return o}function sf(e){const n=((-.75*(e+1)- -3.75)*(e+1)+-6)*(e+1)- -3,r=((-.75+2)*e-(-.75+3))*e*e+1,i=((-.75+2)*(1-e)-(-.75+3))*(1-e)*(1-e)+1;return[n,r,i,1-n-r-i]}function sy(e,t,n){const{width:r,height:i,channels:a,data:s}=e,o=new Uint8Array(t*n*3),u=r/t,l=i/n,h=p=>Math.max(0,Math.min(r-1,p)),c=p=>Math.max(0,Math.min(i-1,p));for(let p=0;p<n;p++){const m=(p+.5)*l-.5,g=Math.floor(m),y=sf(m-g);for(let _=0;_<t;_++){const $=(_+.5)*u-.5,x=Math.floor($),T=sf($-x),S=(p*t+_)*3;for(let I=0;I<3;I++){let E=0;for(let M=0;M<4;M++){const v=c(g-1+M)*r;let z=0;for(let B=0;B<4;B++)z+=T[B]*s[(v+h(x-1+B))*a+I];E+=y[M]*z}o[S+I]=Math.min(255,Math.max(0,Math.round(E)))}}}return o}function of(e,t){const n=ay(e.width,e.height,t),r=Kt(e,n.resizedWidth,n.resizedHeight),i=t*t,a=new Float32Array(3*i).fill(iy/255);for(let s=0;s<n.resizedHeight;s++){const o=(s+n.padY)*t+n.padX,u=s*n.resizedWidth;for(let l=0;l<n.resizedWidth;l++){const h=(u+l)*3,c=o+l;a[c]=r[h]/255,a[i+c]=r[h+1]/255,a[2*i+c]=r[h+2]/255}}return{tensor:a,params:n}}function uf(e,t,n,r){const i=[],a=Math.floor(e.length/6);for(let s=0;s<a;s++){const o=e[s*6],u=e[s*6+1],l=e[s*6+2],h=e[s*6+3],c=e[s*6+4],p=e[s*6+5];if(c<n)continue;const m=Math.round(p);if(m<0||m>=r)continue;const g=(o-t.padX)/t.scale,y=(u-t.padY)/t.scale,_=(l-t.padX)/t.scale,$=(h-t.padY)/t.scale;i.push({classIndex:m,confidence:c,box:[Math.trunc(g),Math.trunc(y),Math.trunc(_-g),Math.trunc($-y)],boxFloat:[g,y,_-g,$-y]})}return i}function Jn(e){const t=Math.floor(e);return e-t===.5?t%2===0?t:t+1:Math.round(e)}function lf(e){if(e.length===0)return Number.NaN;const t=[...e].sort((r,i)=>r-i),n=Math.floor(t.length/2);return t.length%2===1?t[n]:(t[n-1]+t[n])/2}function df(e,t){if(e.length===0)return Number.NaN;const n=[...e].sort((s,o)=>s-o),r=t/100*(n.length-1),i=Math.floor(r),a=Math.ceil(r);return i===a?n[i]:n[i]*(a-r)+n[a]*(r-i)}const oy=.6,uy=.74;function cf(e,t,n){const r=[],i=Math.floor(e.length/6);for(let a=0;a<i;a++){if(e[a*6+4]<n)continue;const o=(e[a*6]-t.padX)/t.scale,u=(e[a*6+1]-t.padY)/t.scale,l=(e[a*6+2]-t.padX)/t.scale,h=(e[a*6+3]-t.padY)/t.scale,c=Jn((o+l)/2),p=Jn((u+h)/2),m=Jn((l-o+(h-u))/4);m>=1&&r.push({cx:c,cy:p,r:m})}return r}function ly(e){const t=[];for(const n of[...e].sort((r,i)=>r.r-i.r)){const r=(oy*n.r)**2;t.every(i=>(n.cx-i.cx)**2+(n.cy-i.cy)**2>r)&&t.push(n)}return t}function dy(e){const t=[];for(const n of[...e].sort((r,i)=>i.r-r.r))t.every(r=>Math.hypot(n.cx-r.cx,n.cy-r.cy)>=uy*(n.r+r.r))&&t.push(n);return t}function cy(e){if(e.length===0)return[];const t=Math.max(1,Math.trunc(lf(e.map(n=>n.r))*1.5));return[...e].sort((n,r)=>{const i=Math.floor(n.cy/t),a=Math.floor(r.cy/t);return i!==a?i-a:n.cx-r.cx})}function hf(e,t,n){const r=cf(e,t,n);return r.length===0?[]:cy(dy(ly(r)))}function hy(e,t,n){return cf(e,t,n)}function Da(e,t,n){const r=[],i=Math.floor(e.length/6);for(let a=0;a<i;a++)e[a*6+4]<n||r.push([(e[a*6]-t.padX)/t.scale,(e[a*6+1]-t.padY)/t.scale,(e[a*6+2]-t.padX)/t.scale,(e[a*6+3]-t.padY)/t.scale]);return r}const pf=["brown","grey","blue","green","yellow","red","purple"],py={brown:"raw",grey:"manufactured",blue:"civilian",green:"scientific",yellow:"commercial",red:"military",purple:"guild"},fy=.7;function my(e){const t=e.map((i,a)=>a).sort((i,a)=>e[a].confidence-e[i].confidence),n=new Set,r=[];for(const i of t){const a=e[i],[s,o,u,l]=a.box;let h=!1;for(const c of r){const p=e[c];if(p.family!==a.family)continue;const[m,g,y,_]=p.box,$=Math.max(0,Math.min(s+u,m+y)-Math.max(s,m)),x=Math.max(0,Math.min(o+l,g+_)-Math.max(o,g)),T=Math.max(1,Math.min(u*l,y*_));if($*x>=fy*T){h=!0;break}}h?n.add(i):r.push(i)}return e.filter((i,a)=>!n.has(a))}function ff(e,t,n){const r=uf(e,t,n,pf.length).map(i=>{const a=pf[i.classIndex];return{color:a,family:py[a],box:i.box,confidence:i.confidence}});return my(r)}const gy=8,yy=.8,mf=1.25;function wy(e){if(e.length<gy)return[];const t=[],n=[];for(const s of e){const[,,o,u]=s.box;o>u*mf?t.push(s):u>o*mf&&n.push(s)}const[r,i,a]=t.length>=n.length?[t,n,"vertical"]:[n,t,"horizontal"];return r.length<yy*e.length||i.length===0?[]:i.map(s=>({family:s.family,color:s.color,box:[...s.box],reason:`${s.color} banner sits ${a} while ${r.length}/${e.length} of the tableau faces the other way — probably a stray card poking into the frame`}))}const _y=2.25,gf=8;function by(e){if(e.length<gf)return[];const t=e.map(c=>[c.box[0]+c.box[2]/2,c.box[1]+c.box[3]/2]),n=e.map(c=>Math.hypot(c.box[2],c.box[3])).sort((c,p)=>c-p),r=_y*n[Math.floor(n.length/2)],i=r*r,a=e.map((c,p)=>p),s=c=>{for(;a[c]!==c;)a[c]=a[a[c]],c=a[c];return c};for(let c=0;c<e.length;c++)for(let p=c+1;p<e.length;p++){const m=t[c][0]-t[p][0],g=t[c][1]-t[p][1];m*m+g*g<=i&&(a[s(c)]=s(p))}const o=new Map;for(let c=0;c<e.length;c++){const p=s(c);o.set(p,[...o.get(p)??[],c])}let u=[];for(const c of o.values())c.length>u.length&&(u=c);if(u.length<gf||u.length===e.length)return[];const l=new Set(u),h=e.map((c,p)=>p).filter(c=>!l.has(c));return h.map(c=>({family:e[c].family,color:e[c].color,box:[...e[c].box],reason:`${e[c].color} banner sits in a detached group of ${h.length}, away from the ${u.length}-card tableau — probably the draw/discard pile, not this player's city`}))}const kt={banner:{onnx:"banner_yolo.onnx",input:1280,conf:.5},coin:{onnx:"coin_yolo.onnx",input:1280,conf:.25},laurel:{onnx:"laurel_yolo.onnx",input:1280,conf:.25},token:{onnx:"token_yolo.onnx",input:1280,conf:.4},wonder:{onnx:"wonder_yolo.onnx",input:1280,conf:.3}};function $t(e,t,n){const r=Math.max(e,t,n),i=Math.min(e,t,n),a=r-i,s=r===0?0:Math.round(255*a/r);if(a===0)return{h:0,s,v:r};let o;return r===e?o=60*(t-n)/a:r===t?o=120+60*(n-e)/a:o=240+60*(e-t)/a,o<0&&(o+=360),{h:Math.round(o/2),s,v:r}}const $y=.42,xy=22,vy=43,Sy=120,Ty=1.5,Iy=.72,ky=110,yf=3;function er(e,t,n){const{width:r,height:i,channels:a,data:s}=e;if(r<4||i<4)return 0;const o=Math.floor(r/2),u=Math.floor(i/2),l=Math.trunc(Math.min(r,i)*$y);if(l<1)return 0;let h=0;for(let c=0;c<i;c++)for(let p=0;p<r;p++){if((p-o)**2+(c-u)**2>l*l)continue;const m=(c*r+p)*a,g=s[m],y=s[m+1],_=s[m+2];!t&&g>=250&&y>=250&&_>=250||(n(g,y,_),h+=1)}return h}function Ey(e){let t=0,n=0,r=0,i=er(e,!1,(a,s,o)=>{const u=$t(a,s,o);t+=u.h,n+=u.s,r+=u.v});return i===0&&(i=er(e,!0,(a,s,o)=>{const u=$t(a,s,o);t+=u.h,n+=u.s,r+=u.v})),i===0?null:{h:t/i,s:n/i,v:r/i}}function My(e){let t=0,n=0,r=er(e,!1,(a,s)=>{t+=a,n+=s});if(r===0&&(r=er(e,!0,(a,s)=>{t+=a,n+=s})),r===0)return null;const i=n/r;return i<=1e-6?null:t/r/i}function Cy(e){let t=0;const n=er(e,!0,(r,i,a)=>{t+=$t(r,i,a).s});return n===0?null:t/n}function Ay(e){const t=Ey(e);if(t===null||t.s<=xy)return 1;if(t.s>=Sy){const n=My(e);return n!==null&&n>=Ty?6:3}return t.s>=vy?3:6}function Ry(e,t){const n=[...t];if(e.length!==3||t.length!==3||new Set(t).size===3&&t.every(s=>[1,3,6].includes(s)))return n;const r=e.map(s=>s.r).sort((s,o)=>s-o);if(r[0]<=0||!(r[1]>=r[0]*1.12&&r[2]>=r[1]*1.12))return n;const i=[0,1,2].sort((s,o)=>e[s].r-e[o].r),a=new Map([[i[0],1],[i[1],3],[i[2],6]]);return[0,1,2].map(s=>a.get(s))}function zy(e,t){const n=[...t];if(e.length<yf||t.length!==e.length)return n;const r=e.map(s=>Cy(s)),i=r.filter(s=>s!==null);if(i.length<yf)return n;const a=lf(i);return a<=0||r.forEach((s,o)=>{s!==null&&n[o]!==1&&s<Iy*a&&s<ky&&(n[o]=1)}),n}function wf(e,t){const{cx:n,cy:r,r:i}=t,a=Math.max(0,n-i),s=Math.max(0,r-i),o=Math.min(e.width,n+i),u=Math.min(e.height,r+i),l=Math.max(0,o-a),h=Math.max(0,u-s),c=new Uint8Array(l*h*3);for(let p=0;p<h;p++)for(let m=0;m<l;m++){const g=(p*l+m)*3;if((m+a-n)**2+(p+s-r)**2<=i*i){const _=((p+s)*e.width+(m+a))*e.channels;c[g]=e.data[_],c[g+1]=e.data[_+1],c[g+2]=e.data[_+2]}else c[g]=255,c[g+1]=255,c[g+2]=255}return{width:l,height:h,channels:3,data:c}}function Oy(e,t){const n=t.map(a=>wf(e,a)),r=n.map(a=>Ay(a)),i=Ry(t,r);return zy(n,i)}function Ny(e){const{width:t,height:n,channels:r,data:i}=e,a=new Uint8Array(t*n);for(let s=0,o=0;s<a.length;s++,o+=r)a[s]=i[o]*4899+i[o+1]*9617+i[o+2]*1868+8192>>14;return{width:t,height:n,data:a}}function _f(e,t,n){const r=new Uint8Array(t*n),i=e.width/t,a=e.height/n;for(let s=0;s<n;s++){const o=s*a,u=Math.min((s+1)*a,e.height);for(let l=0;l<t;l++){const h=l*i,c=Math.min((l+1)*i,e.width);let p=0,m=0;for(let g=Math.floor(o);g<u;g++){const y=Math.min(g+1,u)-Math.max(g,o);if(!(y<=0))for(let _=Math.floor(h);_<c;_++){const $=Math.min(_+1,c)-Math.max(_,h);$<=0||(p+=e.data[g*e.width+_]*$*y,m+=$*y)}}r[s*t+l]=Math.min(255,Math.max(0,Jn(p/m)))}}return{width:t,height:n,data:r}}function By(e){const t=new Array(256).fill(0);for(const u of e.data)t[u]+=1;const n=e.data.length;let r=0;for(;r<256&&t[r]===0;)r+=1;const i=new Uint8Array(n);if(r>=255||t[r]===n)return i.fill(r<256?r:0),{width:e.width,height:e.height,data:i};const a=255/(n-t[r]),s=new Uint8Array(256);let o=0;for(let u=r+1;u<256;u++)o+=t[u],s[u]=Math.min(255,Math.max(0,Jn(o*a)));for(let u=0;u<n;u++)i[u]=s[e.data[u]];return{width:e.width,height:e.height,data:i}}function Py(e){const{width:t,height:n,data:r}=e,i=new Uint8Array(t*n);for(let a=0;a<n;a++)for(let s=0;s<t;s++){let o=!0;for(let u=-1;u<=1&&o;u++)for(let l=-1;l<=1;l++){const h=s+l,c=a+u;if(!(h<0||h>=t||c<0||c>=n)&&r[c*t+h]===0){o=!1;break}}i[a*t+s]=o&&r[a*t+s]>0?255:0}return{width:t,height:n,data:i}}function Dy(e){const{width:t,height:n,data:r}=e,i=new Uint8Array(t*n);for(let a=0;a<n;a++)for(let s=0;s<t;s++){let o=!1;for(let u=-1;u<=1&&!o;u++)for(let l=-1;l<=1;l++){const h=s+l,c=a+u;if(h>=0&&h<t&&c>=0&&c<n&&r[c*t+h]>0){o=!0;break}}i[a*t+s]=o?255:0}return{width:t,height:n,data:i}}function Ua(e){const{width:t,height:n,data:r}=e,i=new Int32Array(t*n),a=[],s=new Int32Array(t*n);let o=1;for(let u=0;u<r.length;u++){if(r[u]===0||i[u]!==0)continue;let l=0,h=0;s[h++]=u,i[u]=o;let c=0,p=0,m=0;for(;l<h;){const g=s[l++],y=g%t,_=g/t|0;c+=1,p+=y,m+=_;for(let $=-1;$<=1;$++)for(let x=-1;x<=1;x++){if(x===0&&$===0)continue;const T=y+x,S=_+$;if(T<0||T>=t||S<0||S>=n)continue;const I=S*t+T;r[I]>0&&i[I]===0&&(i[I]=o,s[h++]=I)}}a[o]={area:c,centroidX:p/c,centroidY:m/c},o+=1}return{labels:i,stats:a}}function Uy(e,t,n){return bf(Float32Array.from(e.data),e.width,t,n)}function bf(e,t,n,r){const i=new Float32Array(t*t),a=t/2,s=-n*Math.PI/180,o=Math.cos(s),u=Math.sin(s);for(let l=0;l<t;l++)for(let h=0;h<t;h++){const c=h-a,p=l-a,m=o*c-u*p+a,g=u*c+o*p+a,y=Math.floor(m),_=Math.floor(g),$=m-y,x=g-_,T=(E,M)=>E>=0&&E<t&&M>=0&&M<t?e[M*t+E]:r,S=T(y,_)*(1-$)+T(y+1,_)*$,I=T(y,_+1)*(1-$)+T(y+1,_+1)*$;i[l*t+h]=S*(1-x)+I*x}return i}const Ly=.9,Fy=.34,Gy=[.55,.6,.66,.72],Wy=22,qy=88,Vy=35,Mn=28,La=4,Hy=Array.from({length:15},(e,t)=>-21+t*3),$f=[-2,0,2],jy=3,Ky=.3;function Yy(e){return e.templates.flatMap(({label:t,bits:n})=>{const r=Uint8Array.from(atob(n),i=>i.charCodeAt(0));return r.length!==e.size*e.size?[]:[{label:t,bits:Float32Array.from(r)}]})}function Xy(e){let t=e.width,n=-1,r=e.height,i=-1,a=0;for(let y=0;y<e.height;y++)for(let _=0;_<e.width;_++)e.data[y*e.width+_]>0&&(a+=1,t=Math.min(t,_),n=Math.max(n,_),r=Math.min(r,y),i=Math.max(i,y));if(a<8)return null;const s=n-t+1,o=i-r+1,u=Math.max(o,s),l=new Uint8Array(u*u),h=Math.floor((u-s)/2),c=Math.floor((u-o)/2);for(let y=0;y<o;y++)for(let _=0;_<s;_++)l[(y+c)*u+(_+h)]=e.data[(y+r)*e.width+(_+t)];const p=Mn-2*La,m=_f({width:u,height:u,data:l},p,p),g=new Float32Array(Mn*Mn);for(let y=0;y<p;y++)for(let _=0;_<p;_++)g[(y+La)*Mn+(_+La)]=m.data[y*p+_]>110?1:0;return g}function Zy(e,t){const{width:n,height:r,channels:i,data:a}=e,s=Math.floor(r/2),o=Math.floor(n/2),u=Math.trunc(Math.min(n,r)*Fy);if(u<4)return null;const l=s-u,h=o-u,c=2*u,p=2*u;if(c<6||p<6)return null;const m=new Int16Array(c*p),g=new Int16Array(c*p),y=new Int16Array(c*p),_=new Uint8Array(c*p),$=[],x=Math.min(c,p)/2;for(let O=0;O<c;O++)for(let Z=0;Z<p;Z++){const D=((O+l)*n+(Z+h))*i,{h:V,s:F,v:L}=$t(a[D],a[D+1],a[D+2]),ne=O*p+Z;m[ne]=V,g[ne]=F,y[ne]=L,Math.sqrt((Z-p/2)**2+(O-c/2)**2)/x<=t&&(_[ne]=1,$.push(L))}if($.length<16)return null;const T=df($,55);let S=0,I=0,E=0;const M=O=>m[O]>=Wy&&m[O]<=qy&&g[O]>=Vy,v=O=>y[O]>=T&&g[O]<=95&&!M(O)&&_[O]===1;for(let O=0;O<c*p;O++)_[O]===1&&(E+=1,y[O]>=130&&!M(O)&&(S+=1),v(O)&&(I+=1));const z=S>.5*E&&I<.15*E,B=new Uint8Array(c*p);if(z){const O=df($,45);for(let Z=0;Z<c*p;Z++)B[Z]=_[Z]===1&&y[Z]<=O?255:0}else for(let O=0;O<c*p;O++)B[O]=v(O)?255:0;const X={width:p,height:c,data:B},U=Py(X);let H=Ua(U),A=H;if(H.stats.length<=1&&(H=Ua(X),A=H,H.stats.length<=1))return null;const G=Math.min(c,p)/2;let R=0,P=-1;for(let O=1;O<A.stats.length;O++){const Z=A.stats[O];if(Z===void 0)continue;const D=Math.hypot(Z.centroidX-p/2,Z.centroidY-c/2)/G,V=Z.area*(1-.6*Math.min(D,1));V>P&&(P=V,R=O)}if(R===0)return null;const Y=new Uint8Array(c*p);for(let O=0;O<c*p;O++)Y[O]=A.labels[O]===R?255:0;return Xy(Dy({width:p,height:c,data:Y}))}function Qy(e,t,n,r,i,a){const s=Mn;let o=0,u=0;for(let l=0;l<s;l++){const h=l-a;if(!(h<0||h>=s))for(let c=0;c<s;c++){const p=c-i;if(p<0||p>=s)continue;const m=e[h*s+p];m!==0&&(u+=m,o+=m*n[l*s+c])}}return o/(u+r-o+1e-6)}function Jy(e,t){const n=t.reduce((i,a)=>i+a,0);let r=-1;for(const i of Hy){const a=i===0?e:bf(e,Mn,i,0),s=a.reduce((o,u)=>o+u,0);for(const o of $f)for(const u of $f){const l=Qy(a,s,t,n,o,u);l>r&&(r=l)}}return r}function ew(e,t){if(t.length===0||Math.min(e.width,e.height)<8)return[null,0];const n=[];for(const s of Gy){const o=Zy(e,s);if(o!==null)for(const{label:u,bits:l}of t)n.push([Jy(o,l),u])}if(n.length===0)return[null,0];if(n.sort((s,o)=>o[0]-s[0]),n[0][0]<Ky)return[null,0];const r=new Map;for(const[s,o]of n.slice(0,jy))r.set(o,(r.get(o)??0)+s);let i=0,a=-1;for(const[s,o]of r)o>a&&(a=o,i=s);return[i,n[0][0]]}const tw=2560,nw=.3,rw=.5,iw=1.6,aw=3,sw=5;function ow(e){const t=Math.min(1,tw/Math.max(e.width,e.height)),n=Math.max(32,Math.round(e.width*t/32)*32),r=Math.max(32,Math.round(e.height*t/32)*32),i=n*r,a=new Float32Array(3*i),s=e.width/n,o=e.height/r;for(let u=0;u<r;u++){const l=(u+.5)*o-.5,h=Math.max(0,Math.min(e.height-1,Math.floor(l))),c=Math.min(e.height-1,h+1),p=Math.max(0,Math.min(1,l-h));for(let m=0;m<n;m++){const g=(m+.5)*s-.5,y=Math.max(0,Math.min(e.width-1,Math.floor(g))),_=Math.min(e.width-1,y+1),$=Math.max(0,Math.min(1,g-y));for(let x=0;x<3;x++){const T=2-x,S=(h*e.width+y)*e.channels+T,I=(h*e.width+_)*e.channels+T,E=(c*e.width+y)*e.channels+T,M=(c*e.width+_)*e.channels+T,v=e.data[S]*(1-$)+e.data[I]*$,z=e.data[E]*(1-$)+e.data[M]*$,B=v*(1-p)+z*p;a[x*i+u*n+m]=(B/255-.5)/.5}}}return{tensor:a,width:n,height:r}}function uw(e,t,n){const r=new Uint8Array(e.length);for(let i=0;i<n;i++){const a=i===n-1;for(let s=0;s<t;s++){const o=i*t+s;let u=e[o];if(s+1<t&&e[o+1]>u&&(u=e[o+1]),!a){const l=o+t;e[l]>u&&(u=e[l]),s+1<t&&e[l+1]>u&&(u=e[l+1])}r[o]=u}}return r}function lw(e){if(e.length<3)return e;const t=[...e].sort((a,s)=>a[0]-s[0]||a[1]-s[1]),n=(a,s,o)=>(s[0]-a[0])*(o[1]-a[1])-(s[1]-a[1])*(o[0]-a[0]),r=[];for(const a of t){for(;r.length>=2&&n(r[r.length-2],r[r.length-1],a)<=0;)r.pop();r.push(a)}const i=[];for(let a=t.length-1;a>=0;a--){const s=t[a];for(;i.length>=2&&n(i[i.length-2],i[i.length-1],s)<=0;)i.pop();i.push(s)}return r.pop(),i.pop(),r.concat(i)}function dw(e){if(e.length===1)return{cx:e[0][0],cy:e[0][1],w:0,h:0,angle:0};let t=null,n=1/0;for(let r=0;r<e.length;r++){const[i,a]=e[r],[s,o]=e[(r+1)%e.length],u=s-i,l=o-a,h=Math.hypot(u,l);if(h===0)continue;const c=u/h,p=l/h;let m=1/0,g=-1/0,y=1/0,_=-1/0;for(const[S,I]of e){const E=S*c+I*p,M=-S*p+I*c;E<m&&(m=E),E>g&&(g=E),M<y&&(y=M),M>_&&(_=M)}const $=g-m,x=_-y,T=$*x;if(T<n){n=T;const S=(m+g)/2,I=(y+_)/2;t={cx:S*c-I*p,cy:S*p+I*c,w:$,h:x,angle:Math.atan2(p,c)}}}return t}function cw(e,t,n,r){const i=Math.cos(r.angle),a=Math.sin(r.angle),s=r.w/2,o=r.h/2,u=Math.abs(s*i)+Math.abs(o*a),l=Math.abs(s*a)+Math.abs(o*i),h=Math.max(0,Math.floor(r.cx-u)),c=Math.min(t-1,Math.ceil(r.cx+u)),p=Math.max(0,Math.floor(r.cy-l)),m=Math.min(n-1,Math.ceil(r.cy+l));let g=0,y=0;for(let _=p;_<=m;_++)for(let $=h;$<=c;$++){const x=$-r.cx,T=_-r.cy,S=x*i+T*a,I=-x*a+T*i;Math.abs(S)<=s&&Math.abs(I)<=o&&(g+=e[_*t+$],y+=1)}return y===0?0:g/y}function hw(e){const t=Math.cos(e.angle),n=Math.sin(e.angle),r=e.w/2,i=e.h/2,s=[...[[e.cx+-r*t- -i*n,e.cy+-r*n+-i*t],[e.cx+r*t- -i*n,e.cy+r*n+-i*t],[e.cx+r*t-i*n,e.cy+r*n+i*t],[e.cx+-r*t-i*n,e.cy+-r*n+i*t]]].sort((y,_)=>y[0]-_[0]),[o,u,l,h]=s,[c,p]=o[1]<=u[1]?[o,u]:[u,o],[m,g]=l[1]<=h[1]?[l,h]:[h,l];return[[c[0],c[1]],[m[0],m[1]],[g[0],g[1]],[p[0],p[1]]]}function pw(e,t,n,r){const{width:i,height:a}=t;let s=new Uint8Array(i*a);for(let m=0;m<s.length;m++)s[m]=e[m]>nw?255:0;s=uw(s,i,a);const o={width:i,height:a,data:s},{labels:u}=Ua(o),l=new Map;for(let m=0;m<a;m++)for(let g=0;g<i;g++){const y=u[m*i+g];if(y===0)continue;let _=l.get(y);_===void 0&&(_=new Map,l.set(y,_));const $=_.get(m);$===void 0?_.set(m,[g,g]):(g<$[0]&&($[0]=g),g>$[1]&&($[1]=g))}const h=n/i,c=r/a,p=[];for(const[m,g]of l){const y=[];for(const[B,[X,U]]of g)y.push([X-.5,B-.5],[X-.5,B+.5],[U+.5,B-.5],[U+.5,B+.5]);const _=dw(lw(y));if(Math.min(_.w,_.h)<aw)continue;const $=cw(e,i,a,_);if($<rw)continue;const x=_.w*_.h*iw/(2*(_.w+_.h)),T={..._,w:_.w+2*x,h:_.h+2*x};if(Math.min(T.w,T.h)<sw+2)continue;const I=hw(T).map(([B,X])=>[Math.min(n,Math.max(0,Math.round(B*h))),Math.min(r,Math.max(0,Math.round(X*c)))]),E=I.map(B=>B[0]),M=I.map(B=>B[1]),v=Math.min(...E),z=Math.min(...M);p.push({quad:I,x:v,y:z,width:Math.max(...E)-v,height:Math.max(...M)-z,score:$})}return p.sort((m,g)=>g.score-m.score)}function fw(e,t){const[n,r,i,a]=t,s=Math.max(1,Math.round(Math.max(Math.hypot(r[0]-n[0],r[1]-n[1]),Math.hypot(i[0]-a[0],i[1]-a[1])))),o=Math.max(1,Math.round(Math.max(Math.hypot(a[0]-n[0],a[1]-n[1]),Math.hypot(i[0]-r[0],i[1]-r[1])))),u=mw([[0,0],[s,0],[s,o],[0,o]],[n,r,i,a]),l=new Uint8Array(s*o*e.channels);for(let c=0;c<o;c++)for(let p=0;p<s;p++){const m=u[6]*p+u[7]*c+u[8],g=(u[0]*p+u[1]*c+u[2])/m,y=(u[3]*p+u[4]*c+u[5])/m,_=Math.floor(g),$=Math.floor(y),x=g-_,T=y-$,S=Math.max(0,Math.min(e.width-1,_)),I=Math.max(0,Math.min(e.width-1,_+1)),E=Math.max(0,Math.min(e.height-1,$)),M=Math.max(0,Math.min(e.height-1,$+1));for(let v=0;v<e.channels;v++){const z=e.data[(E*e.width+S)*e.channels+v],B=e.data[(E*e.width+I)*e.channels+v],X=e.data[(M*e.width+S)*e.channels+v],U=e.data[(M*e.width+I)*e.channels+v],H=z*(1-x)+B*x,A=X*(1-x)+U*x;l[(c*s+p)*e.channels+v]=Math.round(H*(1-T)+A*T)}}const h={width:s,height:o,channels:e.channels,data:l};return o/s>=1.5?Yt(h,3):h}function mw(e,t){const n=[],r=[];for(let i=0;i<4;i++){const[a,s]=e[i],[o,u]=t[i];n.push([a,s,1,0,0,0,-o*a,-o*s]),r.push(o),n.push([0,0,0,a,s,1,-u*a,-u*s]),r.push(u)}for(let i=0;i<8;i++){let a=i;for(let o=i+1;o<8;o++)Math.abs(n[o][i])>Math.abs(n[a][i])&&(a=o);[n[i],n[a]]=[n[a],n[i]],[r[i],r[a]]=[r[a],r[i]];const s=n[i][i];for(let o=i;o<8;o++)n[i][o]/=s;r[i]/=s;for(let o=0;o<8;o++){if(o===i)continue;const u=n[o][i];if(u!==0){for(let l=i;l<8;l++)n[o][l]-=u*n[i][l];r[o]-=u*r[i]}}}return[r[0],r[1],r[2],r[3],r[4],r[5],r[6],r[7],1]}function Yt(e,t){const n=(t%4+4)%4;if(n===0)return e;const{width:r,height:i,channels:a,data:s}=e,o=n%2===0?r:i,u=n%2===0?i:r,l=new Uint8Array(o*u*a);for(let h=0;h<i;h++)for(let c=0;c<r;c++){let p,m;n===1?(p=i-1-h,m=c):n===2?(p=r-1-c,m=i-1-h):(p=h,m=r-1-c);const g=(h*r+c)*a,y=(m*o+p)*a;for(let _=0;_<a;_++)l[y+_]=s[g+_]}return{width:o,height:u,channels:a,data:l}}const gw=.6;(()=>{const e=new Uint8Array(256);for(let t=0;t<256;t++)e[t]=Math.min(255,Math.round(Math.pow(t/255,gw)*255));return e})();const Pt=48,yw=320;function ww(e){return["blank",...e.characters," "]}function _w(e,t,n){let r="";const i=[];for(let s=0;s<e.length;s++){const o=e[s];o!==0&&(s>0&&e[s-1]===o||(r+=n[o]??"",i.push(t[s])))}if(i.length===0)return["",0];const a=i.reduce((s,o)=>s+o,0)/i.length;return[r,a]}function bw(e,t){const n=Math.trunc(Pt*t),r=e.width/e.height,i=Math.ceil(Pt*r)>n?n:Math.ceil(Pt*r),a=new Float32Array(3*Pt*n),s=Pt*n,o=e.width/i,u=e.height/Pt;for(let l=0;l<Pt;l++){const h=(l+.5)*u-.5,c=Math.max(0,Math.min(e.height-1,Math.floor(h))),p=Math.min(e.height-1,c+1),m=Math.max(0,Math.min(1,h-c));for(let g=0;g<i;g++){const y=(g+.5)*o-.5,_=Math.max(0,Math.min(e.width-1,Math.floor(y))),$=Math.min(e.width-1,_+1),x=Math.max(0,Math.min(1,y-_));for(let T=0;T<3;T++){const S=2-T,I=(c*e.width+_)*e.channels+S,E=(c*e.width+$)*e.channels+S,M=(p*e.width+_)*e.channels+S,v=(p*e.width+$)*e.channels+S,z=e.data[I]*(1-x)+e.data[E]*x,B=e.data[M]*(1-x)+e.data[v]*x,X=z*(1-m)+B*m;a[T*s+l*n+g]=(X/255-.5)/.5}}}return{tensor:a,width:n}}const $w=62,xw=8,vw=5;function Fa(e){return e?e.normalize("NFKD").replace(new RegExp("\\p{M}","gu"),"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim():""}function Sw(e,t){const n=e.length,r=t.length;if(n===0||r===0)return 0;let i=new Int32Array(r+1),a=new Int32Array(r+1);for(let s=1;s<=n;s++){for(let o=1;o<=r;o++)a[o]=e[s-1]===t[o-1]?i[o-1]+1:Math.max(i[o],a[o-1]);[i,a]=[a,i]}return i[r]}function Rr(e,t){return e.length===0&&t.length===0?100:200*Sw(e,t)/(e.length+t.length)}function xf(e,t){const n=r=>r.split(/\s+/).filter(Boolean).sort().join(" ");return Rr(n(e),n(t))}function Tw(e,t){const n=new Set(e.split(/\s+/).filter(Boolean)),r=new Set(t.split(/\s+/).filter(Boolean)),i=[...n].filter(h=>r.has(h)).sort(),a=[...n].filter(h=>!r.has(h)).sort(),s=[...r].filter(h=>!n.has(h)).sort(),o=i.join(" "),u=[o,a.join(" ")].filter(Boolean).join(" "),l=[o,s.join(" ")].filter(Boolean).join(" ");return o.length>0&&(a.length===0||s.length===0)?100:Math.max(Rr(o,u),Rr(o,l),Rr(u,l))}function Iw(e){const t=new Set,n=[];for(const r of e){const i=r.nameFr??r.name;for(const a of[Fa(i),Fa(r.name)])if(a)for(const s of[a,a.replace(/ /g,"")])s&&!t.has(s)&&(t.add(s),n.push({key:s,id:r.id,display:i,...r.kind!==void 0?{kind:r.kind}:{}}))}return n}function kw(e,t){const n=Fa(e);if(!n||t.length===0)return null;const i=Iw(t).map(h=>({...h,score:Tw(n,h.key)})).sort((h,c)=>c.score-h.score).slice(0,xw).filter(h=>h.score>=$w);if(i.length===0)return null;const a=i[0].score,s=i.filter(h=>a-h.score<=vw),o=[...new Set(n.split(/\s+/).filter(Boolean))].join(" ");let u=s[0],l=[xf(o,u.key),u.score];for(const h of s.slice(1)){const c=[xf(o,h.key),h.score];(c[0]>l[0]||c[0]===l[0]&&c[1]>l[1])&&(u=h,l=c)}return{id:u.id,name:u.display,...u.kind!==void 0?{kind:u.kind}:{},confidence:Math.round(u.score/100*1e4)/1e4}}const vf=5e3,Ga=.75,Sf=15,Ew=1.25,Mw=2.4,Cw=.003,Aw=.85,Rw=4,Wa=2600,qa=2,Va=.3,Tf=.1,If=.012,zw=22,kf=.5,zr=.12;function Ye(e,t){const n=new e.Mat(t.height,t.width,e.CV_8UC3),r=n.data,i=t.channels;for(let a=0,s=t.width*t.height;a<s;a++)r[a*3]=t.data[a*i],r[a*3+1]=t.data[a*i+1],r[a*3+2]=t.data[a*i+2];return n}function Ow(e,t,n,r){const i=r.map(re=>re[0]),a=r.map(re=>re[1]),s=i.reduce((re,fe)=>re+fe,0)/i.length,o=a.reduce((re,fe)=>re+fe,0)/a.length,u=Math.max(Math.max(...i)-Math.min(...i),Math.max(...a)-Math.min(...a));if(u<4)return null;const l=u*Rw,h=Math.max(0,Math.trunc(s-l)),c=Math.min(n.width,Math.trunc(s+l)),p=Math.max(0,Math.trunc(o-l)),m=Math.min(n.height,Math.trunc(o+l));if(c-h<8||m-p<8)return null;const g=Math.max(n.width,n.height)<Wa?qa:1,y=Ye(e,n),_=Ye(e,t),$=new e.Rect(h,p,c-h,m-p),x=y.roi($),T=new e.Mat;g!==1?e.resize(x,T,new e.Size(0,0),g,g,e.INTER_CUBIC):x.copyTo(T);const S=new e.Mat,I=new e.Mat;e.cvtColor(_,S,e.COLOR_RGB2GRAY),e.cvtColor(T,I,e.COLOR_RGB2GRAY);const E=new e.ORB(vf),M=new e.KeyPointVector,v=new e.KeyPointVector,z=new e.Mat,B=new e.Mat,X=new e.Mat,U=[y,_,x,T,S,I,M,v,z,B,X],H=re=>{for(const fe of U)try{fe.delete()}catch{}try{E.delete()}catch{}return re};if(E.detectAndCompute(S,X,M,z),E.detectAndCompute(I,X,v,B),z.rows<8||B.rows<8)return H(null);const A=new e.BFMatcher(e.NORM_HAMMING),G=new e.DMatchVectorVector;A.knnMatch(z,B,G,2);const R=[],P=[];for(let re=0;re<G.size();re++){const fe=G.get(re);if(fe.size()===2){const Ae=fe.get(0),Ge=fe.get(1);if(Ae.distance<Ga*Ge.distance){const Be=M.get(Ae.queryIdx).pt,Je=v.get(Ae.trainIdx).pt;R.push(Be.x,Be.y),P.push(Je.x,Je.y)}}}if(G.delete(),A.delete(),R.length/2<8)return H(null);const Y=e.matFromArray(R.length/2,1,e.CV_32FC2,R),O=e.matFromArray(P.length/2,1,e.CV_32FC2,P),Z=new e.Mat,D=e.findHomography(Y,O,e.RANSAC,5,Z);let V=0;for(let re=0;re<Z.rows;re++)V+=Z.data[re];const F=D.rows===3?[...D.data64F]:null;if(Y.delete(),O.delete(),Z.delete(),D.delete(),F===null||V<Sf)return H(null);const L=1/g,ne=[[L,0,h],[0,L,p],[0,0,1]],se=[0,1,2].map(re=>[0,1,2].map(fe=>ne[re][0]*F[fe]+ne[re][1]*F[3+fe]+ne[re][2]*F[6+fe]));return H({H:se,inliers:V})}function Ha(e,t,n){if(e.length!==4||e.some(u=>!Number.isFinite(u[0])||!Number.isFinite(u[1])))return!1;let r=0;for(let u=0;u<4;u++){const[l,h]=e[u],[c,p]=e[(u+1)%4];r+=l*p-c*h}const i=Math.abs(r/2)/(t*n);if(i<Cw||i>Aw)return!1;const a=e.map((u,l)=>{const h=e[(l+1)%4];return Math.hypot(h[0]-u[0],h[1]-u[1])}),s=Math.min(...a);if(s<1)return!1;const o=Math.max(...a)/s;return o>=Ew&&o<=Mw}function ja(e,t,n){const r=e[2][0]*t+e[2][1]*n+e[2][2];return[(e[0][0]*t+e[0][1]*n+e[0][2])/r,(e[1][0]*t+e[1][1]*n+e[1][2])/r]}function Ka(e,t,n,r){const i=n.width,a=n.height,s=Math.max(8,Math.trunc(Va*i)),o=i+2*s,u=a+2*s;if(o*u>4e7)return null;const l=r.map(U=>[U[0],U[1],U[2]-s*(U[0]+U[1])+0]);for(let U=0;U<3;U++)l[U][2]=r[U][2]-s*r[U][0]-s*r[U][1];const h=Ye(e,t),c=new e.Mat,p=e.matFromArray(3,3,e.CV_64F,l.flat());e.warpPerspective(h,c,p,new e.Size(o,u),e.WARP_INVERSE_MAP);const m=new e.Mat;e.cvtColor(c,m,e.COLOR_RGB2Lab),h.delete(),p.delete();const g=m.data,y=Math.max(4,Math.trunc(s/3)),_=[[],[],[]],$=(U,H)=>{const A=(H*o+U)*3;_[0].push(g[A]),_[1].push(g[A+1]),_[2].push(g[A+2])};for(let U=0;U<u;U++)for(let H=0;H<o;H++)(U<y||U>=u-y||H<y||H>=o-y)&&$(H,U);const x=U=>{U.sort((A,G)=>A-G);const H=U.length>>1;return U.length%2?U[H]:(U[H-1]+U[H])/2},T=[x(_[0]),x(_[1]),x(_[2])],S=(U,H)=>{const A=(H*o+U)*3,G=g[A]-T[0],R=g[A+1]-T[1],P=g[A+2]-T[2];return Math.sqrt(G*G+R*R+P*P)>zw},I=Math.max(6,Math.trunc(Tf*i)),E=Math.max(6,Math.trunc(Tf*a)),M=Math.max(2,Math.trunc(If*i)),v=Math.max(2,Math.trunc(If*a)),z=U=>{let H=0,A=0;for(const G of U)A=G?A+1:0,A>H&&(H=A);return H/Math.max(1,U.length)},B=U=>{let H,A,G,R,P;if(U==="L"?(H=s,A=s+a,G=Math.max(0,s-M-I),R=Math.max(0,s-M),P=!1):U==="R"?(H=s,A=s+a,G=s+i+M,R=Math.min(o,s+i+M+I),P=!1):(H=Math.max(0,s-v-E),A=Math.max(0,s-v),G=s,R=s+i,P=!0),A<=H||R<=G)return 0;const Y=[];if(P)for(let O=G;O<R;O++){let Z=0;for(let D=H;D<A;D++)S(O,D)&&Z++;Y.push(Z/(A-H)>kf)}else for(let O=H;O<A;O++){let Z=0;for(let D=G;D<R;D++)S(D,O)&&Z++;Y.push(Z/(R-G)>kf)}return z(Y)},X={L:B("L"),R:B("R"),T:B("T")};return c.delete(),m.delete(),X}const Nw=6e3,Bw=8,Ef=.5,Pw=.6;function Dw(e,t,n,r){if(n.size===0)return[];const i=Math.max(t.width,t.height)<Wa?qa:1,a=Ye(e,t),s=new e.Mat;i!==1?e.resize(a,s,new e.Size(0,0),i,i,e.INTER_CUBIC):a.copyTo(s);const o=new e.Mat;e.cvtColor(s,o,e.COLOR_RGB2GRAY),a.delete(),s.delete();const u=new e.ORB(Nw),l=new e.Mat,h=new e.KeyPointVector,c=new e.Mat;u.detectAndCompute(o,l,h,c);const p=[],m=new e.BFMatcher(e.NORM_HAMMING);try{if(c.rows<8)return p;for(const[g,y]of n){if(r!==void 0&&Date.now()>r)break;const _=Ye(e,y),$=new e.Mat;e.cvtColor(_,$,e.COLOR_RGB2GRAY);const x=new e.KeyPointVector,T=new e.Mat;u.detectAndCompute($,l,x,T);const S=[_,x,T],I=()=>{for(const se of S)se.delete();$.delete()};if(T.rows<8){I();continue}const E=new e.DMatchVectorVector;m.knnMatch(T,c,E,2);const M=[],v=[];for(let se=0;se<E.size();se++){const re=E.get(se);if(re.size()===2){const fe=re.get(0);if(fe.distance<Ga*re.get(1).distance){const Ae=x.get(fe.queryIdx).pt,Ge=h.get(fe.trainIdx).pt;M.push(Ae.x,Ae.y),v.push(Ge.x,Ge.y)}}}if(E.delete(),M.length/2<8){I();continue}const z=e.matFromArray(M.length/2,1,e.CV_32FC2,M),B=e.matFromArray(v.length/2,1,e.CV_32FC2,v),X=new e.Mat,U=e.findHomography(z,B,e.RANSAC,5,X);let H=0;for(let se=0;se<X.rows;se++)H+=X.data[se];const A=U.rows===3?[...U.data64F]:null;if(z.delete(),B.delete(),X.delete(),U.delete(),A===null||H<Bw){I();continue}const G=1/i,R=[[G*A[0],G*A[1],G*A[2]],[G*A[3],G*A[4],G*A[5]],[A[6],A[7],A[8]]],P=[[0,0],[y.width,0],[y.width,y.height],[0,y.height]].map(([se,re])=>ja(R,se,re));if(!Ha(P,t.width,t.height)){I();continue}const Y=Ye(e,t),O=e.matFromArray(3,3,e.CV_64F,R.flat()),Z=new e.Mat;e.warpPerspective(Y,Z,O,new e.Size(y.width,y.height),e.WARP_INVERSE_MAP);const D=new e.Mat;e.cvtColor(Z,D,e.COLOR_RGB2GRAY);const V=new e.Mat;e.matchTemplate(D,$,V,e.TM_CCOEFF_NORMED);const F=V.data32F[0];if(Y.delete(),O.delete(),Z.delete(),D.delete(),V.delete(),F<Ef){I();continue}const L=Ka(e,t,y,R),ne=Ya(L);p.push({id:g,confidence:Math.max(0,F),footprint:P,built:L!==null&&Math.max(L.L,L.R,L.T)>=zr,tuckRegion:Xa(P,ne)}),I()}}finally{o.delete(),l.delete(),h.delete(),c.delete();try{u.delete(),m.delete()}catch{}}return p}function Ya(e){return e!==null&&e.R>=zr?["R"]:[]}function Xa(e,t){if(e.length<4||t.length===0)return null;const n=e.map(y=>[y[0],y[1]]),r=Math.hypot(n[1][0]-n[0][0],n[1][1]-n[0][1]),i=Math.hypot(n[2][0]-n[3][0],n[2][1]-n[3][1]),a=.5*(r+i),s=Va*a;if(!(s>0))return null;const o=n.reduce((y,_)=>y+_[0],0)/n.length,u=n.reduce((y,_)=>y+_[1],0)/n.length,l={T:[0,1],R:[1,2],L:[0,3]},h=[...n];for(const y of["L","R","T"]){if(!t.includes(y))continue;const[_,$]=l[y],x=n[_],T=n[$];let S=-(T[1]-x[1]),I=T[0]-x[0];const E=(x[0]+T[0])/2,M=(x[1]+T[1])/2;S*(E-o)+I*(M-u)<0&&(S=-S,I=-I);const v=Math.hypot(S,I);v<=1e-6||(S=S/v*s,I=I/v*s,h.push([x[0]+S,x[1]+I],[T[0]+S,T[1]+I]))}const c=h.map(y=>y[0]),p=h.map(y=>y[1]),m=Math.round(Math.min(...c)),g=Math.round(Math.min(...p));return{x:m,y:g,width:Math.round(Math.max(...c))-m,height:Math.round(Math.max(...p))-g}}function Uw(e,t,n,r){const i=Ow(e,n,t,r);if(i===null)return null;const s=[[0,0],[n.width,0],[n.width,n.height],[0,n.height]].map(([l,h])=>ja(i.H,l,h));if(!Ha(s,t.width,t.height))return null;const o=Ka(e,t,n,i.H);if(o===null)return null;const u=Ya(o);return{built:Math.max(o.L,o.R,o.T)>=zr,footprint:s,overflow:u,edgeScores:o,inliers:i.inliers}}const Lw=.88;function Mf(e,t,n,r){if(r.length!==4)return null;const i=n.width,a=n.height,s=Math.max(8,Math.trunc(Va*i)),o=i+2*s,u=a+2*s;if(o*u>4e7)return null;const l=s+Math.trunc(i*Lw),h=o-l;if(h<1)return null;const c=Ye(e,t),p=e.matFromArray(4,1,e.CV_32FC2,[0,0,i,0,i,a,0,a]),m=e.matFromArray(4,1,e.CV_32FC2,[r[0][0],r[0][1],r[1][0],r[1][1],r[2][0],r[2][1],r[3][0],r[3][1]]),g=e.getPerspectiveTransform(p,m),y=[...g.data64F],_=[0,1,2].flatMap(M=>[y[M*3],y[M*3+1],y[M*3+2]-s*y[M*3]-s*y[M*3+1]]),$=e.matFromArray(3,3,e.CV_64F,_),x=new e.Mat;e.warpPerspective(c,x,$,new e.Size(o,u),e.WARP_INVERSE_MAP);const T=x.roi(new e.Rect(l,0,h,u)),S=new e.Mat;T.copyTo(S);const I=S.data,E=new Uint8ClampedArray(h*u*3);E.set(I.subarray(0,E.length));for(const M of[c,p,m,g,$,x,T,S])try{M.delete()}catch{}return{width:h,height:u,channels:3,data:E}}function Fw(e,t,n,r){const[i,a,s,o]=r;if(s<8||o<8)return null;const u=Math.trunc(.06*s),l=Math.trunc(.06*o),h=Math.max(0,Math.trunc(i-u)),c=Math.min(n.width,Math.trunc(i+s+u)),p=Math.max(0,Math.trunc(a-l)),m=Math.min(n.height,Math.trunc(a+o+l));if(c-h<8||m-p<8)return null;const g=Math.max(n.width,n.height)<Wa?qa:1,y=Ye(e,n),_=Ye(e,t),$=y.roi(new e.Rect(h,p,c-h,m-p)),x=new e.Mat;g!==1?e.resize($,x,new e.Size(0,0),g,g,e.INTER_CUBIC):$.copyTo(x);const T=new e.Mat,S=new e.Mat;e.cvtColor(_,T,e.COLOR_RGB2GRAY),e.cvtColor(x,S,e.COLOR_RGB2GRAY);const I=new e.ORB(vf),E=new e.KeyPointVector,M=new e.KeyPointVector,v=new e.Mat,z=new e.Mat,B=new e.Mat,X=[y,_,$,x,T,S,E,M,v,z,B],U=se=>{for(const re of X)try{re.delete()}catch{}try{I.delete()}catch{}return se};if(I.detectAndCompute(T,B,E,v),I.detectAndCompute(S,B,M,z),v.rows<8||z.rows<8)return U(null);const H=new e.BFMatcher(e.NORM_HAMMING),A=new e.DMatchVectorVector;H.knnMatch(v,z,A,2);const G=[],R=[];for(let se=0;se<A.size();se++){const re=A.get(se);if(re.size()===2){const fe=re.get(0),Ae=re.get(1);if(fe.distance<Ga*Ae.distance){const Ge=E.get(fe.queryIdx).pt,Be=M.get(fe.trainIdx).pt;G.push(Ge.x,Ge.y),R.push(Be.x,Be.y)}}}if(A.delete(),H.delete(),G.length/2<8)return U(null);const P=e.matFromArray(G.length/2,1,e.CV_32FC2,G),Y=e.matFromArray(R.length/2,1,e.CV_32FC2,R),O=new e.Mat,Z=e.findHomography(P,Y,e.RANSAC,5,O);let D=0;for(let se=0;se<O.rows;se++)D+=O.data[se];const V=Z.rows===3?[...Z.data64F]:null;if(P.delete(),Y.delete(),O.delete(),Z.delete(),V===null||D<Sf)return U(null);const F=1/g,L=[[F,0,h],[0,F,p],[0,0,1]],ne=[0,1,2].map(se=>[0,1,2].map(re=>L[se][0]*V[re]+L[se][1]*V[3+re]+L[se][2]*V[6+re]));return U({H:ne,inliers:D})}const Gw=620;function Ww(e,t){return{width:t.cols,height:t.rows,channels:3,data:new Uint8Array(t.data.slice(0,t.rows*t.cols*3))}}function Cf(e,t,n,r){const i=Af(e,t,n,r);if(i!==null)return i;try{const[a,s,o,u]=r.map(I=>Math.trunc(I));if(Math.min(o,u)>=Gw||o<=0||u<=0)return null;const l=Math.trunc(o*.25),h=Math.trunc(u*.25),c=Math.max(0,a-l),p=Math.max(0,s-h),m=Math.min(t.width,a+o+l),g=Math.min(t.height,s+u+h);if(m<=c||g<=p)return null;const y=Ye(e,t),_=y.roi(new e.Rect(c,p,m-c,g-p)),$=new e.Mat;e.resize(_,$,new e.Size((m-c)*2,(g-p)*2),0,0,e.INTER_CUBIC);const x=Ww(e,$);for(const I of[y,_,$])try{I.delete()}catch{}const T=[(a-c)*2,(s-p)*2,o*2,u*2],S=Af(e,x,n,T);return S===null?null:{...S,footprint:S.footprint.map(([I,E])=>[I*.5+c,E*.5+p])}}catch{return null}}function Af(e,t,n,r){const i=Fw(e,n,t,r);if(i===null)return null;const s=[[0,0],[n.width,0],[n.width,n.height],[0,n.height]].map(([$,x])=>ja(i.H,$,x));if(!Ha(s,t.width,t.height))return null;const o=Ye(e,t),u=e.matFromArray(3,3,e.CV_64F,i.H.flat()),l=new e.Mat;e.warpPerspective(o,l,u,new e.Size(n.width,n.height),e.WARP_INVERSE_MAP);const h=Ye(e,n),c=new e.Mat,p=new e.Mat;e.cvtColor(l,c,e.COLOR_RGB2GRAY),e.cvtColor(h,p,e.COLOR_RGB2GRAY);const m=new e.Mat;e.matchTemplate(c,p,m,e.TM_CCOEFF_NORMED);const g=m.data32F[0];for(const $ of[o,u,l,h,c,p,m])try{$.delete()}catch{}if(g<Ef)return null;const y=Ka(e,t,n,i.H);if(y===null)return null;const _=Ya(y);return{built:Math.max(y.L,y.R,y.T)>=zr,footprint:s,overflow:_,edgeScores:y,inliers:i.inliers}}function qw(e,t,n,r=.03){let i=null,a=1/0;for(const s of e){const[o,u,l,h]=s;if(l<=0||h<=0)continue;const c=r*l,p=r*h;if(t>=o-c&&t<=o+l+c&&n>=u-p&&n<=u+h+p){const m=l*h;m<a&&(a=m,i=[o,u,l,h])}}return i}const Vw=.3,Hw=.3;function jw(e,t){const n=e.filter(a=>a.edgeScores!==null);if(n.length===0)return[];const r=n.length>=2&&n.every(a=>{const{L:s,R:o,T:u}=a.edgeScores;return Math.min(s,o,u)>=Vw}),i=[];return e.forEach((a,s)=>{if(!a.built||a.edgeScores===null)return;const{L:o,R:u,T:l}=a.edgeScores,h=Math.max(o,u,l)<Hw;if(!r&&!h)return;t.some(([p,m])=>p>=a.zone.x0&&p<=a.zone.x1&&m>=a.zone.y0&&m<=a.zone.y1)||i.push(s)}),i}const Et=128,Za=.5;function Qa(e){const t=Kt(e,Et,Et),n=Et*Et,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let a=0;a<3;a++)r[a*n+i]=t[i*3+a]/255;return r}function Rf(e){const t=e[1]??0;return{built:t>=Za,prob:t}}const tr=120,nr=179,Kw=1.3,Yw=3.6,Xw=.45,Zw=6e-4,Qw=.02,Jw=6e3,e_=.78,t_=1.25,n_=2.4,r_=.05,i_=1.5,a_=.5,s_=.9,o_=150,u_=18,l_=34,d_=90,c_=130,h_=.13,p_=.15,Or="magistrates-guild",Ja="merchants-guild";function f_(e,t){const n=Ye(e,t),r=new e.Mat;e.cvtColor(n,r,e.COLOR_RGB2HSV),n.delete();const i=new e.Mat(r.rows,r.cols,r.type(),[tr,30,40,0]),a=new e.Mat(r.rows,r.cols,r.type(),[nr,255,205,255]),s=new e.Mat;e.inRange(r,i,a,s),r.delete(),i.delete(),a.delete();const o=new Uint8Array(s.data),u=e.getStructuringElement(e.MORPH_RECT,new e.Size(31,31)),l=new e.Mat;e.morphologyEx(s,l,e.MORPH_CLOSE,u),s.delete(),u.delete();const h=new e.Mat,c=new e.Mat,p=new e.Mat,m=e.connectedComponentsWithStats(l,h,c,p,8);l.delete(),h.delete(),p.delete();const g=t.width*t.height,y=[];for(let _=1;_<m;_++){const $=c.intAt(_,0),x=c.intAt(_,1),T=c.intAt(_,2),S=c.intAt(_,3),I=c.intAt(_,4),E=I/g;E<Zw||E>Qw||I/Math.max(T*S,1)<Xw||y.push({x:$,y:x,w:T,h:S})}return c.delete(),{blobs:y,mask:o,maskWidth:t.width}}function m_(e,t,n,r,i,a,s){const o=e,u=a,l=s,h=i;if(!h.gray){const F=Ye(e,r);h.gray=new o.Mat,o.cvtColor(F,h.gray,o.COLOR_RGB2GRAY),F.delete(),h.k=new o.KeyPointVector,h.d=new o.Mat;const L=new o.Mat;u.detectAndCompute(h.gray,L,h.k,h.d),L.delete()}const c=n,p=new o.Mat,m=new o.KeyPointVector,g=new o.Mat;u.detectAndCompute(c,p,m,g),p.delete();const y=F=>(m.delete(),g.delete(),F);if(h.d.rows<8||g.rows<8)return y(null);const _=new o.DMatchVectorVector;l.knnMatch(h.d,g,_,2);const $=[],x=[];for(let F=0;F<_.size();F++){const L=_.get(F);if(L.size()===2){const ne=L.get(0);if(ne.distance<e_*L.get(1).distance){const se=h.k.get(ne.queryIdx).pt,re=m.get(ne.trainIdx).pt;$.push(se.x,se.y),x.push(re.x,re.y)}}}if(_.delete(),$.length/2<8)return y(null);const T=o.matFromArray($.length/2,1,o.CV_32FC2,$),S=o.matFromArray(x.length/2,1,o.CV_32FC2,x),I=new o.Mat,E=o.findHomography(T,S,o.RANSAC,5,I);if(T.delete(),S.delete(),I.delete(),E.rows!==3)return E.delete(),y(null);const M=[...E.data64F],v=(F,L)=>{const ne=M[6]*F+M[7]*L+M[8];return[(M[0]*F+M[1]*L+M[2])/ne,(M[3]*F+M[4]*L+M[5])/ne]},z=[[0,0],[r.width,0],[r.width,r.height],[0,r.height]].map(([F,L])=>v(F,L));if(z.some(F=>!Number.isFinite(F[0])||!Number.isFinite(F[1])))return E.delete(),y(null);const B=z.map((F,L)=>{const ne=z[(L+1)%4];return Math.hypot(ne[0]-F[0],ne[1]-F[1])}),X=Math.min(...B);if(X<1)return E.delete(),y(null);const U=Math.max(...B)/X;let H=0;for(let F=0;F<4;F++){const[L,ne]=z[F],[se,re]=z[(F+1)%4];H+=L*re-se*ne}const A=t,G=Math.abs(H/2)/(A.rows*A.cols);if(U<t_||U>n_||G<r_||G>i_)return E.delete(),y(null);const R=new o.Mat;o.warpPerspective(A,R,E,new o.Size(r.width,r.height),o.WARP_INVERSE_MAP),E.delete();const P=new o.Mat;o.cvtColor(R,P,o.COLOR_RGB2GRAY),R.delete();const Y=Math.trunc(r.height/2),O=P.roi(new o.Rect(0,0,r.width,Y)),Z=h.gray.roi(new o.Rect(0,0,r.width,Y)),D=new o.Mat;o.matchTemplate(O,Z,D,o.TM_CCOEFF_NORMED);const V=D.data32F[0];return O.delete(),Z.delete(),D.delete(),P.delete(),y(V)}function g_(e,t,n){let r,i;if(n===Or)r=Ja,i=h_;else if(n===Ja)r=Or,i=p_;else return null;const{x:a,y:s,w:o,h:u}=t;if(o<8||u<8)return null;const l=Math.trunc(o/2);let h=0,c=null;for(const[p,m]of[[0,l],[l,o]]){let g=0,y=0;for(let $=s;$<s+u;$++)for(let x=a+p;x<a+m;x++){const T=($*e.width+x)*e.channels,{h:S,s:I,v:E}=$t(e.data[T],e.data[T+1],e.data[T+2]);if(S>=tr&&S<=nr&&I>=30&&I<=170&&E<=170)continue;g++,(r===Ja?S>=u_&&S<=l_&&I>=d_&&E>=c_:S>=95&&S<=130&&I>=80)&&y++}if(g<20)continue;const _=y/g;_>h&&(h=_,c={x:a+p,y:s,w:m-p,h:u})}return h>=i&&c!==null?{id:r,box:c}:null}const y_=1.7,w_=140,__=170,b_=.2,$_=.1,zf=240,Of=80,Nf=60,x_=50,Bf="scientists-guild",Pf="tacticians-guild",Nr=["shipowners-guild","merchants-guild","builders-guild","moneylenders-guild"];function v_(e,t,n){const{x:r,y:i,w:a,h:s}=n,o=new Float32Array(s);for(let S=0;S<s;S++){let I=0;for(let E=0;E<a;E++)e[(i+S)*t+r+E]>0&&I++;o[S]=I/a}const u=[];for(let S=0;S<s;S++)o[S]>.3&&u.push(S);if(u.length<5)return[];const l=u[0],h=u[u.length-1],c=h-l;if(c<5)return[];const p=a/c;if(p<Kw||p>Yw)return[];if(p>=y_)return[{x:r,y:i+l,w:a,h:c}];const m=new Float32Array(s),g=.3*(8*.5-1)+.8,y=[];let _=0;for(let S=-4;S<=4;S++){const I=Math.exp(-(S*S)/(2*g*g));y.push(I),_+=I}for(let S=0;S<s;S++){let I=0;for(let E=-4;E<=4;E++){const M=Math.min(s-1,Math.max(0,S+E));I+=o[M]*y[E+4]}m[S]=I/_}const $=l+Math.trunc(c*.3),x=l+Math.trunc(c*.78);let T=l+Math.trunc(c/2);if(x>$){let S=1/0;for(let I=$;I<x;I++)m[I]<S&&(S=m[I],T=I)}return[{x:r,y:i+l,w:a,h:T-l},{x:r,y:i+T,w:a,h:h-T}]}function S_(e,t){const n=Math.max(0,t.x),r=Math.max(0,t.y),i=Math.min(e.width,t.x+t.w),a=Math.min(e.height,t.y+t.h),s=Math.max(0,i-n),o=Math.max(0,a-r),u=new Uint8Array(s*o*3);for(let l=0;l<o;l++)for(let h=0;h<s;h++){const c=((r+l)*e.width+n+h)*e.channels,p=(l*s+h)*3;u[p]=e.data[c],u[p+1]=e.data[c+1],u[p+2]=e.data[c+2]}return{width:s,height:o,channels:3,data:u}}function T_(e){let t=0,n=0;for(let r=0,i=e.width*e.height;r<i;r++){const a=r*e.channels,{h:s,s:o,v:u}=$t(e.data[a],e.data[a+1],e.data[a+2]);o>=40&&u>=40&&u<=205&&(t++,s>=w_&&s<=__&&n++)}return t===0?0:n/t}function I_(e){let t=0;const n=e.width*e.height;for(let r=0;r<n;r++){const i=r*e.channels,{h:a,s,v:o}=$t(e.data[i],e.data[i+1],e.data[i+2]);!(a>=tr&&a<=nr)&&s>=70&&o>=50&&t++}return n===0?0:t/n}function Df(e,t){const n=Ye(e,t),r=new e.Mat;e.resize(n,r,new e.Size(zf,Of),0,0,e.INTER_AREA),n.delete();const i=new Uint8Array(r.data);return r.delete(),{width:zf,height:Of,channels:3,data:i}}function k_(e){const t=e.width*e.height,n=[0,0,0];for(let a=0;a<t;a++){const s=a*e.channels;n[0]+=e.data[s],n[1]+=e.data[s+1],n[2]+=e.data[s+2]}n[0]/=t,n[1]/=t,n[2]/=t;const r=(n[0]+n[1]+n[2])/3,i=new Uint8Array(t*3);for(let a=0;a<t;a++){const s=a*e.channels;for(let o=0;o<3;o++){const u=n[o]>1e-6?r/n[o]:1;i[a*3+o]=Math.max(0,Math.min(255,Math.round(e.data[s+o]*u)))}}return{width:e.width,height:e.height,channels:3,data:i}}function Uf(e,t){const n=k_(t),r=n.width*n.height,i=new Uint8Array(r);let a=0;for(let g=0;g<r;g++){const y=g*3,{h:_,s:$,v:x}=$t(n.data[y],n.data[y+1],n.data[y+2]);!(_>=tr&&_<=nr&&$>=30&&$<=170&&x<=170)&&x>=40&&(i[g]=1,a++)}const s=a<20,o=Ye(e,n),u=new e.Mat;e.cvtColor(o,u,e.COLOR_RGB2Lab),o.delete();const l=u.data;let h=0,c=0,p=0,m=0;for(let g=0;g<r;g++)!s&&i[g]===0||(h+=l[g*3]*100/255,c+=l[g*3+1]-128,p+=l[g*3+2]-128,m++);return u.delete(),m===0?[0,0,0]:[h/m,c/m,p/m]}function E_(e){let t=0,n=0,r=0,i=0,a=0;const s=e.width*e.height;for(let u=0;u<s;u++){const l=u*e.channels,{h,s:c,v:p}=$t(e.data[l],e.data[l+1],e.data[l+2]);h>=tr&&h<=nr&&c>=30&&c<=170&&p<=170||(t++,c>=70&&p>=50&&(h>=95&&h<=130?n++:h>=35&&h<=92?r++:h<=10?i++:h>=15&&h<=34&&p>=80&&a++))}const o=Math.max(t,1);return{blue:n/o,green:r/o,red:i/o,gold:a/o}}function M_(e){const t=e.width*e.height,n={blue:0,green:0,red:0,gold:0,brown:0,grey:0};for(let r=0;r<t;r++){const i=r*e.channels,{h:a,s,v:o}=$t(e.data[i],e.data[i+1],e.data[i+2]);s>=Nf&&o>=x_?(a>=95&&a<=128&&n.blue++,a>=35&&a<=85&&n.green++,(a<=8||a>=170)&&n.red++,a>=18&&a<=34&&n.gold++,a>=4&&a<=17&&o<150&&n.brown++):s<Nf&&o>=70&&o<=235&&n.grey++}for(const r of Object.keys(n))n[r]/=t;return n}function C_(e,t){let n=0,r=0;for(let o=0;o<e.length;o++)n+=e[o],r+=t[o];n/=e.length,r/=t.length;let i=0,a=0,s=0;for(let o=0;o<e.length;o++){const u=e[o]-n,l=t[o]-r;i+=u*l,a+=u*u,s+=l*l}return i/(Math.sqrt(a*s)+1e-6)}function Lf(e,t){const n=Ye(e,t),r=new e.Mat;e.cvtColor(n,r,e.COLOR_RGB2GRAY),n.delete();const i=Float32Array.from(r.data);return r.delete(),i}function A_(e,t){const n=new Map,r=new Map;for(const[i,a]of t){const s=Df(e,a);n.set(i,Lf(e,s)),Nr.includes(i)&&r.set(i,Uf(e,s))}return{gray:n,warmLab:r}}function R_(e,t,n){const r=Df(e,t),i=E_(r);if(i.blue>=.15&&i.blue>i.red&&i.blue>2*i.gold)return Or;if(i.green>=.08&&i.green>i.blue&&i.green>i.gold)return Bf;if(i.red>=.15&&i.red>i.blue&&i.red>1.5*i.gold)return Pf;const a=M_(r),s={blue:a.blue,green:a.green,red:a.red,gold:a.gold,browngrey:a.brown+a.grey};let o="blue";for(const l of Object.keys(s))s[l]>s[o]&&(o=l);if(s[o]<=0)return"";let u;if(o==="blue")u=Or;else if(o==="green")u=Bf;else if(o==="red")u=Pf;else{const l=Lf(e,r);let h="",c=-2;for(const p of Nr){const m=n.gray.get(p);if(m===void 0)continue;const g=C_(l,m);g>c&&(c=g,h=p)}u=h||Nr[0]}if(Nr.includes(u)&&n.warmLab.size>0){const l=Uf(e,r);let h=u,c=1/0;for(const[p,m]of n.warmLab){const g=Math.hypot(l[0]-m[0],l[1]-m[1],l[2]-m[2]);g<c&&(c=g,h=p)}return h}return u}function z_(e,t,n,r,i){var y;const a=[],{blobs:s,mask:o,maskWidth:u}=f_(e,t);if(s.length===0||n.size===0)return a;const l=e,h=new l.ORB(Jw),c=new l.BFMatcher(l.NORM_HAMMING),p=new Map;for(const _ of n.keys())p.set(_,{});const m=Ye(e,t);let g=null;try{for(const _ of s){if(r!==void 0&&Date.now()>r)break;const $=_.x+Math.trunc(_.w/2),x=_.y+Math.trunc(_.h/2),T=Math.max(o_,Math.trunc(s_*Math.max(_.w,_.h))),S=Math.max(0,$-T),I=Math.max(0,x-T),E=Math.min(t.width,$+T),M=Math.min(t.height,x+T);if(E-S<16||M-I<16)continue;const v=m.roi(new l.Rect(S,I,E-S,M-I)),z=new l.Mat;l.cvtColor(v,z,l.COLOR_RGB2GRAY);let B=null,X=-2;for(const[G,R]of n){if(r!==void 0&&Date.now()>r)break;const P=m_(e,v,z,R,p.get(G),h,c);P!==null&&P>X&&(X=P,B=G)}v.delete(),z.delete();const U=new Set;if(B!==null&&X>=a_){a.push({id:B,boundingBox:{x:_.x,y:_.y,width:_.w,height:_.h},confidence:1}),U.add(B);const G=g_(t,_,B);G&&(a.push({id:G.id,boundingBox:{x:G.box.x,y:G.box.y,width:G.box.w,height:G.box.h},confidence:.9}),U.add(G.id))}if(i===void 0||i.size===0)continue;const H=v_(o,u,_);if(H.length!==2)continue;const A=H.map(G=>S_(t,G));if(!A.some(G=>G.width*G.height===0||I_(G)<$_))for(let G=0;G<H.length;G++){const R=A[G];if(T_(R)<b_)continue;g===null&&(g=A_(e,i));const P=R_(e,R,g);if(P&&!U.has(P)){U.add(P);const Y=H[G];a.push({id:P,boundingBox:{x:Y.x,y:Y.y,width:Y.w,height:Y.h},confidence:1})}}}}finally{m.delete();for(const _ of p.values()){const $=_;for(const x of["gray","k","d"])try{(y=$[x])==null||y.delete()}catch{}}try{h.delete(),c.delete()}catch{}}return a}const Ff=128,O_=.56,N_=15,B_=.58,P_=70,D_=50,U_=.12,L_=.2,F_=.1,G_=.17,Gf=.15;function W_(e){const t=new Map;for(const[n,r]of Object.entries(e.templates)){const i=Uint8Array.from(atob(r),a=>a.charCodeAt(0));i.length===e.size*e.size&&t.set(n,i)}return t}function Wf(e,t){const{width:n,height:r,channels:i,data:a}=e,s=Math.floor(n/2),o=Math.floor(r/2),u=Math.trunc(Math.min(n,r)*.5*t);if(u<1)return e;const l=Math.max(0,s-u),h=Math.max(0,o-u),c=Math.min(n,s+u),p=Math.min(r,o+u),m=c-l,g=p-h,y=new Uint8Array(m*g*i);for(let _=0;_<g;_++){const $=((_+h)*n+l)*i;y.set(a.subarray($,$+m*i),_*m*i)}return{width:m,height:g,channels:i,data:y}}function q_(e){const t=Wf(e,O_),n=Ny(t),r=_f(n,Ff,Ff);return By(r)}function V_(e,t){const n=e.length;let r=0,i=0;for(let u=0;u<n;u++)r+=e[u],i+=t[u];r/=n,i/=n;let a=0,s=0,o=0;for(let u=0;u<n;u++){const l=e[u]-r,h=t[u]-i;a+=l*h,s+=l*l,o+=h*h}return a/(Math.sqrt(s*o)+1e-6)}function H_(e){const t=new Map([["masonry",0],["strategy",0]]),n=Wf(e,B_),{width:r,height:i,channels:a,data:s}=n,o=r*i||1;let u=0,l=0;for(let p=0;p<r*i;p++){const m=p*a,{h:g,s:y,v:_}=$t(s[m],s[m+1],s[m+2]);y>=P_&&_>=D_&&(g>=95&&g<=130&&(u+=1),(g<=8||g>=170)&&(l+=1))}const h=u/o,c=l/o;return h>=U_&&t.set("masonry",Gf*Math.min(1,h/L_)),c>=F_&&t.set("strategy",Gf*Math.min(1,c/G_)),t}function j_(e,t){if(t.size===0||e.width===0||e.height===0)return["",0];const n=q_(e);let r=0;for(const l of n.data)r+=l;const i=r/n.data.length,a=[];for(let l=0;l<360;l+=N_)a.push(Uy(n,l,i));const s=new Map;for(const[l,h]of t){let c=-1/0;for(const p of a){const m=V_(p,h);m>c&&(c=m)}s.set(l,c)}for(const[l,h]of H_(e))h>0&&s.has(l)&&s.set(l,s.get(l)+h);let o="",u=-1/0;for(const[l,h]of s)h>u&&(o=l,u=h);return[o,u]}const Xt=224,K_=512,Y_=[.485,.456,.406],X_=[.229,.224,.225];function Z_(e){const t=atob(e.x),n=new Uint8Array(t.length);for(let i=0;i<t.length;i++)n[i]=t.charCodeAt(i);const r=new Float32Array(n.buffer);if(r.length!==e.ids.length*e.dim)throw new Error(`token_embed_index: ${r.length} floats != ${e.ids.length}x${e.dim}`);return{dim:e.dim,ids:e.ids,x:r}}function Q_(e){const t=Kt(e,Xt,Xt),n=Xt*Xt,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let a=0;a<3;a++)r[a*n+i]=(t[i*3+a]/255-Y_[a])/X_[a];return r}function J_(e){const t=3*Xt*Xt,n=new Float32Array(4*t);for(let r=0;r<4;r++)n.set(Q_(Yt(e,r)),r*t);return n}function eb(e,t=K_){const n=e.length/t,r=new Float32Array(t);for(let a=0;a<n;a++)for(let s=0;s<t;s++)r[s]+=e[a*t+s];let i=0;for(let a=0;a<t;a++)r[a]/=n,i+=r[a]*r[a];i=Math.max(Math.sqrt(i),1e-9);for(let a=0;a<t;a++)r[a]/=i;return r}function tb(e,t){let n=0,r=-2;for(let i=0;i<e.ids.length;i++){let a=0;const s=i*e.dim;for(let o=0;o<e.dim;o++)a+=e.x[s+o]*t[o];a>r&&(r=a,n=i)}return{id:e.ids[n],cosine:r}}const Cn=96,nb=["builders-guild","magistrates-guild","merchants-guild","moneylenders-guild","scientists-guild","shipowners-guild","tacticians-guild"],rb=.45;function ib(e){const t=Kt(e,Cn,Cn),n=Cn*Cn,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let a=0;a<3;a++)r[a*n+i]=t[i*3+a]/255;return r}function ab(e){let t=0;for(let r=1;r<e.length;r++)e[r]>e[t]&&(t=r);const n=e[t];return{id:n>=rb?nb[t]??"":"",prob:n}}const An=128,sb=["circus-maximus","piraeus","the-appian-way","the-colossus","the-great-library","the-great-lighthouse","the-hanging-gardens","the-mausoleum","the-pyramids","the-sphinx","the-statue-of-zeus","the-temple-of-artemis"],ob=.5,ub=.9;function lb(e){const t=Kt(e,An,An),n=An*An,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let a=0;a<3;a++)r[a*n+i]=t[i*3+a]/255;return r}function db(e){const{width:t,height:n,channels:r,data:i}=e,a=new Uint8ClampedArray(t*n*r);for(let s=0;s<t;s++)for(let o=0;o<n;o++){const u=s,h=((n-1-o)*t+u)*r,c=(s*n+o)*r;for(let p=0;p<r;p++)a[c+p]=i[h+p]}return{width:n,height:t,channels:r,data:a}}function cb(e,t){let n=e;const r=(t%4+4)%4;for(let i=0;i<r;i++)n=db(n);return n}function hb(e){let t=0;for(let n=1;n<e.length;n++)e[n]>e[t]&&(t=n);return{index:t,prob:e[t]}}async function pb(e,t){let n=0,r=-1;for(let i=0;i<4;i++){const a=i===0?e:cb(e,i),s=await t(lb(a)),o=hb(s);o.prob>r&&(r=o.prob,n=o.index)}return{id:r>=ob?sb[n]??"":"",prob:r}}const Rn=96,fb=[1,2,3,4,5,6,7],mb=.8;function gb(e){const t=sy(e,e.width*2,e.height*2),n=Kt({width:e.width*2,height:e.height*2,channels:3,data:t},Rn,Rn),r=Rn*Rn,i=new Float32Array(3*r);for(let a=0;a<r;a++)for(let s=0;s<3;s++)i[s*r+a]=n[a*3+s]/255;return i}function yb(e){let t=0;for(let n=1;n<e.length;n++)e[n]>e[t]&&(t=n);return{value:fb[t],prob:e[t]}}const zn=128,qf=.35,wb=["fp","laurel"],_b=.85;function bb(e){const t=Kt(e,zn,zn),n=zn*zn,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let a=0;a<3;a++)r[a*n+i]=t[i*3+a]/255;return r}function $b(e){return e[wb.indexOf("fp")]}const Zt=64,Vf=.5,xb=[.67,1.24];function vb(e,t,n,r){const i=Math.max(0,t-r),a=Math.max(0,n-r),s=Math.min(e.width,t+r),o=Math.min(e.height,n+r),u=s-i,l=o-a;if(u<=0||l<=0)return null;const h=e.channels,c=new Uint8ClampedArray(u*l*3),p=r*r;for(let _=0;_<l;_++){const $=a+_,x=$-n;for(let T=0;T<u;T++){const S=i+T,I=S-t,E=(_*u+T)*3;if(I*I+x*x<=p){const M=($*e.width+S)*h;c[E]=e.data[M],c[E+1]=e.data[M+1],c[E+2]=e.data[M+2]}else c[E]=255,c[E+1]=255,c[E+2]=255}}const m=Kt({width:u,height:l,channels:3,data:c},Zt,Zt),g=Zt*Zt,y=new Float32Array(3*g);for(let _=0;_<g;_++)for(let $=0;$<3;$++)y[$*g+_]=m[_*3+$]/255;return y}function Sb(e){return e[1]}const Tb=2.25,Ib=3,kb=1.15,Eb=.5,Mb=2.5,Cb=.75,Ab=2.25,Rb=1.3,zb=.77;function Br(e,t){const n=Math.max(0,Math.max(e[0],t[0])-Math.min(e[0]+e[2],t[0]+t[2])),r=Math.max(0,Math.max(e[1],t[1])-Math.min(e[1]+e[3],t[1]+t[3]));return Math.hypot(n,r)}function Ob(e){const t=Array.from(new Map(e.map(a=>[`${a[0]},${a[1]}`,a])).values());if(t.sort((a,s)=>a[0]-s[0]||a[1]-s[1]),t.length<=2)return t;const n=(a,s,o)=>(s[0]-a[0])*(o[1]-a[1])-(s[1]-a[1])*(o[0]-a[0]),r=[];for(const a of t){for(;r.length>=2&&n(r[r.length-2],r[r.length-1],a)<=0;)r.pop();r.push(a)}const i=[];for(const a of[...t].reverse()){for(;i.length>=2&&n(i[i.length-2],i[i.length-1],a)<=0;)i.pop();i.push(a)}return[...r.slice(0,-1),...i.slice(0,-1)]}function Hf(e,t,n){let r=!1;const i=n.length;for(let a=0;a<i;a+=1){const[s,o]=n[a],[u,l]=n[(a+1)%i];if(o>t!=l>t){const h=(u-s)*(t-o)/(l-o)+s;e<h&&(r=!r)}}return r}function Nb(e,t,n){if(n.length>=3&&Hf(e,t,n))return 0;let r=Number.POSITIVE_INFINITY;const i=n.length;for(let a=0;a<i;a+=1){const[s,o]=n[a],[u,l]=n[i>1?(a+1)%i:a],h=u-s,c=l-o,p=h*h+c*c,m=p===0?0:Math.max(0,Math.min(1,((e-s)*h+(t-o)*c)/p));r=Math.min(r,Math.hypot(e-(s+m*h),t-(o+m*c)))}return r}function Bb(e,t,n){const r=Math.max(Math.abs(e-(n[0]+n[2]/2))-n[2]/2,0),i=Math.max(Math.abs(t-(n[1]+n[3]/2))-n[3]/2,0);return Math.hypot(r,i)}function Pb(e,t,n){const[r,i]=e,a=t[0]-r,s=t[1]-i;if(a===0&&s===0)return!1;const[o,u,l,h]=n;let c=0,p=1;const m=[[-a,r-o],[a,l-r],[-s,i-u],[s,h-i]];for(const[g,y]of m){if(g===0){if(y<0)return!1;continue}const _=y/g;if(g<0?c=Math.max(c,_):p=Math.min(p,_),c>p)return!1}return c>=p?!1:c>=.1&&p<=.95||p-c>=.15}const es=e=>e.box[3]/Math.max(1,e.box[2]),Dt=e=>es(e)>kb,On=e=>es(e)>=Rb||es(e)<=zb;function ts(e){const[t,n,r,i]=e.box;if(r>=i){const s=7*i;return[t,n-s,r,i+2*s]}const a=7*r;return[t-a,n,r+2*a,i]}function Db(e,t,n,r,i){const a=new Set(t),s=[...e.map((R,P)=>({box:[R[0],R[1],R[2],R[3]],kind:a.has(P)?"card":"tucked",src:["banner",P]})),...n.map((R,P)=>({box:[R[0],R[1],R[2],R[3]],kind:"wonder",src:["wonder",P]}))],o=e.map(()=>"player"),u=n.map(()=>"player");if(s.length===0)return{bannerOwner:o,wonderOwner:u,opponentFound:!1,hulls:[],pointOwner:()=>"player"};const l=s.map(R=>[R.box[0]+R.box[2]/2,R.box[1]+R.box[3]/2]);let h=s.filter(R=>R.kind!=="wonder").map(R=>Math.hypot(R.box[2],R.box[3])).sort((R,P)=>R-P);h.length===0&&(h=s.map(R=>Math.hypot(R.box[2],R.box[3])).sort((R,P)=>R-P));const c=h[Math.floor(h.length/2)],p=(Tb*c)**2,m=s.map((R,P)=>P),g=R=>{let P=R;for(;m[P]!==P;)m[P]=m[m[P]],P=m[P];return P},y=s.map((R,P)=>R.kind==="card"?P:-1).filter(R=>R>=0),_=s.map((R,P)=>R.kind!=="card"?P:-1).filter(R=>R>=0);for(let R=0;R<y.length;R+=1)for(let P=R+1;P<y.length;P+=1){const Y=y[R],O=y[P],Z=s[Y],D=s[O];if(On(Z)&&On(D)&&Dt(Z)!==Dt(D))continue;const V=l[Y][0]-l[O][0],F=l[Y][1]-l[O][1],L=V*V+F*F;let ne=L<=p;!ne&&On(Z)&&On(D)&&Dt(Z)===Dt(D)&&L<=(4*c)**2&&(ne=Br(ts(Z),ts(D))<=.5*c),ne&&(m[g(Y)]=g(O))}for(let R=0;R<_.length;R+=1)for(let P=R+1;P<_.length;P+=1){const Y=_[R],O=_[P];Br(s[Y].box,s[O].box)<=Cb*c&&(m[g(Y)]=g(O))}const $=new Map;for(const R of _){const P=g(R);$.set(P,[...$.get(P)??[],R])}const x=new Map;for(const R of y){const P=g(R);x.set(P,[...x.get(P)??[],R])}for(const R of $.values()){const P=R.filter(D=>s[D].kind==="wonder"&&On(s[D])).map(D=>Dt(s[D])),Y=P.length>0?P.filter(Boolean).length*2>P.length:null,O=[];for(const[D,V]of x){let F=Number.POSITIVE_INFINITY;for(const se of R)for(const re of V)F=Math.min(F,Br(s[se].box,s[re].box));if(F>Ab*c)continue;const ne=V.filter(se=>Dt(s[se])).length/V.length>=.5;Y!==null&&ne!==Y||O.push([D,F,ne])}if(O.length===0)continue;const Z=new Set(O.map(D=>D[2]));if(O.length>=2&&Z.size===1&&Y!==null){const D=O[0][0];for(const[V]of O.slice(1))m[g(V)]=g(D);m[g(R[0])]=g(D)}else{const D=O.reduce((V,F)=>F[1]<V[1]?F:V);m[g(R[0])]=g(D[0])}}let T=new Map;for(let R=0;R<s.length;R+=1){const P=g(R);T.set(P,[...T.get(P)??[],R])}const S=s.map((R,P)=>R.kind==="wonder"?P:-1).filter(R=>R>=0);if(S.length>0){const R=(Y,O)=>{const[Z,D,V,F]=ts(s[Y]),[L,ne,se,re]=s[O].box,fe=Math.max(0,Math.min(Z+V,L+se)-Math.max(Z,L)),Ae=Math.max(0,Math.min(D+F,ne+re)-Math.max(D,ne));return fe*Ae>=.9*s[Y].box[2]*s[Y].box[3]},P=new Map;for(let Y=0;Y<s.length;Y+=1)if(!(s[Y].kind!=="card"||!On(s[Y])))for(const O of S){const Z=Br(s[Y].box,s[O].box);if(Z<=.8*c&&Dt(s[Y])!==Dt(s[O])&&R(Y,O)){const D=P.get(O);(!D||Z<D[1])&&P.set(O,[Y,Z])}}for(const[Y,[O]]of P){const Z=g(Y);for(const[D,V]of T){const F=V.indexOf(O);if(F>=0&&D!==Z){V.splice(F,1),T.set(Z,[...T.get(Z)??[],O]),s[O].kind="tucked";break}}}T=new Map([...T].filter(([,Y])=>Y.length>0))}const I=R=>R.filter(P=>s[P].kind==="card").length,E=R=>{const P=R.filter(Y=>s[Y].kind==="card"||s[Y].kind==="wonder");return P.length===0?null:P.filter(Y=>Dt(s[Y])).length/P.length},M=R=>[R.reduce((P,Y)=>P+l[Y][0],0)/R.length,R.reduce((P,Y)=>P+l[Y][1],0)/R.length],v=[i[0]/2,i[1]/2],z=[...T.values()].sort((R,P)=>{const Y=I(R),O=I(P);if(Y!==O)return O-Y;const Z=Math.hypot(M(R)[0]-v[0],M(R)[1]-v[1]),D=Math.hypot(M(P)[0]-v[0],M(P)[1]-v[1]);return Z-D}),B=M(z[0]),X=E(z[0]),U=[],H=[];let A=!1;return z.forEach((R,P)=>{let Y;if(P===0||I(R)<Ib)Y="player";else{const D=E(R),V=D!==null&&X!==null&&Math.abs(D-X)>=Eb,F=M(R),L=r.some(ne=>Pb(B,F,ne));Y=V||L?"opponent":"player"}Y==="opponent"&&(A=!0);const O=[],Z=[];for(const D of R){const[V,F,L,ne]=s[D].box;O.push([V,F],[V+L,F],[V,F+ne],[V+L,F+ne]),Z.push(s[D].box);const[se,re]=s[D].src;se==="banner"?o[re]=Y:u[re]=Y}U.push([Y,Ob(O)]),H.push([Y,Z])}),{bannerOwner:o,wonderOwner:u,opponentFound:A,hulls:U,pointOwner:(R,P)=>{if(U.length===0)return"player";const Y=c>0?Mb*c:Number.POSITIVE_INFINITY,O=F=>Math.min(...H[F][1].map(L=>Bb(R,P,L))),Z=U.map(([,F],L)=>F.length>=3&&Hf(R,P,F)?L:-1).filter(F=>F>=0);if(Z.length>0){const F=Z.reduce((L,ne)=>O(ne)<O(L)?ne:L);return U[F][0]}let D=-1,V=Number.POSITIVE_INFINITY;return U.forEach(([,F],L)=>{const ne=Nb(R,P,F);ne<V&&(D=L,V=ne)}),D>=0&&V<=Y?U[D][0]:"none"}}}const Ub=[{id:"merchants-guild",name:"Merchants Guild",nameFr:"Guilde des commerçants",color:"guild",age:3,victoryPoints:0,variableScoring:"merchantsGuild",cost:{clay:1,wood:1,glass:1,papyrus:1}},{id:"shipowners-guild",name:"Shipowners Guild",nameFr:"Guilde des armateurs",color:"guild",age:3,victoryPoints:0,variableScoring:"shipownersGuild",cost:{clay:2,glass:1,papyrus:1}},{id:"builders-guild",name:"Builders Guild",nameFr:"Guilde des bâtisseurs",color:"guild",age:3,victoryPoints:0,variableScoring:"buildersGuild",cost:{stone:2,clay:1,wood:1,glass:1}},{id:"magistrates-guild",name:"Magistrates Guild",nameFr:"Guilde des magistrats",color:"guild",age:3,victoryPoints:0,variableScoring:"magistratesGuild",cost:{wood:2,clay:1,papyrus:1}},{id:"scientists-guild",name:"Scientists Guild",nameFr:"Guilde des scientifiques",color:"guild",age:3,victoryPoints:0,variableScoring:"scientistsGuild",cost:{wood:2,clay:2}},{id:"tacticians-guild",name:"Tacticians Guild",nameFr:"Guilde des tacticiens",color:"guild",age:3,victoryPoints:0,variableScoring:"tacticiansGuild",cost:{stone:2,clay:1,papyrus:1}},{id:"moneylenders-guild",name:"Moneylenders Guild",nameFr:"Guilde des usuriers",color:"guild",age:3,victoryPoints:0,variableScoring:"moneylendersGuild",cost:{stone:2,wood:2}}],Lb=[{id:"lumber-yard",name:"Lumber Yard",nameFr:"Chantier",color:"raw",age:1,victoryPoints:0},{id:"logging-camp",name:"Logging Camp",nameFr:"Exploitation",color:"raw",age:1,victoryPoints:0,coinCost:1},{id:"clay-pool",name:"Clay Pool",nameFr:"Bassin argileux",color:"raw",age:1,victoryPoints:0},{id:"clay-pit",name:"Clay Pit",nameFr:"Cavité",color:"raw",age:1,victoryPoints:0,coinCost:1},{id:"quarry",name:"Quarry",nameFr:"Gisement",color:"raw",age:1,victoryPoints:0},{id:"stone-pit",name:"Stone Pit",nameFr:"Mine",color:"raw",age:1,victoryPoints:0,coinCost:1},{id:"glassworks",name:"Glassworks",nameFr:"Verrerie",color:"manufactured",age:1,victoryPoints:0,coinCost:1},{id:"press",name:"Press",nameFr:"Presse",color:"manufactured",age:1,victoryPoints:0,coinCost:1},{id:"theater",name:"Theater",nameFr:"Théâtre",color:"civilian",age:1,victoryPoints:3},{id:"altar",name:"Altar",nameFr:"Autel",color:"civilian",age:1,victoryPoints:3,providesChain:"moon"},{id:"baths",name:"Baths",nameFr:"Bains",color:"civilian",age:1,victoryPoints:3,providesChain:"drop",cost:{stone:1}},{id:"pharmacist",name:"Pharmacist",nameFr:"Officine",color:"scientific",age:1,victoryPoints:0,scienceSymbol:"mortar",providesChain:"mortar-chain",cost:{glass:2}},{id:"apothecary",name:"Apothecary",nameFr:"Apothicaire",color:"scientific",age:1,victoryPoints:1,scienceSymbol:"wheel",providesChain:"wheel-chain",cost:{glass:1}},{id:"workshop",name:"Workshop",nameFr:"Atelier",color:"scientific",age:1,victoryPoints:1,scienceSymbol:"pendulum",providesChain:"pendulum-chain",cost:{papyrus:1}},{id:"scriptorium",name:"Scriptorium",nameFr:"Scriptorium",color:"scientific",age:1,victoryPoints:0,scienceSymbol:"inkwell",providesChain:"inkwell-chain",coinCost:2},{id:"stone-reserve",name:"Stone Reserve",nameFr:"Dépôt de pierre",color:"commercial",age:1,victoryPoints:0,coinCost:3},{id:"clay-reserve",name:"Clay Reserve",nameFr:"Dépôt d'argile",color:"commercial",age:1,victoryPoints:0,coinCost:3},{id:"wood-reserve",name:"Wood Reserve",nameFr:"Dépôt de bois",color:"commercial",age:1,victoryPoints:0,coinCost:3},{id:"tavern",name:"Tavern",nameFr:"Taverne",color:"commercial",age:1,victoryPoints:0,providesChain:"jug"},{id:"guard-tower",name:"Guard Tower",nameFr:"Tour de garde",color:"military",age:1,victoryPoints:0,shields:1},{id:"stable",name:"Stable",nameFr:"Écuries",color:"military",age:1,victoryPoints:0,shields:1,providesChain:"horseshoe",cost:{wood:1}},{id:"garrison",name:"Garrison",nameFr:"Caserne",color:"military",age:1,victoryPoints:0,shields:1,providesChain:"sword",cost:{clay:1}},{id:"palisade",name:"Palisade",nameFr:"Palissade",color:"military",age:1,victoryPoints:0,shields:1,providesChain:"tower",coinCost:2}],Fb=[{id:"sawmill",name:"Sawmill",nameFr:"Scierie",color:"raw",age:2,victoryPoints:0,coinCost:2},{id:"brickyard",name:"Brickyard",nameFr:"Briqueterie",color:"raw",age:2,victoryPoints:0,coinCost:2},{id:"shelf-quarry",name:"Shelf Quarry",nameFr:"Carrière",color:"raw",age:2,victoryPoints:0,coinCost:2},{id:"glass-blower",name:"Glass-Blower",nameFr:"Soufflerie",color:"manufactured",age:2,victoryPoints:0,coinCost:2},{id:"drying-room",name:"Drying Room",nameFr:"Séchoir",color:"manufactured",age:2,victoryPoints:0,coinCost:2},{id:"courthouse",name:"Courthouse",nameFr:"Tribunal",color:"civilian",age:2,victoryPoints:5,cost:{wood:2,glass:1}},{id:"statue",name:"Statue",nameFr:"Statue",color:"civilian",age:2,victoryPoints:4,providesChain:"column",chainFrom:"moon",cost:{clay:2}},{id:"temple",name:"Temple",nameFr:"Temple",color:"civilian",age:2,victoryPoints:4,providesChain:"sun",chainFrom:"drop",cost:{wood:1,papyrus:1}},{id:"aqueduct",name:"Aqueduct",nameFr:"Aqueduc",color:"civilian",age:2,victoryPoints:5,cost:{stone:3}},{id:"rostrum",name:"Rostrum",nameFr:"Rostres",color:"civilian",age:2,victoryPoints:4,providesChain:"horseshoe",cost:{stone:1,wood:1}},{id:"school",name:"School",nameFr:"École",color:"scientific",age:2,victoryPoints:1,scienceSymbol:"wheel",providesChain:"wheel-chain-2",cost:{wood:1,papyrus:2}},{id:"laboratory",name:"Laboratory",nameFr:"Laboratoire",color:"scientific",age:2,victoryPoints:1,scienceSymbol:"pendulum",providesChain:"pendulum-chain-2",cost:{wood:1,glass:2}},{id:"library",name:"Library",nameFr:"Bibliothèque",color:"scientific",age:2,victoryPoints:2,scienceSymbol:"inkwell",chainFrom:"inkwell-chain",cost:{stone:1,wood:1,glass:1}},{id:"dispensary",name:"Dispensary",nameFr:"Dispensaire",color:"scientific",age:2,victoryPoints:2,scienceSymbol:"mortar",chainFrom:"mortar-chain",cost:{clay:2,stone:1}},{id:"forum",name:"Forum",nameFr:"Forum",color:"commercial",age:2,victoryPoints:0,providesChain:"barrel",coinCost:3,cost:{clay:1}},{id:"caravansery",name:"Caravansery",nameFr:"Caravansérail",color:"commercial",age:2,victoryPoints:0,coinCost:2,cost:{glass:1,papyrus:1}},{id:"customs-house",name:"Customs House",nameFr:"Douanes",color:"commercial",age:2,victoryPoints:0,coinCost:4},{id:"brewery",name:"Brewery",nameFr:"Brasserie",color:"commercial",age:2,victoryPoints:0,providesChain:"barrel-2"},{id:"horse-breeders",name:"Horse Breeders",nameFr:"Haras",color:"military",age:2,victoryPoints:0,shields:1,chainFrom:"horseshoe",cost:{clay:1,wood:1}},{id:"barracks",name:"Barracks",nameFr:"Baraquements",color:"military",age:2,victoryPoints:0,shields:1,chainFrom:"sword",coinCost:3},{id:"archery-range",name:"Archery Range",nameFr:"Champ de tir",color:"military",age:2,victoryPoints:0,shields:2,providesChain:"target",cost:{stone:1,wood:1,papyrus:1}},{id:"parade-ground",name:"Parade Ground",nameFr:"Place d'armes",color:"military",age:2,victoryPoints:0,shields:2,providesChain:"mask",cost:{clay:2,glass:1}},{id:"walls",name:"Walls",nameFr:"Muraille",color:"military",age:2,victoryPoints:0,shields:2,cost:{stone:2}}],Gb=[{id:"pantheon",name:"Pantheon",nameFr:"Panthéon",color:"civilian",age:3,victoryPoints:6,chainFrom:"sun",cost:{clay:1,wood:1,papyrus:2}},{id:"gardens",name:"Gardens",nameFr:"Jardins",color:"civilian",age:3,victoryPoints:6,chainFrom:"column",cost:{clay:2,wood:2}},{id:"town-hall",name:"Town Hall",nameFr:"Hôtel de ville",color:"civilian",age:3,victoryPoints:7,cost:{stone:3,wood:2}},{id:"palace",name:"Palace",nameFr:"Palace",color:"civilian",age:3,victoryPoints:7,cost:{clay:1,stone:1,wood:1,glass:2}},{id:"senate",name:"Senate",nameFr:"Sénat",color:"civilian",age:3,victoryPoints:5,chainFrom:"horseshoe",cost:{clay:2,stone:1,papyrus:1}},{id:"obelisk",name:"Obelisk",nameFr:"Obélisque",color:"civilian",age:3,victoryPoints:5,cost:{stone:2,glass:1}},{id:"academy",name:"Academy",nameFr:"Académie",color:"scientific",age:3,victoryPoints:3,scienceSymbol:"sundial",cost:{stone:1,wood:1,glass:2}},{id:"study",name:"Study",nameFr:"Étude",color:"scientific",age:3,victoryPoints:3,scienceSymbol:"sundial",cost:{wood:2,glass:1,papyrus:1}},{id:"university",name:"University",nameFr:"Université",color:"scientific",age:3,victoryPoints:2,scienceSymbol:"globe",chainFrom:"wheel-chain-2",cost:{clay:1,glass:1,papyrus:1}},{id:"observatory",name:"Observatory",nameFr:"Observatoire",color:"scientific",age:3,victoryPoints:2,scienceSymbol:"globe",chainFrom:"pendulum-chain-2",cost:{stone:1,papyrus:2}},{id:"chamber-of-commerce",name:"Chamber of Commerce",nameFr:"Chambre de commerce",color:"commercial",age:3,victoryPoints:3,variableScoring:"chamberOfCommerce",cost:{papyrus:2}},{id:"port",name:"Port",nameFr:"Port",color:"commercial",age:3,victoryPoints:3,variableScoring:"port",cost:{wood:1,glass:1,papyrus:1}},{id:"armory",name:"Armory",nameFr:"Armurerie",color:"commercial",age:3,victoryPoints:3,variableScoring:"armory",cost:{stone:2,glass:1}},{id:"lighthouse",name:"Lighthouse",nameFr:"Phare",color:"commercial",age:3,victoryPoints:3,variableScoring:"lighthouse",chainFrom:"jug",cost:{clay:2,glass:1}},{id:"arena",name:"Arena",nameFr:"Arène",color:"commercial",age:3,victoryPoints:3,variableScoring:"arena",chainFrom:"barrel-2",cost:{clay:1,stone:1,wood:1}},{id:"pretorium",name:"Pretorium",nameFr:"Prétoire",color:"military",age:3,victoryPoints:0,shields:3,coinCost:8},{id:"arsenal",name:"Arsenal",nameFr:"Arsenal",color:"military",age:3,victoryPoints:0,shields:3,cost:{clay:3,wood:2}},{id:"fortifications",name:"Fortifications",nameFr:"Fortifications",color:"military",age:3,victoryPoints:0,shields:2,chainFrom:"tower",cost:{stone:2,clay:1,papyrus:1}},{id:"siege-workshop",name:"Siege Workshop",nameFr:"Atelier de siège",color:"military",age:3,victoryPoints:0,shields:2,chainFrom:"target",cost:{wood:3,glass:1}},{id:"circus",name:"Circus",nameFr:"Cirque",color:"military",age:3,victoryPoints:0,shields:2,chainFrom:"mask",cost:{clay:2,stone:2}}],Wb=[...Lb,...Fb,...Gb,...Ub];Object.fromEntries(Wb.map(e=>[e.id,e]));const qb=Object.fromEntries([{id:"the-appian-way",name:"The Appian Way",nameFr:"La Via Appia",victoryPoints:3,description:"The opponent loses 3 coins. Take another turn. Once built, repeated discards are not affected. Worth 3 victory points."},{id:"circus-maximus",name:"Circus Maximus",nameFr:"Le Circus Maximus",victoryPoints:3,shields:1,description:"Destroy one grey (manufactured) card the opponent has built. Provides 1 shield. Worth 3 victory points."},{id:"the-colossus",name:"The Colossus",nameFr:"Le Colosse",victoryPoints:3,shields:2,description:"Provides 2 shields. Worth 3 victory points."},{id:"the-great-library",name:"The Great Library",nameFr:"La Grande Bibliothèque",victoryPoints:4,description:"Randomly draw 3 of the Progress tokens discarded at game setup and keep one. Worth 4 victory points."},{id:"the-great-lighthouse",name:"The Great Lighthouse",nameFr:"Le Grand Phare",victoryPoints:4,description:"Once built, the owner may take any raw or manufactured good of choice each turn (production effect). Worth 4 victory points."},{id:"the-hanging-gardens",name:"The Hanging Gardens",nameFr:"Les Jardins Suspendus",victoryPoints:3,description:"Gain 6 coins. Take another turn. Worth 3 victory points."},{id:"the-mausoleum",name:"The Mausoleum",nameFr:"Le Mausolée",victoryPoints:2,description:"Build, for free, any one card from the discard pile. Worth 2 victory points."},{id:"piraeus",name:"Piraeus",nameFr:"Le Pirée",victoryPoints:2,description:"Once built, the owner may take any one manufactured good (glass or papyrus) of choice each turn. Take another turn. Worth 2 victory points."},{id:"the-pyramids",name:"The Pyramids",nameFr:"Les Pyramides",victoryPoints:9,description:"Worth 9 victory points."},{id:"the-sphinx",name:"The Sphinx",nameFr:"Le Sphinx",victoryPoints:6,description:"Take another turn. Worth 6 victory points."},{id:"the-statue-of-zeus",name:"The Statue of Zeus",nameFr:"La Statue de Zeus",victoryPoints:3,shields:1,description:"Destroy one brown (raw) card the opponent has built. Provides 1 shield. Worth 3 victory points."},{id:"the-temple-of-artemis",name:"The Temple of Artemis",nameFr:"Le Temple d'Artémis",victoryPoints:0,description:"Gain 12 coins. Take another turn. Worth 0 victory points."}].map(e=>[e.id,e]));Object.fromEntries([{id:"agriculture",name:"Agriculture",nameFr:"Agriculture",victoryPoints:4,description:"Gain 6 coins immediately. Worth 4 victory points at game end."},{id:"architecture",name:"Architecture",nameFr:"Architecture",description:"Any future Wonder constructed by the owner costs 2 fewer resources of the owner's choice."},{id:"economy",name:"Economy",nameFr:"Économie",description:"When the opponent uses the trading-cost coins (pays the bank to buy goods), the owner receives those coins instead."},{id:"law",name:"Law",nameFr:"Loi",variableScoring:"law",description:"Grants one science symbol, counting toward the six-symbol scientific victory and toward pairs of identical symbols."},{id:"masonry",name:"Masonry",nameFr:"Maçonnerie",description:"Any future blue (civilian) building constructed by the owner costs 2 fewer resources of the owner's choice."},{id:"mathematics",name:"Mathematics",nameFr:"Mathématiques",variableScoring:"mathematics",description:"Worth 3 victory points at game end for EACH Progress token the owner possesses (including this one)."},{id:"philosophy",name:"Philosophy",nameFr:"Philosophie",victoryPoints:7,description:"Worth 7 victory points at game end."},{id:"strategy",name:"Strategy",nameFr:"Stratégie",description:"Whenever the owner builds a red (military) building, it provides 1 additional shield."},{id:"theology",name:"Theology",nameFr:"Théologie",description:"Every future Wonder built by the owner grants an extra turn."},{id:"urbanism",name:"Urbanism",nameFr:"Urbanisme",description:"Gain 6 coins immediately. When the owner builds a card for free via a chain link, they also gain 4 coins."}].map(e=>[e.id,e]));const Ne="/7wd-scorer/models/";let jf=!1;const Pr=new Map;function Kf(){var e;jf||(ze.wasm.wasmPaths="/7wd-scorer/ort/",ze.wasm.numThreads=globalThis.crossOriginIsolated?Math.max(1,(((e=globalThis.navigator)==null?void 0:e.hardwareConcurrency)??4)-2):1,jf=!0)}const ns=new Set;function Vb(e){Kf();let t=Pr.get(e);return t===void 0&&(t=nt.create(`${Ne}${kt[e].onnx}`,{executionProviders:ns.has(e)?["wasm"]:["webgpu","wasm"]}),Pr.set(e,t),t.catch(()=>Pr.delete(e))),t}let rs=null,is=null;const Hb=.75,jb=4,Kb=.65,Yb=3e4;let as=null;function ss(){return as===null&&(as=(async()=>{try{let e;return self.importScripts("/7wd-scorer/opencv/opencv.js"),e=self.cv,typeof(e==null?void 0:e.then)=="function"&&(e=await e),typeof(e==null?void 0:e.getBuildInformation)!="function"&&(e=await new Promise(t=>{e.onRuntimeInitialized=()=>t(e)})),e}catch(e){return console.warn("[wonders-reg] opencv.js load failed:",e),null}})()),as}const Yf=new Map;function os(e){let t=Yf.get(e);return t===void 0&&(t=(async()=>{try{const n=await fetch(`${Ne}${e}`);if(!n.ok)return null;const r=await createImageBitmap(await n.blob()),a=new OffscreenCanvas(r.width,r.height).getContext("2d");a.drawImage(r,0,0);const s=a.getImageData(0,0,r.width,r.height);return{width:r.width,height:r.height,channels:4,data:new Uint8Array(s.data.buffer)}}catch{return null}})(),Yf.set(e,t)),t}function us(e){return os(`wonder-refs/${e}.jpg`)}const Xf=["builders-guild","magistrates-guild","merchants-guild","moneylenders-guild","scientists-guild","shipowners-guild","tacticians-guild"];async function Xb(){const e=new Map;for(const t of Xf){const n=await os(`guild-refs/${t}.jpg`);n!==null&&e.set(t,n)}return e}async function Zb(){const e=new Map;for(const t of Xf){const n=await os(`guild-band-refs/${t}.png`);n!==null&&e.set(t,n)}return e}const Qb=.6,Jb=12,e1=45e3;let ls=null;function Zf(){return ls===null&&(Kf(),ls=(async()=>{try{const[e,t,n,r]=await Promise.all([nt.create(`${Ne}ocr/ch_PP-OCRv4_det_infer.onnx`,{executionProviders:["webgpu","wasm"]}),nt.create(`${Ne}ocr/ch_PP-OCRv4_rec_infer.onnx`,{executionProviders:["webgpu","wasm"]}),fetch(`${Ne}ocr_charset.json`).then(i=>i.ok?i.json():null),fetch(`${Ne}wonder_names.json`).then(i=>i.ok?i.json():null)]);return n===null||r===null?(console.warn("[wonders-ocr] charset/names asset missing"),null):{det:e,rec:t,charset:ww(n),catalog:r.entries}}catch(e){return console.warn("[wonders-ocr] bundle load failed:",e),null}})()),ls}async function t1(e,t){const n=Math.max(yw/Pt,t.width/t.height),{tensor:r,width:i}=bw(t,n),a={[e.rec.inputNames[0]]:new Le("float32",r,[1,3,Pt,i])},s=(await e.rec.run(a))[e.rec.outputNames[0]],[o,u,l]=s.dims,h=s.data,c=new Array(u),p=new Array(u);for(let m=0;m<u;m++){let g=0,y=-1/0;const _=m*l;for(let $=0;$<l;$++){const x=h[_+$];x>y&&(y=x,g=$)}c[m]=g,p[m]=y}return _w(c,p,e.charset)}async function n1(e,t){const n=await Zf();if(n===null)return{wonders:[],aborted:!1};const r=new Map,i=Date.now()+e1;let a=!1;e:for(const s of[0,1,2,3]){if(Date.now()>i){a=!0;break}t(`wonder names: rotation ${s*90}°…`,s/4);const o=Yt(e,s),u=ow(o),l={[n.det.inputNames[0]]:new Le("float32",u.tensor,[1,3,u.height,u.width])},h=(await n.det.run(l))[n.det.outputNames[0]],c=pw(h.data,u,o.width,o.height).slice(0,Jb);console.debug(`[wonders-ocr] rot ${s*90}: ${c.length} det boxes`,c.slice(0,5).map(p=>`${p.width}x${p.height}@${p.score.toFixed(2)}`));for(const p of c){if(Date.now()>i){a=!0;break e}const m=fw(o,p.quad);if(m.width<m.height*1.5)continue;const[g,y]=await t1(n,m);if(console.debug(`[wonders-ocr] rec "${g}" @${y.toFixed(2)}`),y<Qb||g.trim().length<jb)continue;const _=kw(g,n.catalog);if(console.debug("[wonders-ocr] fuzzy",_),_===null||_.confidence<Hb||_.kind!=="wonder")continue;const $=r.get(_.id);($===void 0||_.confidence>$.confidence)&&r.set(_.id,{id:_.id,name:_.name,confidence:_.confidence,nameBox:Qf(p,s,e.width,e.height)})}}return{wonders:[...r.values()],aborted:a}}function Qf(e,t,n,r){const i=(t%4+4)%4;if(i===0)return{x:e.x,y:e.y,width:e.width,height:e.height};const a=(c,p)=>i===1?[p,r-1-c]:i===2?[n-1-c,r-1-p]:[n-1-p,c],s=[a(e.x,e.y),a(e.x+e.width,e.y+e.height)],o=s.map(c=>c[0]),u=s.map(c=>c[1]),l=Math.min(...o),h=Math.min(...u);return{x:l,y:h,width:Math.max(...o)-l,height:Math.max(...u)-h}}function r1(){return is===null&&(is=fetch(`${Ne}laurel_gallery.json`).then(async e=>e.ok?Yy(await e.json()):[]).catch(()=>[])),is}function i1(e,t,n,r){return Dr(e,t-r,n-r,2*r,2*r)}function Dr(e,t,n,r,i){const a=Math.max(0,Math.round(t)),s=Math.max(0,Math.round(n)),o=Math.min(e.width,Math.round(t+r)),u=Math.min(e.height,Math.round(n+i)),l=Math.max(0,o-a),h=Math.max(0,u-s),c=new Uint8Array(l*h*3);for(let p=0;p<h;p++)for(let m=0;m<l;m++){const g=((p+s)*e.width+(m+a))*e.channels,y=(p*l+m)*3;c[y]=e.data[g],c[y+1]=e.data[g+1],c[y+2]=e.data[g+2]}return{width:l,height:h,channels:3,data:c}}function a1(){return rs===null&&(rs=fetch(`${Ne}token_templates.json`).then(async e=>e.ok?W_(await e.json()):new Map).catch(()=>new Map)),rs}let ds=null;function s1(){return ds===null&&(ds=(async()=>{try{const e=await fetch(`${Ne}token_embed_index.json`);if(!e.ok)return null;const t=Z_(await e.json());return{session:await nt.create(`${Ne}token_embed.onnx`,{executionProviders:["wasm"]}),index:t}}catch{return null}})()),ds}const o1=.92;let cs=null;function u1(){return cs===null&&(cs=(async()=>{try{return(await fetch(`${Ne}guild_classifier.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}guild_classifier.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),cs}let hs=null;function l1(){return hs===null&&(hs=(async()=>{try{return(await fetch(`${Ne}laurel_digit.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}laurel_digit.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),hs}let ps=null;function d1(){return ps===null&&(ps=(async()=>{try{return(await fetch(`${Ne}laurel_filter.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}laurel_filter.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ps}async function c1(e,t,n){const[r,i,a,s]=t,o=a-r,u=s-i;if(o<=0||u<=0)return null;const l=Math.trunc(qf*o),h=Math.trunc(qf*u),c=Math.max(0,r-l),p=Math.max(0,i-h),m=Math.min(e.width,a+l),g=Math.min(e.height,s+h),y=Dr(e,c,p,m-c,g-p);if(y.width<=0||y.height<=0)return null;try{const _=bb(y),$=await n.run({[n.inputNames[0]]:new Le("float32",_,[1,3,zn,zn])});return $b($[n.outputNames[0]].data)}catch{return null}}let fs=null;function h1(){return fs===null&&(fs=(async()=>{try{return(await fetch(`${Ne}coin_filter_cnn.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}coin_filter_cnn.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),fs}async function p1(e,t,n){if(t.length===0)return[];try{const r=async o=>{const u=[];for(let p=0;p<t.length;p++){const m=vb(e,Math.round(t[p].cx),Math.round(t[p].cy),Math.round(o[p]));if(m===null)return null;u.push(m)}const l=new Float32Array(t.length*3*Zt*Zt);u.forEach((p,m)=>l.set(p,m*p.length));const c=(await n.run({[n.inputNames[0]]:new Le("float32",l,[t.length,3,Zt,Zt])}))[n.outputNames[0]].data;return t.map((p,m)=>Sb(c.subarray(m*2,m*2+2)))},i=await r(t.map(o=>o.r));if(i===null)return null;const a=t.map(o=>o.r).sort((o,u)=>o-u),s=a.length%2===1?a[(a.length-1)/2]:(a[a.length/2-1]+a[a.length/2])/2;if(s>=8){const o=await r(t.map(()=>s));if(o!==null)return i.map((u,l)=>Math.max(u,o[l]))}return i}catch{return null}}let ms=null;function Jf(){return ms===null&&(ms=(async()=>{try{return(await fetch(`${Ne}tuck_classifier.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}tuck_classifier.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ms}const em=.2,f1=.3,tm=.25,m1=.1;let gs=null;function g1(){return gs===null&&(gs=(async()=>{try{return(await fetch(`${Ne}track_band.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}track_band.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),gs}let ys=null;function y1(e,t,n){if(t.height<=0)return!1;const r=t.width/t.height;if(Math.abs(Math.log(r))<=tm)return!1;const i=e.x+e.width,a=e.y+e.height;for(const s of n){const o=s.box;if(!o||o.length<4||o[3]<=0)continue;const u=o[0]+o[2]/2,l=o[1]+o[3]/2;if(!(u>=e.x&&u<=i&&l>=e.y&&l<=a))continue;const h=o[2]/o[3];if(!(Math.abs(Math.log(h))<=tm)&&r>1==h>1)return!0}return!1}const w1=.4;function _1(e,t){const n=Math.min(e.x+e.width,t.x+t.width)-Math.max(e.x,t.x),r=Math.min(e.y+e.height,t.y+t.height)-Math.max(e.y,t.y);if(n<=0||r<=0)return 0;const i=e.width*e.height;return i>0?n*r/i:0}function b1(e,t){const n=[],r=[];for(const i of t){if(!i.builtWithCardUnderneath)continue;i.boundingBox&&n.push(i.boundingBox);const a=i.tuckRegion;a&&r.push(a)}return n.length===0&&r.length===0?e:e.filter(i=>{const a=i.boundingBox;if(!a)return!0;const s=a.x+a.width/2,o=a.y+a.height/2;for(const u of n)if(s>=u.x&&s<=u.x+u.width&&o>=u.y&&o<=u.y+u.height||_1(a,u)>=w1)return!1;for(const u of r)if(s>=u.x&&s<=u.x+u.width&&o>=u.y&&o<=u.y+u.height)return!1;return!0})}function $1(){return ys===null&&(ys=(async()=>{try{return(await fetch(`${Ne}tuck_box.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}tuck_box.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ys}async function x1(e,t,n){const[r,i,a,s]=t;if(a<=0||s<=0)return null;const o=Math.round(a*em),u=Math.round(s*em),l=Math.max(0,Math.round(r-o)),h=Math.max(0,Math.round(i-u)),c=Math.min(e.width,Math.round(r+a+o)),p=Math.min(e.height,Math.round(i+s+u)),m=c-l,g=p-h;if(m<=0||g<=0)return null;const y=e.channels,_=new Uint8ClampedArray(m*g*y);for(let T=0;T<g;T++){const S=((h+T)*e.width+l)*y;_.set(e.data.subarray(S,S+m*y),T*m*y)}const $={width:m,height:g,channels:y,data:_};let x=null;for(let T=0;T<4;T++){const S=T===0?$:Yt($,T),I=S.width,E=I-Math.floor(f1*I),M=I-E;if(M<=0)continue;const v=new Uint8ClampedArray(M*S.height*S.channels);for(let H=0;H<S.height;H++){const A=(H*I+E)*S.channels;v.set(S.data.subarray(A,A+M*S.channels),H*M*S.channels)}const z={width:M,height:S.height,channels:S.channels,data:v},B=Qa(z),U=(await n.run({[n.inputNames[0]]:new Le("float32",B,[1,3,Et,Et])}))[n.outputNames[0]].data[1]??0;x=x===null?U:Math.max(x,U)}return x}let ws=null;function v1(){return ws===null&&(ws=(async()=>{try{return(await fetch(`${Ne}wonder_classifier.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}wonder_classifier.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ws}async function S1(e,t,n,r,i,a){var p;const s=(m,g,y,_)=>{const $=Math.max(0,Math.round(m)),x=Math.max(0,Math.round(g)),T=Math.min(t.width,Math.round(m+y)),S=Math.min(t.height,Math.round(g+_)),I=T-$,E=S-x;if(I<=0||E<=0)return null;const M=t.channels,v=new Uint8ClampedArray(I*E*M);for(let z=0;z<E;z++){const B=((x+z)*t.width+$)*M;v.set(t.data.subarray(B,B+I*M),z*I*M)}return{width:I,height:E,channels:M,data:v}},o=async m=>(await r.run({[r.inputNames[0]]:new Le("float32",m,[1,3,An,An])}))[r.outputNames[0]].data,u=new Map;for(const m of n){const[g,y,_,$]=m;if(_<=0||$<=0)continue;const x=s(g,y,_,$);if(x===null)continue;const{id:T,prob:S}=await pb(x,o);if(T===""||S<ub)continue;const I=u.get(T);(I===void 0||S>I.prob)&&u.set(T,{prob:S,box:m})}const l=[],h=await Jf(),c=await $1();for(const[m,{prob:g,box:y}]of u){const[_,$,x,T]=y;let S={x:Math.round(_),y:Math.round($),width:Math.round(x),height:Math.round(T)},I=null,E=[],M=null;if(Date.now()<i)try{const G=await us(m);if(G!==null){const R=Cf(e,t,G,y);if(R!==null){I=R.footprint,E=R.overflow;const P=I.map(D=>D[0]),Y=I.map(D=>D[1]),O=Math.max(0,Math.round(Math.min(...P))),Z=Math.max(0,Math.round(Math.min(...Y)));if(S={x:O,y:Z,width:Math.min(t.width,Math.round(Math.max(...P)))-O,height:Math.min(t.height,Math.round(Math.max(...Y)))-Z},h!==null)try{const D=Mf(e,t,G,I);if(D!==null){const V=Qa(D),F=await h.run({[h.inputNames[0]]:new Le("float32",V,[1,3,Et,Et])});M=Rf(F[h.outputNames[0]].data).prob}}catch{}}}}catch(G){console.warn(`[wonders-cls] ${m} registration failed:`,G)}const v=I!==null?Xa(I,E):null,z=[];if(M!==null&&z.push(M>=Za?1:0),c!==null)try{const G=await x1(t,y,c);G!==null&&z.push(G>=Za?1:0)}catch{}const B=v??S,X=a.some(G=>{const R=G.box[0]+G.box[2]/2,P=G.box[1]+G.box[3]/2;return R>=B.x&&R<=B.x+B.width&&P>=B.y&&P<=B.y+B.height});z.push(X?1:0);let U=z.length>0&&z.reduce((G,R)=>G+R,0)*2>z.length;U&&y1(B,S,a)&&(U=!1);const H={id:m,name:((p=qb[m])==null?void 0:p.name)??m,builtWithCardUnderneath:U,boundingBox:S,confidence:Math.round(g*1e4)/1e4,...v?{tuckRegion:v}:{}},A=v??S;l.push({obj:H,edgeScores:null,zone:{x0:A.x,y0:A.y,x1:A.x+A.width,y1:A.y+A.height}})}return l}async function T1(e,t){const n=await s1();if(n!==null)try{const r=J_(e),i=new Le("float32",r,[4,3,Xt,Xt]),s=(await n.session.run({image:i}))[n.session.outputNames[0]].data,{id:o,cosine:u}=tb(n.index,eb(s));return u<o1?["",-1]:[o,u]}catch{}return j_(e,t)}async function _s(e){const t=await createImageBitmap(e);try{const r=new OffscreenCanvas(t.width,t.height).getContext("2d",{willReadFrequently:!0});if(r===null)throw new Error("OffscreenCanvas 2D context unavailable.");r.drawImage(t,0,0);const{data:i}=r.getImageData(0,0,t.width,t.height);return{width:t.width,height:t.height,channels:4,data:i}}finally{t.close()}}async function Qt(e,t){const n=kt[e],{tensor:r,params:i}=of(t,n.input),a=async()=>{const s=await Vb(e),o={[s.inputNames[0]]:new Le("float32",r,[1,3,n.input,n.input])};return{rows:(await s.run(o))[s.outputNames[0]].data,params:i}};try{return await a()}catch(s){if(ns.has(e))throw s;return ns.add(e),Pr.delete(e),await a()}}const I1=6,k1=2,E1=5,M1=2;async function C1(e){const t={kind:"unknown",confidence:0,banners:null,laurels:null,coins:null,pawnFound:!1},n=await _s(e),r=await Qt("banner",n),i=ff(r.rows,r.params,kt.banner.conf);if(t.banners=i.length,i.length>=I1)return{...t,kind:"player",confidence:Math.min(1,i.length/12)};const a=await Qt("laurel",n),s=Da(a.rows,a.params,kt.laurel.conf);if(t.laurels=s.length,s.length>=k1)return{...t,kind:"player",confidence:Math.min(1,s.length/8)};const o=await Qt("coin",n),u=hf(o.rows,o.params,kt.coin.conf);return t.coins=u.length,u.length>=E1?{...t,kind:"player",confidence:.5}:t.banners!==null&&t.banners<=M1?{...t,kind:"board",confidence:.4}:t}function A1(){return{wonders:[],guilds:[],progressTokens:[],laurels:[],cardVictoryPoints:{value:0,laurelsKept:0,laurelsUnread:0,complete:!0},cardCounts:{byFamily:{},source:"none",tuckedExcluded:0},coins:{total:0,confidence:0,source:"none",coins:[]}}}async function bs(e,t,n,r,i=()=>{},a="player"){const s={},o=[],u=[],l=[],h=[],c=[],p=[];let m=0,g=0,y=0,_=0,$=0;for(const E of e){$+=1;const M=`${t} photo ${$}/${e.length}`;r(`${M}: reading pixels…`,.01);const v=await _s(E);r(`${M}: card banners…`,.04);const z=await Qt("banner",v);let B=ff(z.rows,z.params,kt.banner.conf);r(`${M}: progress tokens…`,.08);let X=[];const U=await g1();if(U!==null)try{const q=of(v,1280),Q=await U.run({[U.inputNames[0]]:new Le("float32",q.tensor,[1,3,1280,1280])});X=Da(Q[U.outputNames[0]].data,q.params,m1)}catch{}const H=await Qt("token",v),A=await a1(),G=l.length,R=[];for(const q of hy(H.rows,H.params,kt.token.conf)){if(R.push({cx:q.cx,cy:q.cy,r:q.r}),X.some(([le,de,Te,ce])=>q.cx>=le&&q.cx<=Te&&q.cy>=de&&q.cy<=ce))continue;const[Q,te]=await T1(wf(v,q),A);Q===""&&te<0?R.pop():Q===""?g+=1:l.some(le=>le.id===Q)||l.push({id:Q,center:[q.cx,q.cy],radius:q.r,confidence:Math.round(te*1e4)/1e4})}r(`${M}: coins…`,.14);const P=await Qt("coin",v),Y=hf(P.rows,P.params,kt.coin.conf).filter(q=>!R.some(Q=>(q.cx-Q.cx)**2+(q.cy-Q.cy)**2<=q.r*q.r)),O=Oy(v,Y),Z=await h1(),D=Z!==null?await p1(v,Y,Z):null,V=(D!==null?Y.filter((q,Q)=>D[Q]>=Vf).map(q=>q.r):[]).sort((q,Q)=>q-Q),F=V.length>0?V.length%2===1?V[(V.length-1)/2]:(V[V.length/2-1]+V[V.length/2])/2:null,L=[];if(Y.forEach((q,Q)=>{const te=D!==null?D[Q]:null;if(te!==null&&te<Vf){const[de,Te]=xb;if(!(F!==null&&F>0&&q.r/F>=de&&q.r/F<=Te))return;L.push({denomination:null,center:[q.cx,q.cy],radius:q.r,suspect:!0,suspectReason:`content rejected as non-coin (P=${te.toFixed(2)}) but the size matches this photo's confirmed coins — glare-blinded real coin OR a look-alike object; confirm or remove (a busy table warrants a cleaner photo)`});return}const le=O[Q];L.push({denomination:le,center:[q.cx,q.cy],radius:q.r,denomSource:"colour"})}),L.length>=2){const q=L.map(te=>te.radius).sort((te,le)=>te-le),Q=q.length%2===1?q[(q.length-1)/2]:(q[q.length/2-1]+q[q.length/2])/2;if(Q>0)for(const te of L)te.radius/Q>2&&(te.suspect=!0,te.suspectReason=`radius ${te.radius}px is ${(te.radius/Q).toFixed(1)}x the photo's median coin radius — probably not a coin`)}const ne=h.length,se=[],re=Date.now()+Yb;let fe=null,Ae=null;const Ge=()=>(Ae===null&&(Ae=(async()=>{try{const{rows:q,params:Q}=await Qt("wonder",v);return uf(q,Q,kt.wonder.conf,Number.POSITIVE_INFINITY).map(te=>te.box)}catch{return[]}})()),Ae),Be=[];let Je=!1;const Me=await v1();if(Me!==null){const q=await Ge();if(q.length>0&&(fe=await ss(),fe!==null)){r(`${M}: identifying wonders…`,.35);const Q=await S1(fe,v,q,Me,re,B);for(const te of Q)h.some(le=>le.id===te.obj.id)||(h.push(te.obj),Be.push({obj:te.obj,edgeScores:te.edgeScores,zone:te.zone}),se.push(te.zone));Je=Q.length>0}}Je||r(`${M}: wonder names…`,.2);const pe=Je?{wonders:[],aborted:!1}:await n1(v,(q,Q)=>r(`${M}: ${q}`,.2+.35*(Q??0)));fe===null&&(fe=pe.wonders.length>0?await ss():null);for(const q of pe.wonders){let Q=null;if(fe!==null&&Date.now()<re){r(`${M}: registering ${q.name}…`,.6);try{const te=await us(q.id);if(te!==null){let le=Uw(fe,v,te,[[q.nameBox.x,q.nameBox.y],[q.nameBox.x+q.nameBox.width,q.nameBox.y],[q.nameBox.x+q.nameBox.width,q.nameBox.y+q.nameBox.height],[q.nameBox.x,q.nameBox.y+q.nameBox.height]]);if(le===null){const de=await Ge(),Te=qw(de,q.nameBox.x+q.nameBox.width/2,q.nameBox.y+q.nameBox.height/2);Te!==null&&(le=Cf(fe,v,te,Te))}if(le!==null){let de=le.built,Te=!1;const ce=await Jf();if(ce!==null)try{const De=Mf(fe,v,te,le.footprint);if(De!==null){const et=Qa(De),Ze=await ce.run({[ce.inputNames[0]]:new Le("float32",et,[1,3,Et,Et])});de=Rf(Ze[ce.outputNames[0]].data).built,Te=!0}}catch{}const ge=le.footprint.map(De=>De[0]),Ce=le.footprint.map(De=>De[1]),Ee=Math.max(0,Math.round(Math.min(...ge))),We=Math.max(0,Math.round(Math.min(...Ce)));Q={built:de,boundingBox:{x:Ee,y:We,width:Math.min(v.width,Math.round(Math.max(...ge)))-Ee,height:Math.min(v.height,Math.round(Math.max(...Ce)))-We},tuckRegion:Xa(le.footprint,le.overflow),edgeScores:le.edgeScores,builtByTuck:Te}}}}catch(te){console.warn(`[wonders-reg] ${q.id} failed:`,te)}}if(Q!==null){const te=Q.tuckRegion??Q.boundingBox;se.push({x0:te.x,y0:te.y,x1:te.x+te.width,y1:te.y+te.height})}else{const te=Math.max(8,q.nameBox.height),le=Math.round(q.nameBox.width*.15);se.push({x0:q.nameBox.x-le,y0:q.nameBox.y-te*2.5,x1:q.nameBox.x+q.nameBox.width+le,y1:q.nameBox.y+q.nameBox.height+te*2.5})}if(!h.some(te=>te.id===q.id)){const te=(Q==null?void 0:Q.builtByTuck)===!0,le=te?Q.built:!1,de=!te&&(Q==null?void 0:Q.built)===!0,Te={id:q.id,name:q.name,builtWithCardUnderneath:le,boundingBox:(Q==null?void 0:Q.boundingBox)??{x:0,y:0,width:0,height:0},...Q!=null&&Q.tuckRegion?{tuckRegion:Q.tuckRegion}:{},confidence:q.confidence,...de?{suspect:!0,suspectReason:"built-unconfirmed"}:{}};h.push(Te),Be.push({obj:Te,edgeScores:Q&&!Q.builtByTuck?Q.edgeScores:null,zone:se[se.length-1]})}}if(!Je){const q=jw(Be.map(Q=>({built:Q.obj.builtWithCardUnderneath,edgeScores:Q.edgeScores,zone:Q.zone})),B.map(Q=>[Q.box[0]+Q.box[2]/2,Q.box[1]+Q.box[3]/2]));for(const Q of q){const te=Be[Q];te.obj.builtWithCardUnderneath=!1,n.push({code:"INCONSISTENT_STATE",message:`${t}: wonder '${te.obj.id}' was NOT marked built — the card-under-wonder signal saturated on this surface and no tucked card banner supports it. Tick it in the review if it really was built.`})}if(B.length>0){const Q=new Set(q);for(let te=0;te<Be.length;te++){const le=Be[te];if(Q.has(te)||!le.obj.builtWithCardUnderneath)continue;const de=le.obj.tuckRegion;if(de===void 0)continue;if(!B.some(ce=>{const ge=ce.box[0]+ce.box[2]/2,Ce=ce.box[1]+ce.box[3]/2;return ge>=de.x&&ge<=de.x+de.width&&Ce>=de.y&&Ce<=de.y+de.height})){const ce=le.obj;ce.builtWithCardUnderneath=!1,ce.suspect=!0,ce.suspectReason="built-unconfirmed"}}}}if(pe.aborted&&n.push({code:"LOW_CONFIDENCE",message:`${M}: the wonder-name read ran out of its time budget on this device — ${pe.wonders.length} wonder(s) read before the cutoff; check the built-wonders list.`}),fe!==null&&pe.wonders.length>0&&Date.now()<re)try{const q=await Zf(),Q=(q==null?void 0:q.catalog.filter(le=>le.kind==="wonder").map(le=>le.id))??[],te=new Map;for(const le of Q)if(!h.some(de=>de.id===le)){const de=await us(le);de!==null&&te.set(le,de)}if(te.size>0){r(`${M}: searching occluded wonders…`,.7);const le=Dw(fe,v,te,re);for(const de of le){const Te=de.footprint.map(Ze=>Ze[0]),ce=de.footprint.map(Ze=>Ze[1]),ge=Math.max(0,Math.round(Math.min(...Te))),Ce=Math.max(0,Math.round(Math.min(...ce))),Ee={x:ge,y:Ce,width:Math.min(v.width,Math.round(Math.max(...Te)))-ge,height:Math.min(v.height,Math.round(Math.max(...ce)))-Ce};if(h.some(Ze=>{const at=Ze.boundingBox,Ue=Math.max(0,Math.min(at.x+at.width,Ee.x+Ee.width)-Math.max(at.x,Ee.x)),lt=Math.max(0,Math.min(at.y+at.height,Ee.y+Ee.height)-Math.max(at.y,Ee.y)),tt=Ue*lt,ht=at.width*at.height+Ee.width*Ee.height-tt;return ht>0&&tt/ht>Pw}))continue;const De=q==null?void 0:q.catalog.find(Ze=>Ze.id===de.id);h.push({id:de.id,name:(De==null?void 0:De.nameFr)??(De==null?void 0:De.name)??de.id,builtWithCardUnderneath:de.built,boundingBox:Ee,...de.tuckRegion?{tuckRegion:de.tuckRegion}:{},confidence:Math.round(de.confidence*1e4)/1e4});const et=de.tuckRegion??Ee;se.push({x0:et.x,y0:et.y,x1:et.x+et.width,y1:et.y+et.height})}}}catch(q){console.warn("[wonders-reg] discovery failed:",q)}const Xe=a==="opponent";let Ut=(q,Q)=>!Xe;try{const q=h.slice(ne),Q=[];B.forEach((ce,ge)=>{const Ce=ce.box[0]+ce.box[2]/2,Ee=ce.box[1]+ce.box[3]/2;se.some(We=>Ce>=We.x0&&Ce<=We.x1&&Ee>=We.y0&&Ee<=We.y1)||Q.push(ge)});const te=[],le=[];q.forEach((ce,ge)=>{const Ce=ce.boundingBox;Ce&&Ce.width>0&&(te.push(ge),le.push([Ce.x,Ce.y,Ce.width,Ce.height]))});const de=Db(B.map(ce=>ce.box),Q,le,X,[v.width,v.height]);Ut=(ce,ge)=>de.pointOwner(ce,ge)==="opponent"===Xe,B=B.filter((ce,ge)=>de.bannerOwner[ge]==="opponent"===Xe);const Te=q.map(()=>"player");te.forEach((ce,ge)=>{Te[ce]=de.wonderOwner[ge]});for(let ce=q.length-1;ce>=0;ce-=1)Te[ce]==="opponent"!==Xe&&h.splice(ne+ce,1);se.length=0;for(const ce of h.slice(ne)){const ge=ce.tuckRegion??ce.boundingBox;ge&&se.push({x0:ge.x,y0:ge.y,x1:ge.x+ge.width,y1:ge.y+ge.height})}for(let ce=l.length-1;ce>=G;ce-=1){const[ge,Ce]=l[ce].center;Ut(ge,Ce)||l.splice(ce,1)}}catch(q){console.warn("[city-split] failed (side unfiltered):",q)}for(const q of L)Ut(q.center[0],q.center[1])&&(m+=q.denomination??0,u.push(q));const vt=[];for(const q of B){const Q=q.box[0]+q.box[2]/2,te=q.box[1]+q.box[3]/2;if(se.some(de=>Q>=de.x0&&Q<=de.x1&&te>=de.y0&&te<=de.y1)){_+=1;continue}vt.push(q),s[q.family]=(s[q.family]??0)+1,y+=1}const ct=wy(vt),wn=new Set(ct.map(q=>q.box.join(",")));for(const q of by(vt))wn.has(q.box.join(","))||ct.push(q);for(const q of ct)p.push(q);if(vt.some(q=>q.family==="guild")){const q=await u1();if(q!==null){r(`${M}: identifying guilds…`,.75);for(const Q of vt)if(Q.family==="guild")try{const[te,le,de,Te]=Q.box,ce=Dr(v,te,le,de,Te),ge=ib(ce),Ce={[q.inputNames[0]]:new Le("float32",ge,[1,3,Cn,Cn])},We=(await q.run(Ce))[q.outputNames[0]].data,{id:De,prob:et}=ab(We);De!==""&&!c.some(Ze=>Ze.id===De)&&c.push({id:De,boundingBox:{x:te,y:le,width:de,height:Te},confidence:Math.round(et*1e4)/1e4})}catch(te){console.warn("[guild-cls] failed:",te)}}else if(Date.now()<re)try{const Q=fe??await ss();if(Q!==null){const te=await Xb();if(te.size>0){r(`${M}: identifying guilds…`,.75);const le=await Zb();for(const de of z_(Q,v,te,re,le))c.some(Te=>Te.id===de.id)||c.push(de)}}}catch(Q){console.warn("[guilds-reg] failed:",Q)}}r(`${M}: laurels…`,.8);const Lt=await r1(),Nn=[];for(const q of[0,1,2,3]){const Q=q===0?v:Yt(v,q),te=await Qt("laurel",Q);for(const[le,de,Te,ce]of Da(te.rows,te.params,kt.laurel.conf)){const ge=Qf({x:le,y:de,width:Te-le,height:ce-de},q,v.width,v.height),Ce=ge.x+ge.width/2,Ee=ge.y+ge.height/2,We=.6*Math.max(ge.width,ge.height);Nn.some(([et,Ze,at,Ue])=>{const lt=(et+at)/2,tt=(Ze+Ue)/2;return(Ce-lt)**2+(Ee-tt)**2<We*We})||Nn.push([ge.x,ge.y,ge.x+ge.width,ge.y+ge.height])}}const _n=await l1(),rr=await d1();for(const[q,Q,te,le]of Nn){const de=Math.trunc((q+te)/2),Te=Math.trunc((Q+le)/2);if([...R,...Y].some(Ue=>(de-Ue.cx)**2+(Te-Ue.cy)**2<=Ue.r*Ue.r)||!Ut(de,Te))continue;if(rr!==null){const Ue=await c1(v,[Math.trunc(q),Math.trunc(Q),Math.trunc(te),Math.trunc(le)],rr);if(Ue!==null&&Ue>=_b)continue}const ge=Math.max(6,Math.trunc(Math.max(te-q,le-Q)*Ly)),Ce=i1(v,de,Te,ge);let Ee=null,We=0;const De=new Map;for(const Ue of[0,1,2,3]){const lt=Ue===0?Ce:Yt(Ce,Ue),[tt,ht]=ew(lt,Lt);tt!==null&&(De.set(tt,Math.max(De.get(tt)??0,ht)),ht>We&&(Ee=tt,We=ht))}Ee!==null&&We<Kb&&(Ee=null);const et=We;if(_n!==null){const Ue=Dr(v,Math.trunc(q),Math.trunc(Q),Math.trunc(te-q),Math.trunc(le-Q));let lt=null,tt=0;for(const ht of[0,1,2,3]){const Ur=ht===0?Ue:Yt(Ue,ht),Ss=gb(Ur),Lr=await _n.run({[_n.inputNames[0]]:new Le("float32",Ss,[1,3,Rn,Rn])}),{value:Fr,prob:ir}=yb(Lr[_n.outputNames[0]].data);ir>tt&&(lt=Fr,tt=ir)}lt!==null&&tt>=mb&&(Ee=lt,We=tt)}const Ze=Ee!==null&&[...De.entries()].some(([Ue,lt])=>Ue!==Ee&&lt>=et-.1),at=se.some(Ue=>de>=Ue.x0&&de<=Ue.x1&&Te>=Ue.y0&&Te<=Ue.y1);o.push({value:Ee,valueRead:Ee!==null,center:[Math.round((q+te)/2),Math.round((Q+le)/2)],boundingBox:{x:Math.trunc(q),y:Math.trunc(Q),width:Math.trunc(te-q),height:Math.trunc(le-Q)},confidence:Math.round(We*1e4)/1e4,excluded:at,photoIndex:$-1,...Ze?{suspect:!0,suspectReason:"orientation-ambiguous"}:{}})}i()}_>0?n.push({code:"OVERLAPPING_OBJECTS",message:`${t}: ${_} banner(s) near a wonder were excluded as tucked/consumed (estimated footprint — the server uses the real card box); verify the per-colour counts.`}):y>0&&h.length===0&&n.push({code:"OVERLAPPING_OBJECTS",message:`${t}: no wonder was located on this photo, so a card tucked under a wonder may still be counted — verify the per-colour counts.`});const x=s.guild??0;x!==c.length?n.push({code:"INCONSISTENT_STATE",message:`${t}: ${x} purple banner(s) counted but ${c.length} guild(s) identified — reconcile in the review (stacked guilds or a missed identification).`}):c.length>0&&n.push({code:"LOW_CONFIDENCE",message:`${t}: guild(s) identified by their card art: `+c.map(E=>E.id).join(", ")+" — confirm in the review."});const T=h.filter(E=>E.boundingBox.width===0);T.length>0?n.push({code:"LOW_CONFIDENCE",message:`${t}: wonder(s) identified by name but NOT registered against their reference (${T.map(E=>E.name).join(", ")}) — their BUILT flag is a suggestion: unselect any that was not built.`}):h.length>0&&n.push({code:"LOW_CONFIDENCE",message:`${t}: ${h.length} wonder(s) registered — the BUILT flags were measured (card protruding underneath); confirm in the review.`}),g>0&&n.push({code:"UNRECOGNIZED_OBJECT",message:`${t}: ${g} token disc(s) found but not identified — pick them in the review below.`}),l.length>0&&n.push({code:"LOW_CONFIDENCE",message:`${t}: progress token(s) identified on-device: `+l.map(E=>E.id).join(", ")+" — confirm in the review."}),u.length>0&&n.push({code:"LOW_CONFIDENCE",message:`${t}: coins read as ${m} from ${u.length} tile(s) by their metal COLOUR (the embossed-digit reader is server-only) — confirm the total.`});const S=o.filter(E=>!E.excluded),I=S.filter(E=>E.valueRead);return{...A1(),wonders:h,guilds:b1(c,h),progressTokens:l,laurels:o,cardVictoryPoints:{value:I.reduce((E,M)=>E+(M.value??0),0),laurelsKept:S.length,laurelsUnread:S.length-I.length,complete:S.length===I.length},cardCounts:{byFamily:s,source:y>0?"yolo":"none",tuckedExcluded:_,...p.length>0?{suspects:p}:{}},coins:{total:m,confidence:u.length>0?.5:0,source:u.length>0?"local-colour":"none",coins:u}}}const xt=1280,R1=.3,$s=9;let xs=null;function z1(){return xs===null&&(xs=(async()=>{try{return(await fetch(`${Ne}pawn_ends.onnx`,{method:"HEAD"})).ok?await nt.create(`${Ne}pawn_ends.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),xs}function O1(e){const t=xt/Math.max(e.width,e.height),n=Math.round(e.width*t),r=Math.round(e.height*t),i=new OffscreenCanvas(e.width,e.height);i.getContext("2d",{willReadFrequently:!0}).putImageData(new ImageData(new Uint8ClampedArray(e.data),e.width,e.height),0,0);const o=new OffscreenCanvas(xt,xt).getContext("2d",{willReadFrequently:!0});o.fillStyle="rgb(114,114,114)",o.fillRect(0,0,xt,xt),o.drawImage(i,0,0,e.width,e.height,0,0,n,r);const{data:u}=o.getImageData(0,0,xt,xt),l=xt*xt,h=new Float32Array(3*l);for(let c=0;c<l;c+=1)h[c]=u[c*4]/255,h[l+c]=u[c*4+1]/255,h[2*l+c]=u[c*4+2]/255;return{tensor:h,r:t}}async function N1(e,t){const{tensor:n,r}=O1(t),a=(await e.run({[e.inputNames[0]]:new Le("float32",n,[1,3,xt,xt])}))[e.outputNames[0]].data,s=new Map;for(let o=0;o+5<a.length;o+=6){const u=a[o+4];if(u<R1)continue;const l=Math.round(a[o+5]),h=s.get(l);if(h===void 0||u>h.conf){const c=(a[o]+a[o+2])/2/r,p=(a[o+1]+a[o+3])/2/r;s.set(l,{conf:u,cx:c,cy:p})}}return s}async function B1(e,t){let n=null;for(let g=0;g<4;g+=1){const y=g===0?t:Yt(t,g),_=await N1(e,y);if(_.has(0)&&_.has(1)&&_.has(2)){const $=_.get(0).conf+_.get(1).conf+_.get(2).conf;(n===null||$>n.score)&&(n={score:$,det:_})}}if(n===null)return null;const r=n.det.get(0),i=n.det.get(1),a=n.det.get(2),s=a.cx-i.cx,o=a.cy-i.cy,u=(i.cx+a.cx)/2,l=(i.cy+a.cy)/2,h=s*s+o*o;if(h<=0)return null;const c=((r.cx-u)*s+(r.cy-l)*o)/h*(2*$s),p=Math.min($s,Math.max(-$s,Math.round(c))),m=Math.min(r.conf,i.conf,a.conf);return{position:p,confidence:Math.round(m*1e4)/1e4}}async function P1(e,t){const n=[{code:"LOW_CONFIDENCE",message:"On-device mode: card counts and laurel/token/coin COUNTS are detected locally; laurel values, wonders, guilds, token ids and coin totals are entered in the review (those recognition stages are not ported to the browser yet)."}],r={left:null,right:null},i=e.left.length+e.right.length+(e.both!==void 0?2:0);let a=0;const s=(l,h=0)=>{t(l,i>0?Math.min(.99,(a+h)/i):void 0)},o=()=>{a+=1};for(const l of["left","right"]){const h=e[l];h.length>0&&(r[l]=await bs(h,l,n,s,o))}e.both!==void 0&&(r.left=await bs([e.both],"left",n,s,o,"player"),r.right=await bs([e.both],"right",n,s,o,"opponent"));let u={conflictPawnPosition:0,found:!1,confidence:0};if(e.board!==void 0){try{const l=await _s(e.board),h=await z1();if(h!==null){const c=await B1(h,l);c!==null&&(u={conflictPawnPosition:c.position,found:!0,confidence:c.confidence})}}catch(l){console.warn("[pawn] on-device read failed:",l)}u.found||n.push({code:"MILITARY_PAWN_NOT_FOUND",message:"On-device mode could not read the conflict pawn — set its position below."})}return{imageId:e.imageId,players:r,militaryTrack:u,outcome:{type:"civilian"},confidence:.5,warnings:n}}self.onmessage=e=>{const{id:t,kind:n}=e.data,r=(i,a)=>{self.postMessage({id:t,progress:i,...a!==void 0?{fraction:a}:{}})};(async()=>{try{n==="recognize"&&r("starting the on-device engine…",0);const i=n==="classify"?await C1(e.data.file):await P1(e.data.payload,r);self.postMessage({id:t,ok:!0,result:i})}catch(i){self.postMessage({id:t,ok:!1,error:String(i)})}})()}})();
