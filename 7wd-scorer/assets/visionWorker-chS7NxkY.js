...
`,Lc=e=>`quick_gelu_impl(${e})`,Fc=(e,t)=>{let n=it(e.inputs[0].dataType);e.compute(Ae(e.inputs[0],"QuickGelu",Lc,Uc(n,t.alpha),t.cacheKey,e.inputs[0].dataType))}}),Gc,Wc,qc,sy=ee(()=>{_e(),xe(),So(),Gc=e=>{if(e[0].dims.length!==3)throw new Error("input should have 3 dimensions");if(![2560,5120,10240].includes(e[0].dims[2]))throw new Error("hidden state should be 2560, 5120 or 10240");if(e[1].dims.length!==1)throw new Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw new Error("last dimension of input and bias are not the same")},Wc=e=>{let t=e[0].dims.slice();t[2]=t[2]/2;let n=Y("input",e[0].dataType,e[0].dims,4),r=Y("bias",e[0].dataType,[e[0].dims[2]],4),i=de("output",e[0].dataType,t,4),o=q.size(t)/4,a=Ze(e[0].dataType);return{name:"BiasSplitGelu",getRunData:()=>({outputs:[{dims:t,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(o/64)}}),getShaderSource:s=>`
  ...
  ${s.mainStart()}
    ...
    fn pow_vector_custom(a : vec4<${t}>, b : vec4<${t}>) -> vec4<${t}> {
      ...
      `)},Qc=e=>{kt(e,"Sub",(t,n)=>`${t}-${n}`)},Jc=e=>{kt(e,"Greater",{scalar:(t,n)=>`u32(${t}>${n})`,vector:(t,n)=>`vec4<u32>(${t}>${n})`},void 0,void 0,9)},ed=e=>{kt(e,"Less",{scalar:(t,n)=>`u32(${t}<${n})`,vector:(t,n)=>`vec4<u32>(${t}<${n})`},void 0,void 0,9)},td=e=>{kt(e,"GreaterOrEqual",{scalar:(t,n)=>`u32(${t}>=${n})`,vector:(t,n)=>`vec4<u32>(${t}>=${n})`},void 0,void 0,9)},nd=e=>{kt(e,"LessOrEqual",{scalar:(t,n)=>`u32(${t}<=${n})`,vector:(t,n)=>`vec4<u32>(${t}<=${n})`},void 0,void 0,9)}}),rd,id,od,ad,sd,ud,ly=ee(()=>{we(),_e(),Ke(),xe(),rd=(e,t)=>{if(!e||e.length<1)throw new Error("too few inputs");let n=0,r=e[n],i=r.dataType,o=r.dims.length;e.forEach((a,s)=>{if(s!==n){if(a.dataType!==i)throw new Error("input tensors should be one type");if(a.dims.length!==o)throw new Error("input tensors should have the same shape");a.dims.forEach((u,l)=>{if(l!==t&&u!==r.dims[l])throw new Error("non concat dimensions must match")})}})},id=(e,t)=>`
...
`)};
    ...
    ${m}`},h=[{type:12,data:u},...fe(t,r)];return{name:"Expand",shaderCache:{hint:`${r.length};${a}${s}`,inputDependencies:["rank"]},getShaderSource:l,getRunData:()=>({outputs:[{dims:r,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(u/64)},programUniforms:h})}},eh=e=>{Zd(e.inputs),e.compute(Jd(e.inputs),{inputs:[0]})}}),th,nh,xy=ee(()=>{we(),_e(),xe(),So(),th=e=>{let t=e[0].dataType,n=q.size(e[0].dims),r=q.size(e[1].dims),i=r%4===0,o=a=>{let s=Y("x",t,[1],4),u=Y("bias",t,[1],4),l=de("y",t,[1],4),h=[{name:"output_vec_size",type:"u32"},{name:"bias_size",type:"u32"}],c=f=>`
    ...
    }`};return{name:"FastGeluWithBias",shaderCache:{hint:`${i}`,inputDependencies:["type","type"]},getShaderSource:o,getRunData:a=>({outputs:[{dims:a[0].dims,dataType:a[0].dataType}],programUniforms:[{type:12,data:Math.ceil(n/4)},{type:12,data:r}],dispatchGroup:{x:Math.ceil(n/Wn/4)}})}},nh=e=>{e.inputs.length<2||q.size(e.inputs[1].dims)===0?Bc(e):e.compute(th(e.inputs))}}),rh,ih,oh,ah,$y=ee(()=>{we(),_e(),Ke(),xe(),rh=e=>{if(!e||e.length!==2)throw new Error("Gather requires 2 inputs.")},ih=(e,t)=>{let n=e[0].dims,r=e[1].dims,i=n.length,o=q.normalizeAxis(t.axis,i),a=n.slice(0);a.splice(o,1,...r);let s=n[o],u=e[0].dataType===9?4:1,l=Math.ceil(q.size(a)/u),h=[{type:12,data:l},{type:6,data:s},{type:12,data:o},...fe(e[0].dims,e[1].dims,a)],c=p=>{let f=Y("data",e[0].dataType,e[0].dims.length,u),m=Y("inputIndices",e[1].dataType,e[1].dims.length),y=de("output",e[0].dataType,a.length,u),w=x=>{let M=r.length,v=`var indicesIndices${x}  = ${m.type.indices}(0);`;for(let I=0;I<M;I++)v+=`${M>1?`indicesIndices${x}[${I}]`:`indicesIndices${x}`} = ${a.length>1?`outputIndices${x}[uniforms.axis + ${I}]`:`outputIndices${x}`};`;v+=`
...
`)}
  ...
  ${I.mainStart()}
    ...
    ${x?"inv_std_output[global_idx] = inv_std_dev":""};
        ...
        }`};return{name:"BlockwiseMatMulNBits32",shaderCache:{hint:`${t.blockSize};${c};${p};${w};${y}`,inputDependencies:Array(e.length).fill("rank")},getRunData:()=>({outputs:[{dims:f,dataType:h}],dispatchGroup:{x:I},programUniforms:T}),getShaderSource:N}},up=(e,t)=>{op(e.inputs,t),t.blockSize===32&&e.adapterInfo.isVendor("intel")&&e.adapterInfo.isArchitecture("gen-12lp")?e.compute(sp(e.inputs,t)):e.compute(ap(e.inputs,t))},lp=e=>Re(e)}),cp,dp,hp,pp,fp,mp,gp,yp,wp,Oy=ee(()=>{we(),_e(),xe(),cp=e=>{if(!e||e.length<1)throw new Error("Too few inputs");if(e[0].dataType!==1&&e[0].dataType!==10)throw new Error("Input type must be float or float16.");if(e.length>=2){let t=e[0].dims.length*2===e[1].dims[0];if(e.length===4&&(t=e[3].dims[0]*2===e[1].dims[0]),!t)throw new Error("The pads should be a 1D tensor of shape [2 * input_rank] or [2 * num_axes].")}},dp=(e,t,n)=>{let r="";for(let i=t-1;i>=0;--i)r+=`
  ...
  for (var i = 0u; i < uniforms.num_updates_elements; i++) {
    ...
    ${Bp(t.reduction,"output[data_offset + i]","value",m.type.value)}
      ...
      }`};return{name:"ScatterND",shaderCache:{hint:`${t.cacheKey}_${t.reduction}`,inputDependencies:["rank","rank"]},getRunData:()=>({outputs:[{dims:i,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(a/64)},programUniforms:l}),getShaderSource:h}},Dp=e=>Re({reduction:e.reduction}),Up=(e,t)=>{e.compute(Pp(e.inputs,t),{inputs:[e.inputs[1],e.inputs[2]],outputs:[]})}}),Lp,Fp,Gp,ta,Wp,qp,Vp,Hp,jp,Kp,Yp,Xp,na,Zp,Qp,Jp,ef,tf,nf,rf,Dy=ee(()=>{we(),_e(),Ke(),xe(),Lp=(e,t)=>{if(e.every(n=>n>0||(()=>{throw new Error("Resize requires scales input values to be positive")})),e.length>0){if(t.mode==="linear"){if(!(e.length===2||e.length===3||e.length===4&&e[0]===1&&e[1]===1||e.length===4&&e[0]===1&&e[3]===1||e.length===5&&e[0]===1&&e[1]===1))throw new Error(`For linear mode, Resize requires scales to be 2D, 3D, 4D with either two outermost or one innermost and
...
`}
    ...
    }`;return{name:"Tile",shaderCache:{hint:`${r}`,inputDependencies:["rank"]},getRunData:()=>({outputs:[{dims:i,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(o/64)},programUniforms:[{type:12,data:o},...fe(e[0].dims,i)]}),getShaderSource:l}},xf=e=>{wf(e.inputs),e.compute(bf(e.inputs),{inputs:[0]})}}),$f,vf,Sf,Wy=ee(()=>{we(),_e(),xe(),$f=(e,t,n,r,i)=>{let o=de("output_data",i,n.length,4),a=Y("a_data",t[1].dataType,t[1].dims.length,4),s=Y("b_data",t[2].dataType,t[2].dims.length,4),u=Y("c_data",t[0].dataType,t[0].dims.length,4),l,h=(c,p,f)=>`select(${p}, ${c}, ${f})`;if(!r)l=o.setByOffset("global_idx",h(a.getByOffset("global_idx"),s.getByOffset("global_idx"),u.getByOffset("global_idx")));else{let c=(p,f,m="")=>{let y=`a_data[index_a${f}][component_a${f}]`,w=`b_data[index_b${f}][component_b${f}]`,b=`bool(c_data[index_c${f}] & (0xffu << (component_c${f} * 8)))`;return`
 ...
 * limitations under the License.
 * =============================================================================
 */function ut(e){const t=Math.floor(e);return e-t===.5?t%2===0?t:t+1:Math.round(e)}function Hn(e){if(e.length===0)return Number.NaN;const t=[...e].sort((r,i)=>r-i),n=Math.floor(t.length/2);return t.length%2===1?t[n]:(t[n-1]+t[n])/2}function Zf(e,t){if(e.length===0)return Number.NaN;const n=[...e].sort((a,s)=>a-s),r=t/100*(n.length-1),i=Math.floor(r),o=Math.ceil(r);return i===o?n[i]:n[i]*(o-r)+n[o]*(r-i)}const Zy=114;function Qy(e,t,n,r=1){const i=Math.min(n*r/e,n*r/t),o=Math.round(e*i),a=Math.round(t*i);return{scale:i,padX:Math.floor((n-o)/2),padY:Math.floor((n-a)/2),resizedWidth:o,resizedHeight:a}}function wa(e,t,n){const{width:r,height:i,channels:o,data:a}=e,s=new Uint8Array(t*n*3),u=r/t,l=i/n;for(let h=0;h<n;h++){const c=(h+.5)*l-.5,p=Math.max(0,Math.min(i-1,Math.floor(c))),f=Math.min(i-1,p+1),m=Math.max(0,Math.min(1,c-p));for(let y=0;y<t;y++){const w=(y+.5)*u-.5,b=Math.max(0,Math.min(r-1,Math.floor(w))),x=Math.min(r-1,b+1),M=Math.max(0,Math.min(1,w-b)),v=(p*r+b)*o,I=(p*r+x)*o,T=(f*r+b)*o,k=(f*r+x)*o,S=(h*t+y)*3;for(let A=0;A<3;A++){const N=a[v+A]*(1-M)+a[I+A]*M,U=a[T+A]*(1-M)+a[k+A]*M;s[S+A]=Math.min(255,Math.max(0,Math.round(N*(1-m)+U*m)))}}}return s}function jn(e,t,n){const{width:r,height:i,channels:o,data:a}=e,s=new Uint8Array(t*n*3),u=r/t,l=i/n;for(let h=0;h<n;h++){const c=h*l,p=Math.min((h+1)*l,i);for(let f=0;f<t;f++){const m=f*u,y=Math.min((f+1)*u,r);let w=0,b=0,x=0,M=0;for(let I=Math.floor(c);I<p;I++){const T=Math.min(I+1,p)-Math.max(I,c);if(!(T<=0))for(let k=Math.floor(m);k<y;k++){const S=Math.min(k+1,y)-Math.max(k,m);if(S<=0)continue;const A=S*T,N=(I*r+k)*o;w+=a[N]*A,b+=a[N+1]*A,x+=a[N+2]*A,M+=A}}const v=(h*t+f)*3;s[v]=Math.min(255,Math.max(0,ut(w/M))),s[v+1]=Math.min(255,Math.max(0,ut(b/M))),s[v+2]=Math.min(255,Math.max(0,ut(x/M)))}}return s}function Qf(e){const n=((-.75*(e+1)- -3.75)*(e+1)+-6)*(e+1)- -3,r=((-.75+2)*e-(-.75+3))*e*e+1,i=((-.75+2)*(1-e)-(-.75+3))*(1-e)*(1-e)+1;return[n,r,i,1-n-r-i]}function _a(e,t,n){const{width:r,height:i,channels:o,data:a}=e,s=new Uint8Array(t*n*3),u=r/t,l=i/n,h=p=>Math.max(0,Math.min(r-1,p)),c=p=>Math.max(0,Math.min(i-1,p));for(let p=0;p<n;p++){const f=(p+.5)*l-.5,m=Math.floor(f),y=Qf(f-m);for(let w=0;w<t;w++){const b=(w+.5)*u-.5,x=Math.floor(b),M=Qf(b-x),v=(p*t+w)*3;for(let I=0;I<3;I++){let T=0;for(let k=0;k<4;k++){const S=c(m-1+k)*r;let A=0;for(let N=0;N<4;N++)A+=M[N]*a[(S+h(x-1+N))*o+I];T+=y[k]*A}s[v+I]=Math.min(255,Math.max(0,Math.round(T)))}}}return s}function ba(e,t,n=1){const r=Qy(e.width,e.height,t,n),i=wa(e,r.resizedWidth,r.resizedHeight),o=t*t,a=new Float32Array(3*o).fill(Zy/255);for(let s=0;s<r.resizedHeight;s++){const u=(s+r.padY)*t+r.padX,l=s*r.resizedWidth;for(let h=0;h<r.resizedWidth;h++){const c=(l+h)*3,p=u+h;a[p]=i[c]/255,a[o+p]=i[c+1]/255,a[2*o+p]=i[c+2]/255}}return{tensor:a,params:r}}function xa(e,t,n,r){const i=[],o=Math.floor(e.length/6);for(let a=0;a<o;a++){const s=e[a*6],u=e[a*6+1],l=e[a*6+2],h=e[a*6+3],c=e[a*6+4],p=e[a*6+5];if(c<n)continue;const f=Math.round(p);if(f<0||f>=r)continue;const m=(s-t.padX)/t.scale,y=(u-t.padY)/t.scale,w=(l-t.padX)/t.scale,b=(h-t.padY)/t.scale;i.push({classIndex:f,confidence:c,box:[Math.trunc(m),Math.trunc(y),Math.trunc(w-m),Math.trunc(b-y)],boxFloat:[m,y,w-m,b-y]})}return i}const wr=.8,Jf=.65,Jy=110,ew=1280;function tw(e,t,n){if(n==null)return wr;if(n.length===0)return Jf;const r=Math.max(e,t);if(!(r>0))return wr;const i=ew/r,o=n.filter(u=>Array.isArray(u.box)||u.box!==void 0).map(u=>Math.sqrt(Number(u.box[2])**2+Number(u.box[3])**2)*i).filter(u=>Number.isFinite(u)).sort((u,l)=>u-l);if(o.length===0)return wr;const a=o.length;return(a%2===1?o[(a-1)/2]:(o[a/2-1]+o[a/2])/2)>=Jy?Jf:wr}const em=.25,tm=.6;function nw(e,t,n){const r=Math.trunc(Number(n[0])),i=Math.trunc(Number(n[1])),o=Math.trunc(Number(n[2])),a=Math.trunc(Number(n[3]));if(![r,i,o,a].every(b=>Number.isFinite(b)))return null;const s=o-r,u=a-i;if(s<=0||u<=0)return null;const l=Math.trunc(s*(s>=u?em:tm)),h=Math.trunc(u*(s>=u?tm:em)),c=Math.max(0,r-l),p=Math.max(0,i-h),f=Math.min(Math.trunc(e),o+l),m=Math.min(Math.trunc(t),a+h),y=f-c,w=m-p;return y<=0||w<=0?null:{x:c,y:p,width:y,height:w}}const rw=3,iw=.15,ow=.6;function $a(e,t){return Math.hypot(Number(e[0])-Number(t[0]),Number(e[1])-Number(t[1]))}function aw(e){const t=e.filter(i=>i&&Number.isFinite(Number(i[0]))&&Number.isFinite(Number(i[1])));if(t.length===0)return null;let n=0,r=0;for(const i of t)n+=Number(i[0]),r+=Number(i[1]);return[n/t.length,r/t.length]}function sw(e,t,n){try{if(n==null)return null;const r=Math.trunc(Number(n));if(!Number.isFinite(r)||r===0||!e||e.length<2)return null;const i=[Number(e[0][0]),Number(e[0][1])],o=[Number(e[1][0]),Number(e[1][1])];if(![...i,...o].every(v=>Number.isFinite(v)))return null;const a=$a(i,o);if(!(a>0))return null;const s=[];for(const v of t??[]){const I=Math.trunc(Number(v.n));if(!Number.isFinite(I)||I<rw)continue;const T=aw(v.poly);T!==null&&s.push({owner:v.owner,c:T,n:I,d0:0,d1:0,ecart:0})}if(s.length<2)return null;s.sort((v,I)=>I.n-v.n);const u=s.slice(0,2);let l=!1;s.length>2&&u[1].n>0&&(l=s[2].n/u[1].n>ow);for(const v of u)v.d0=$a(v.c,i),v.d1=$a(v.c,o),v.ecart=Math.abs(v.d0-v.d1);const h=[...u].sort((v,I)=>I.ecart-v.ecart),c=h[0],p=h[1],f=c.d0<c.d1?0:1,m=r>0?1:0,y=f===m?c:p,w=f===m?p:c,b=f===1?c.owner:p.owner,x=f===1?p.owner:c.owner,M=c.ecart/a<iw;return{favoredOwner:w.owner,threatenedOwner:y.owner,ownerAtEnd0:x,ownerAtEnd1:b,distance:Math.abs(r),ambiguous:!!(M||l)}}catch{return null}}function uw(e){if(!e)return null;const t=e.ownerAtEnd1,n=e.ownerAtEnd0;return!t||!n||t===n?null:{left:n,right:t}}const lw=.6;function nm(e,t,n){const r=[],i=Math.floor(e.length/6);for(let o=0;o<i;o++){if(e[o*6+4]<n)continue;const s=(e[o*6]-t.padX)/t.scale,u=(e[o*6+1]-t.padY)/t.scale,l=(e[o*6+2]-t.padX)/t.scale,h=(e[o*6+3]-t.padY)/t.scale,c=ut((s+l)/2),p=ut((u+h)/2),f=ut((l-s+(h-u))/4);f>=1&&r.push({cx:c,cy:p,r:f})}return r}function cw(e){const t=[];for(const n of[...e].sort((r,i)=>r.r-i.r)){const r=(lw*n.r)**2;t.every(i=>(n.cx-i.cx)**2+(n.cy-i.cy)**2>r)&&t.push(n)}return t}function dw(e){if(e.length===0)return[];const t=Math.max(1,Math.trunc(Hn(e.map(n=>n.r))*1.5));return[...e].sort((n,r)=>{const i=Math.floor(n.cy/t),o=Math.floor(r.cy/t);return i!==o?i-o:n.cx-r.cx})}function rm(e,t,n){const r=nm(e,t,n);return r.length===0?[]:dw(cw(r))}function hw(e,t,n){return nm(e,t,n)}function _r(e,t,n){const r=[],i=Math.floor(e.length/6);for(let o=0;o<i;o++)e[o*6+4]<n||r.push([(e[o*6]-t.padX)/t.scale,(e[o*6+1]-t.padY)/t.scale,(e[o*6+2]-t.padX)/t.scale,(e[o*6+3]-t.padY)/t.scale]);return r}const pw=.5,fw=.7,mw=.55;function va(e){const t=e.map(([n,r,i,o])=>Math.min(i-n,o-r)).sort((n,r)=>n-r);return t[Math.floor(t.length/2)]||1}function im(e){if(e.length===0)return[];const t=(pw*va(e))**2,n=[];for(const i of e){const o=(i[0]+i[2])/2,a=(i[1]+i[3])/2,s=n.find(u=>(u.cx-o)**2+(u.cy-a)**2<=t);if(s===void 0)n.push({cx:o,cy:a,boxes:[i]});else{s.boxes.push(i);const u=s.boxes.length;s.cx=(s.cx*(u-1)+o)/u,s.cy=(s.cy*(u-1)+a)/u}}let r=n.map(({boxes:i})=>[Math.trunc(Hn(i.map(o=>o[0]))),Math.trunc(Hn(i.map(o=>o[1]))),Math.trunc(Hn(i.map(o=>o[2]))),Math.trunc(Hn(i.map(o=>o[3])))]);if(r.length>=2){const i=va(r),o=r.map(()=>!0);for(let a=0;a<r.length;a++)if(o[a])for(let s=a+1;s<r.length;s++){if(!o[s])continue;const u=r[a],l=r[s],h=Math.max(0,Math.min(u[2],l[2])-Math.max(u[0],l[0])),c=Math.max(0,Math.min(u[3],l[3])-Math.max(u[1],l[1])),p=h*c,f=(u[2]-u[0])*(u[3]-u[1]),m=(l[2]-l[0])*(l[3]-l[1]);if(p>=fw*Math.min(f,m)){const y=Math.abs(Math.min(u[2]-u[0],u[3]-u[1])-i),w=Math.abs(Math.min(l[2]-l[0],l[3]-l[1])-i);if(o[y<=w?s:a]=!1,!o[a])break}}r=r.filter((a,s)=>o[s])}if(r.length>=3){const i=va(r);r=r.filter(([o,a,s,u])=>Math.min(s-o,u-a)>=mw*i)}return r}const om=["brown","grey","blue","green","yellow","red","purple"],gw={brown:"raw",grey:"manufactured",blue:"civilian",green:"scientific",yellow:"commercial",red:"military",purple:"guild"},yw=.7;function am(e){const t=e.map((i,o)=>o).sort((i,o)=>e[o].confidence-e[i].confidence),n=new Set,r=[];for(const i of t){const o=e[i],[a,s,u,l]=o.box;let h=!1;for(const c of r){const p=e[c];if(p.family!==o.family)continue;const[f,m,y,w]=p.box,b=Math.max(0,Math.min(a+u,f+y)-Math.max(a,f)),x=Math.max(0,Math.min(s+l,m+w)-Math.max(s,m)),M=Math.max(1,Math.min(u*l,y*w));if(b*x>=yw*M){h=!0;break}}h?n.add(i):r.push(i)}return e.filter((i,o)=>!n.has(o))}function ni(e,t,n){const r=xa(e,t,n,om.length).map(i=>{const o=om[i.classIndex];return{color:o,family:gw[o],box:i.box,confidence:i.confidence}});return am(r)}const ww=8,_w=.8,sm=1.25;function bw(e){if(e.length<ww)return[];const t=[],n=[];for(const a of e){const[,,s,u]=a.box;s>u*sm?t.push(a):u>s*sm&&n.push(a)}const[r,i,o]=t.length>=n.length?[t,n,"vertical"]:[n,t,"horizontal"];return r.length<_w*e.length||i.length===0?[]:i.map(a=>({family:a.family,color:a.color,box:[...a.box],reason:`${a.color} banner sits ${o} while ${r.length}/${e.length} of the tableau faces the other way — probably a stray card poking into the frame`}))}const xw=2.25,um=8;function $w(e){if(e.length<um)return[];const t=e.map(c=>[c.box[0]+c.box[2]/2,c.box[1]+c.box[3]/2]),n=e.map(c=>Math.hypot(c.box[2],c.box[3])).sort((c,p)=>c-p),r=xw*n[Math.floor(n.length/2)],i=r*r,o=e.map((c,p)=>p),a=c=>{for(;o[c]!==c;)o[c]=o[o[c]],c=o[c];return c};for(let c=0;c<e.length;c++)for(let p=c+1;p<e.length;p++){const f=t[c][0]-t[p][0],m=t[c][1]-t[p][1];f*f+m*m<=i&&(o[a(c)]=a(p))}const s=new Map;for(let c=0;c<e.length;c++){const p=a(c);s.set(p,[...s.get(p)??[],c])}let u=[];for(const c of s.values())c.length>u.length&&(u=c);if(u.length<um||u.length===e.length)return[];const l=new Set(u),h=e.map((c,p)=>p).filter(c=>!l.has(c));return h.map(c=>({family:e[c].family,color:e[c].color,box:[...e[c].box],reason:`${e[c].color} banner sits in a detached group of ${h.length}, away from the ${u.length}-card tableau — probably the draw/discard pile, not this player's city`}))}const ot={banner:{onnx:"banner_yolo.onnx",input:1280,conf:.5},coin:{onnx:"coin_yolo.onnx",input:1280,conf:.25},laurel:{onnx:"laurel_yolo.onnx",input:1280,conf:.25},token:{onnx:"token_yolo.onnx",input:1280,conf:.4},wonder:{onnx:"wonder_yolo.onnx",input:1280,conf:.3}};function Ct(e,t,n){const r=Math.max(e,t,n),i=Math.min(e,t,n),o=r-i,a=r===0?0:Math.round(255*o/r);if(o===0)return{h:0,s:a,v:r};let s;return r===e?s=60*(t-n)/o:r===t?s=120+60*(n-e)/o:s=240+60*(e-t)/o,s<0&&(s+=360),{h:Math.round(s/2),s:a,v:r}}const vw=.42,Sw=22,Mw=43,Iw=120,Ew=1.5,Tw=.72,kw=110,lm=3;function br(e,t,n){const{width:r,height:i,channels:o,data:a}=e;if(r<4||i<4)return 0;const s=Math.floor(r/2),u=Math.floor(i/2),l=Math.trunc(Math.min(r,i)*vw);if(l<1)return 0;let h=0;for(let c=0;c<i;c++)for(let p=0;p<r;p++){if((p-s)**2+(c-u)**2>l*l)continue;const f=(c*r+p)*o,m=a[f],y=a[f+1],w=a[f+2];!t&&m>=250&&y>=250&&w>=250||(n(m,y,w),h+=1)}return h}function Cw(e){let t=0,n=0,r=0,i=br(e,!1,(o,a,s)=>{const u=Ct(o,a,s);t+=u.h,n+=u.s,r+=u.v});return i===0&&(i=br(e,!0,(o,a,s)=>{const u=Ct(o,a,s);t+=u.h,n+=u.s,r+=u.v})),i===0?null:{h:t/i,s:n/i,v:r/i}}function Aw(e){let t=0,n=0,r=br(e,!1,(o,a)=>{t+=o,n+=a});if(r===0&&(r=br(e,!0,(o,a)=>{t+=o,n+=a})),r===0)return null;const i=n/r;return i<=1e-6?null:t/r/i}function Rw(e){let t=0;const n=br(e,!0,(r,i,o)=>{t+=Ct(r,i,o).s});return n===0?null:t/n}function Ow(e){const t=Cw(e);if(t===null||t.s<=Sw)return 1;if(t.s>=Iw){const n=Aw(e);return n!==null&&n>=Ew?6:3}return t.s>=Mw?3:6}function Nw(e,t){const n=[...t];if(e.length!==3||t.length!==3||new Set(t).size===3&&t.every(a=>[1,3,6].includes(a)))return n;const r=e.map(a=>a.r).sort((a,s)=>a-s);if(r[0]<=0||!(r[1]>=r[0]*1.12&&r[2]>=r[1]*1.12))return n;const i=[0,1,2].sort((a,s)=>e[a].r-e[s].r),o=new Map([[i[0],1],[i[1],3],[i[2],6]]);return[0,1,2].map(a=>o.get(a))}function zw(e,t){const n=[...t];if(e.length<lm||t.length!==e.length)return n;const r=e.map(a=>Rw(a)),i=r.filter(a=>a!==null);if(i.length<lm)return n;const o=Hn(i);return o<=0||r.forEach((a,s)=>{a!==null&&n[s]!==1&&a<Tw*o&&a<kw&&(n[s]=1)}),n}function cm(e,t){const{cx:n,cy:r,r:i}=t,o=Math.max(0,n-i),a=Math.max(0,r-i),s=Math.min(e.width,n+i),u=Math.min(e.height,r+i),l=Math.max(0,s-o),h=Math.max(0,u-a),c=new Uint8Array(l*h*3);for(let p=0;p<h;p++)for(let f=0;f<l;f++){const m=(p*l+f)*3;if((f+o-n)**2+(p+a-r)**2<=i*i){const w=((p+a)*e.width+(f+o))*e.channels;c[m]=e.data[w],c[m+1]=e.data[w+1],c[m+2]=e.data[w+2]}else c[m]=255,c[m+1]=255,c[m+2]=255}return{width:l,height:h,channels:3,data:c}}function Bw(e,t){const n=t.map(o=>cm(e,o)),r=n.map(o=>Ow(o)),i=Nw(t,r);return zw(n,i)}function Pw(e){const{width:t,height:n,channels:r,data:i}=e,o=new Uint8Array(t*n);for(let a=0,s=0;a<o.length;a++,s+=r)o[a]=i[s]*4899+i[s+1]*9617+i[s+2]*1868+8192>>14;return{width:t,height:n,data:o}}function dm(e,t,n){const r=new Uint8Array(t*n),i=e.width/t,o=e.height/n;for(let a=0;a<n;a++){const s=a*o,u=Math.min((a+1)*o,e.height);for(let l=0;l<t;l++){const h=l*i,c=Math.min((l+1)*i,e.width);let p=0,f=0;for(let m=Math.floor(s);m<u;m++){const y=Math.min(m+1,u)-Math.max(m,s);if(!(y<=0))for(let w=Math.floor(h);w<c;w++){const b=Math.min(w+1,c)-Math.max(w,h);b<=0||(p+=e.data[m*e.width+w]*b*y,f+=b*y)}}r[a*t+l]=Math.min(255,Math.max(0,ut(p/f)))}}return{width:t,height:n,data:r}}function Dw(e){const t=new Array(256).fill(0);for(const u of e.data)t[u]+=1;const n=e.data.length;let r=0;for(;r<256&&t[r]===0;)r+=1;const i=new Uint8Array(n);if(r>=255||t[r]===n)return i.fill(r<256?r:0),{width:e.width,height:e.height,data:i};const o=255/(n-t[r]),a=new Uint8Array(256);let s=0;for(let u=r+1;u<256;u++)s+=t[u],a[u]=Math.min(255,Math.max(0,ut(s*o)));for(let u=0;u<n;u++)i[u]=a[e.data[u]];return{width:e.width,height:e.height,data:i}}function Uw(e){const{width:t,height:n,data:r}=e,i=new Uint8Array(t*n);for(let o=0;o<n;o++)for(let a=0;a<t;a++){let s=!0;for(let u=-1;u<=1&&s;u++)for(let l=-1;l<=1;l++){const h=a+l,c=o+u;if(!(h<0||h>=t||c<0||c>=n)&&r[c*t+h]===0){s=!1;break}}i[o*t+a]=s&&r[o*t+a]>0?255:0}return{width:t,height:n,data:i}}function Lw(e){const{width:t,height:n,data:r}=e,i=new Uint8Array(t*n);for(let o=0;o<n;o++)for(let a=0;a<t;a++){let s=!1;for(let u=-1;u<=1&&!s;u++)for(let l=-1;l<=1;l++){const h=a+l,c=o+u;if(h>=0&&h<t&&c>=0&&c<n&&r[c*t+h]>0){s=!0;break}}i[o*t+a]=s?255:0}return{width:t,height:n,data:i}}function Sa(e){const{width:t,height:n,data:r}=e,i=new Int32Array(t*n),o=[],a=new Int32Array(t*n);let s=1;for(let u=0;u<r.length;u++){if(r[u]===0||i[u]!==0)continue;let l=0,h=0;a[h++]=u,i[u]=s;let c=0,p=0,f=0;for(;l<h;){const m=a[l++],y=m%t,w=m/t|0;c+=1,p+=y,f+=w;for(let b=-1;b<=1;b++)for(let x=-1;x<=1;x++){if(x===0&&b===0)continue;const M=y+x,v=w+b;if(M<0||M>=t||v<0||v>=n)continue;const I=v*t+M;r[I]>0&&i[I]===0&&(i[I]=s,a[h++]=I)}}o[s]={area:c,centroidX:p/c,centroidY:f/c},s+=1}return{labels:i,stats:o}}function Fw(e,t,n){return hm(Float32Array.from(e.data),e.width,t,n)}function hm(e,t,n,r){const i=new Float32Array(t*t),o=t/2,a=-n*Math.PI/180,s=Math.cos(a),u=Math.sin(a);for(let l=0;l<t;l++)for(let h=0;h<t;h++){const c=h-o,p=l-o,f=s*c-u*p+o,m=u*c+s*p+o,y=Math.floor(f),w=Math.floor(m),b=f-y,x=m-w,M=(T,k)=>T>=0&&T<t&&k>=0&&k<t?e[k*t+T]:r,v=M(y,w)*(1-b)+M(y+1,w)*b,I=M(y,w+1)*(1-b)+M(y+1,w+1)*b;i[l*t+h]=v*(1-x)+I*x}return i}const Gw=.9,Ww=.34,qw=[.55,.6,.66,.72],Vw=22,Hw=88,jw=35,Kn=28,Ma=4,Kw=Array.from({length:15},(e,t)=>-21+t*3),pm=[-2,0,2],Yw=3,Xw=.3;function Zw(e){return e.templates.flatMap(({label:t,bits:n})=>{const r=Uint8Array.from(atob(n),i=>i.charCodeAt(0));return r.length!==e.size*e.size?[]:[{label:t,bits:Float32Array.from(r)}]})}function Qw(e){let t=e.width,n=-1,r=e.height,i=-1,o=0;for(let y=0;y<e.height;y++)for(let w=0;w<e.width;w++)e.data[y*e.width+w]>0&&(o+=1,t=Math.min(t,w),n=Math.max(n,w),r=Math.min(r,y),i=Math.max(i,y));if(o<8)return null;const a=n-t+1,s=i-r+1,u=Math.max(s,a),l=new Uint8Array(u*u),h=Math.floor((u-a)/2),c=Math.floor((u-s)/2);for(let y=0;y<s;y++)for(let w=0;w<a;w++)l[(y+c)*u+(w+h)]=e.data[(y+r)*e.width+(w+t)];const p=Kn-2*Ma,f=dm({width:u,height:u,data:l},p,p),m=new Float32Array(Kn*Kn);for(let y=0;y<p;y++)for(let w=0;w<p;w++)m[(y+Ma)*Kn+(w+Ma)]=f.data[y*p+w]>110?1:0;return m}function Jw(e,t){const{width:n,height:r,channels:i,data:o}=e,a=Math.floor(r/2),s=Math.floor(n/2),u=Math.trunc(Math.min(n,r)*Ww);if(u<4)return null;const l=a-u,h=s-u,c=2*u,p=2*u;if(c<6||p<6)return null;const f=new Int16Array(c*p),m=new Int16Array(c*p),y=new Int16Array(c*p),w=new Uint8Array(c*p),b=[],x=Math.min(c,p)/2;for(let W=0;W<c;W++)for(let z=0;z<p;z++){const R=((W+l)*n+(z+h))*i,{h:B,s:L,v:G}=Ct(o[R],o[R+1],o[R+2]),Z=W*p+z;f[Z]=B,m[Z]=L,y[Z]=G,Math.sqrt((z-p/2)**2+(W-c/2)**2)/x<=t&&(w[Z]=1,b.push(G))}if(b.length<16)return null;const M=Zf(b,55);let v=0,I=0,T=0;const k=W=>f[W]>=Vw&&f[W]<=Hw&&m[W]>=jw,S=W=>y[W]>=M&&m[W]<=95&&!k(W)&&w[W]===1;for(let W=0;W<c*p;W++)w[W]===1&&(T+=1,y[W]>=130&&!k(W)&&(v+=1),S(W)&&(I+=1));const A=v>.5*T&&I<.15*T,N=new Uint8Array(c*p);if(A){const W=Zf(b,45);for(let z=0;z<c*p;z++)N[z]=w[z]===1&&y[z]<=W?255:0}else for(let W=0;W<c*p;W++)N[W]=S(W)?255:0;const U={width:p,height:c,data:N},V=Uw(U);let F=Sa(V),O=F;if(F.stats.length<=1&&(F=Sa(U),O=F,F.stats.length<=1))return null;const H=Math.min(c,p)/2;let X=0,J=-1;for(let W=1;W<O.stats.length;W++){const z=O.stats[W];if(z===void 0)continue;const R=Math.hypot(z.centroidX-p/2,z.centroidY-c/2)/H,B=z.area*(1-.6*Math.min(R,1));B>J&&(J=B,X=W)}if(X===0)return null;const he=new Uint8Array(c*p);for(let W=0;W<c*p;W++)he[W]=O.labels[W]===X?255:0;return Qw(Lw({width:p,height:c,data:he}))}function e_(e,t,n,r,i,o){const a=Kn;let s=0,u=0;for(let l=0;l<a;l++){const h=l-o;if(!(h<0||h>=a))for(let c=0;c<a;c++){const p=c-i;if(p<0||p>=a)continue;const f=e[h*a+p];f!==0&&(u+=f,s+=f*n[l*a+c])}}return s/(u+r-s+1e-6)}function t_(e,t){const n=t.reduce((i,o)=>i+o,0);let r=-1;for(const i of Kw){const o=i===0?e:hm(e,Kn,i,0),a=o.reduce((s,u)=>s+u,0);for(const s of pm)for(const u of pm){const l=e_(o,a,t,n,s,u);l>r&&(r=l)}}return r}function n_(e,t){if(t.length===0||Math.min(e.width,e.height)<8)return[null,0];const n=[];for(const a of qw){const s=Jw(e,a);if(s!==null)for(const{label:u,bits:l}of t)n.push([t_(s,l),u])}if(n.length===0)return[null,0];if(n.sort((a,s)=>s[0]-a[0]),n[0][0]<Xw)return[null,0];const r=new Map;for(const[a,s]of n.slice(0,Yw))r.set(s,(r.get(s)??0)+a);let i=0,o=-1;for(const[a,s]of r)s>o&&(o=s,i=a);return[i,n[0][0]]}const r_=2560,i_=.3,o_=.5,a_=1.6,s_=3,u_=5;function l_(e){const t=Math.min(1,r_/Math.max(e.width,e.height)),n=Math.max(32,Math.round(e.width*t/32)*32),r=Math.max(32,Math.round(e.height*t/32)*32),i=n*r,o=new Float32Array(3*i),a=e.width/n,s=e.height/r;for(let u=0;u<r;u++){const l=(u+.5)*s-.5,h=Math.max(0,Math.min(e.height-1,Math.floor(l))),c=Math.min(e.height-1,h+1),p=Math.max(0,Math.min(1,l-h));for(let f=0;f<n;f++){const m=(f+.5)*a-.5,y=Math.max(0,Math.min(e.width-1,Math.floor(m))),w=Math.min(e.width-1,y+1),b=Math.max(0,Math.min(1,m-y));for(let x=0;x<3;x++){const M=2-x,v=(h*e.width+y)*e.channels+M,I=(h*e.width+w)*e.channels+M,T=(c*e.width+y)*e.channels+M,k=(c*e.width+w)*e.channels+M,S=e.data[v]*(1-b)+e.data[I]*b,A=e.data[T]*(1-b)+e.data[k]*b,N=S*(1-p)+A*p;o[x*i+u*n+f]=(N/255-.5)/.5}}}return{tensor:o,width:n,height:r}}function c_(e,t,n){const r=new Uint8Array(e.length);for(let i=0;i<n;i++){const o=i===n-1;for(let a=0;a<t;a++){const s=i*t+a;let u=e[s];if(a+1<t&&e[s+1]>u&&(u=e[s+1]),!o){const l=s+t;e[l]>u&&(u=e[l]),a+1<t&&e[l+1]>u&&(u=e[l+1])}r[s]=u}}return r}function d_(e){if(e.length<3)return e;const t=[...e].sort((o,a)=>o[0]-a[0]||o[1]-a[1]),n=(o,a,s)=>(a[0]-o[0])*(s[1]-o[1])-(a[1]-o[1])*(s[0]-o[0]),r=[];for(const o of t){for(;r.length>=2&&n(r[r.length-2],r[r.length-1],o)<=0;)r.pop();r.push(o)}const i=[];for(let o=t.length-1;o>=0;o--){const a=t[o];for(;i.length>=2&&n(i[i.length-2],i[i.length-1],a)<=0;)i.pop();i.push(a)}return r.pop(),i.pop(),r.concat(i)}function h_(e){if(e.length===1)return{cx:e[0][0],cy:e[0][1],w:0,h:0,angle:0};let t=null,n=1/0;for(let r=0;r<e.length;r++){const[i,o]=e[r],[a,s]=e[(r+1)%e.length],u=a-i,l=s-o,h=Math.hypot(u,l);if(h===0)continue;const c=u/h,p=l/h;let f=1/0,m=-1/0,y=1/0,w=-1/0;for(const[v,I]of e){const T=v*c+I*p,k=-v*p+I*c;T<f&&(f=T),T>m&&(m=T),k<y&&(y=k),k>w&&(w=k)}const b=m-f,x=w-y,M=b*x;if(M<n){n=M;const v=(f+m)/2,I=(y+w)/2;t={cx:v*c-I*p,cy:v*p+I*c,w:b,h:x,angle:Math.atan2(p,c)}}}return t}function p_(e,t,n,r){const i=Math.cos(r.angle),o=Math.sin(r.angle),a=r.w/2,s=r.h/2,u=Math.abs(a*i)+Math.abs(s*o),l=Math.abs(a*o)+Math.abs(s*i),h=Math.max(0,Math.floor(r.cx-u)),c=Math.min(t-1,Math.ceil(r.cx+u)),p=Math.max(0,Math.floor(r.cy-l)),f=Math.min(n-1,Math.ceil(r.cy+l));let m=0,y=0;for(let w=p;w<=f;w++)for(let b=h;b<=c;b++){const x=b-r.cx,M=w-r.cy,v=x*i+M*o,I=-x*o+M*i;Math.abs(v)<=a&&Math.abs(I)<=s&&(m+=e[w*t+b],y+=1)}return y===0?0:m/y}function f_(e){const t=Math.cos(e.angle),n=Math.sin(e.angle),r=e.w/2,i=e.h/2,a=[...[[e.cx+-r*t- -i*n,e.cy+-r*n+-i*t],[e.cx+r*t- -i*n,e.cy+r*n+-i*t],[e.cx+r*t-i*n,e.cy+r*n+i*t],[e.cx+-r*t-i*n,e.cy+-r*n+i*t]]].sort((y,w)=>y[0]-w[0]),[s,u,l,h]=a,[c,p]=s[1]<=u[1]?[s,u]:[u,s],[f,m]=l[1]<=h[1]?[l,h]:[h,l];return[[c[0],c[1]],[f[0],f[1]],[m[0],m[1]],[p[0],p[1]]]}function m_(e,t,n,r){const{width:i,height:o}=t;let a=new Uint8Array(i*o);for(let f=0;f<a.length;f++)a[f]=e[f]>i_?255:0;a=c_(a,i,o);const s={width:i,height:o,data:a},{labels:u}=Sa(s),l=new Map;for(let f=0;f<o;f++)for(let m=0;m<i;m++){const y=u[f*i+m];if(y===0)continue;let w=l.get(y);w===void 0&&(w=new Map,l.set(y,w));const b=w.get(f);b===void 0?w.set(f,[m,m]):(m<b[0]&&(b[0]=m),m>b[1]&&(b[1]=m))}const h=n/i,c=r/o,p=[];for(const[f,m]of l){const y=[];for(const[N,[U,V]]of m)y.push([U-.5,N-.5],[U-.5,N+.5],[V+.5,N-.5],[V+.5,N+.5]);const w=h_(d_(y));if(Math.min(w.w,w.h)<s_)continue;const b=p_(e,i,o,w);if(b<o_)continue;const x=w.w*w.h*a_/(2*(w.w+w.h)),M={...w,w:w.w+2*x,h:w.h+2*x};if(Math.min(M.w,M.h)<u_+2)continue;const I=f_(M).map(([N,U])=>[Math.min(n,Math.max(0,Math.round(N*h))),Math.min(r,Math.max(0,Math.round(U*c)))]),T=I.map(N=>N[0]),k=I.map(N=>N[1]),S=Math.min(...T),A=Math.min(...k);p.push({quad:I,x:S,y:A,width:Math.max(...T)-S,height:Math.max(...k)-A,score:b})}return p.sort((f,m)=>m.score-f.score)}function g_(e,t){const[n,r,i,o]=t,a=Math.max(1,Math.round(Math.max(Math.hypot(r[0]-n[0],r[1]-n[1]),Math.hypot(i[0]-o[0],i[1]-o[1])))),s=Math.max(1,Math.round(Math.max(Math.hypot(o[0]-n[0],o[1]-n[1]),Math.hypot(i[0]-r[0],i[1]-r[1])))),u=y_([[0,0],[a,0],[a,s],[0,s]],[n,r,i,o]),l=new Uint8Array(a*s*e.channels);for(let c=0;c<s;c++)for(let p=0;p<a;p++){const f=u[6]*p+u[7]*c+u[8],m=(u[0]*p+u[1]*c+u[2])/f,y=(u[3]*p+u[4]*c+u[5])/f,w=Math.floor(m),b=Math.floor(y),x=m-w,M=y-b,v=Math.max(0,Math.min(e.width-1,w)),I=Math.max(0,Math.min(e.width-1,w+1)),T=Math.max(0,Math.min(e.height-1,b)),k=Math.max(0,Math.min(e.height-1,b+1));for(let S=0;S<e.channels;S++){const A=e.data[(T*e.width+v)*e.channels+S],N=e.data[(T*e.width+I)*e.channels+S],U=e.data[(k*e.width+v)*e.channels+S],V=e.data[(k*e.width+I)*e.channels+S],F=A*(1-x)+N*x,O=U*(1-x)+V*x;l[(c*a+p)*e.channels+S]=Math.round(F*(1-M)+O*M)}}const h={width:a,height:s,channels:e.channels,data:l};return s/a>=1.5?jt(h,3):h}function y_(e,t){const n=[],r=[];for(let i=0;i<4;i++){const[o,a]=e[i],[s,u]=t[i];n.push([o,a,1,0,0,0,-s*o,-s*a]),r.push(s),n.push([0,0,0,o,a,1,-u*o,-u*a]),r.push(u)}for(let i=0;i<8;i++){let o=i;for(let s=i+1;s<8;s++)Math.abs(n[s][i])>Math.abs(n[o][i])&&(o=s);[n[i],n[o]]=[n[o],n[i]],[r[i],r[o]]=[r[o],r[i]];const a=n[i][i];for(let s=i;s<8;s++)n[i][s]/=a;r[i]/=a;for(let s=0;s<8;s++){if(s===i)continue;const u=n[s][i];if(u!==0){for(let l=i;l<8;l++)n[s][l]-=u*n[i][l];r[s]-=u*r[i]}}}return[r[0],r[1],r[2],r[3],r[4],r[5],r[6],r[7],1]}function jt(e,t){const n=(t%4+4)%4;if(n===0)return e;const{width:r,height:i,channels:o,data:a}=e,s=n%2===0?r:i,u=n%2===0?i:r,l=new Uint8Array(s*u*o);for(let h=0;h<i;h++)for(let c=0;c<r;c++){let p,f;n===1?(p=i-1-h,f=c):n===2?(p=r-1-c,f=i-1-h):(p=h,f=r-1-c);const m=(h*r+c)*o,y=(f*s+p)*o;for(let w=0;w<o;w++)l[y+w]=a[m+w]}return{width:s,height:u,channels:o,data:l}}const w_=.6;(()=>{const e=new Uint8Array(256);for(let t=0;t<256;t++)e[t]=Math.min(255,Math.round(Math.pow(t/255,w_)*255));return e})();const Kt=48,__=320;function b_(e){return["blank",...e.characters," "]}function x_(e,t,n){let r="";const i=[];for(let a=0;a<e.length;a++){const s=e[a];s!==0&&(a>0&&e[a-1]===s||(r+=n[s]??"",i.push(t[a])))}if(i.length===0)return["",0];const o=i.reduce((a,s)=>a+s,0)/i.length;return[r,o]}function $_(e,t){const n=Math.trunc(Kt*t),r=e.width/e.height,i=Math.ceil(Kt*r)>n?n:Math.ceil(Kt*r),o=new Float32Array(3*Kt*n),a=Kt*n,s=e.width/i,u=e.height/Kt;for(let l=0;l<Kt;l++){const h=(l+.5)*u-.5,c=Math.max(0,Math.min(e.height-1,Math.floor(h))),p=Math.min(e.height-1,c+1),f=Math.max(0,Math.min(1,h-c));for(let m=0;m<i;m++){const y=(m+.5)*s-.5,w=Math.max(0,Math.min(e.width-1,Math.floor(y))),b=Math.min(e.width-1,w+1),x=Math.max(0,Math.min(1,y-w));for(let M=0;M<3;M++){const v=2-M,I=(c*e.width+w)*e.channels+v,T=(c*e.width+b)*e.channels+v,k=(p*e.width+w)*e.channels+v,S=(p*e.width+b)*e.channels+v,A=e.data[I]*(1-x)+e.data[T]*x,N=e.data[k]*(1-x)+e.data[S]*x,U=A*(1-f)+N*f;o[M*a+l*n+m]=(U/255-.5)/.5}}}return{tensor:o,width:n}}const v_=62,S_=8,M_=5;function Ia(e){return e?e.normalize("NFKD").replace(new RegExp("\\p{M}","gu"),"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim():""}function I_(e,t){const n=e.length,r=t.length;if(n===0||r===0)return 0;let i=new Int32Array(r+1),o=new Int32Array(r+1);for(let a=1;a<=n;a++){for(let s=1;s<=r;s++)o[s]=e[a-1]===t[s-1]?i[s-1]+1:Math.max(i[s],o[s-1]);[i,o]=[o,i]}return i[r]}function ri(e,t){return e.length===0&&t.length===0?100:200*I_(e,t)/(e.length+t.length)}function fm(e,t){const n=r=>r.split(/\s+/).filter(Boolean).sort().join(" ");return ri(n(e),n(t))}function E_(e,t){const n=new Set(e.split(/\s+/).filter(Boolean)),r=new Set(t.split(/\s+/).filter(Boolean)),i=[...n].filter(h=>r.has(h)).sort(),o=[...n].filter(h=>!r.has(h)).sort(),a=[...r].filter(h=>!n.has(h)).sort(),s=i.join(" "),u=[s,o.join(" ")].filter(Boolean).join(" "),l=[s,a.join(" ")].filter(Boolean).join(" ");return s.length>0&&(o.length===0||a.length===0)?100:Math.max(ri(s,u),ri(s,l),ri(u,l))}function T_(e){const t=new Set,n=[];for(const r of e){const i=r.nameFr??r.name;for(const o of[Ia(i),Ia(r.name)])if(o)for(const a of[o,o.replace(/ /g,"")])a&&!t.has(a)&&(t.add(a),n.push({key:a,id:r.id,display:i,...r.kind!==void 0?{kind:r.kind}:{}}))}return n}function k_(e,t){const n=Ia(e);if(!n||t.length===0)return null;const i=T_(t).map(h=>({...h,score:E_(n,h.key)})).sort((h,c)=>c.score-h.score).slice(0,S_).filter(h=>h.score>=v_);if(i.length===0)return null;const o=i[0].score,a=i.filter(h=>o-h.score<=M_),s=[...new Set(n.split(/\s+/).filter(Boolean))].join(" ");let u=a[0],l=[fm(s,u.key),u.score];for(const h of a.slice(1)){const c=[fm(s,h.key),h.score];(c[0]>l[0]||c[0]===l[0]&&c[1]>l[1])&&(u=h,l=c)}return{id:u.id,name:u.display,...u.kind!==void 0?{kind:u.kind}:{},confidence:Math.round(u.score/100*1e4)/1e4}}const mm=5e3,Ea=.75,gm=15,C_=1.25,A_=2.4,R_=.003,O_=.85,N_=4,Ta=2600,ka=2,Ca=.3,ym=.1,wm=.012,z_=22,_m=.5,ii=.12;function tt(e,t){const n=new e.Mat(t.height,t.width,e.CV_8UC3),r=n.data,i=t.channels;for(let o=0,a=t.width*t.height;o<a;o++)r[o*3]=t.data[o*i],r[o*3+1]=t.data[o*i+1],r[o*3+2]=t.data[o*i+2];return n}function B_(e,t,n,r){const i=r.map(te=>te[0]),o=r.map(te=>te[1]),a=i.reduce((te,ye)=>te+ye,0)/i.length,s=o.reduce((te,ye)=>te+ye,0)/o.length,u=Math.max(Math.max(...i)-Math.min(...i),Math.max(...o)-Math.min(...o));if(u<4)return null;const l=u*N_,h=Math.max(0,Math.trunc(a-l)),c=Math.min(n.width,Math.trunc(a+l)),p=Math.max(0,Math.trunc(s-l)),f=Math.min(n.height,Math.trunc(s+l));if(c-h<8||f-p<8)return null;const m=Math.max(n.width,n.height)<Ta?ka:1,y=tt(e,n),w=tt(e,t),b=new e.Rect(h,p,c-h,f-p),x=y.roi(b),M=new e.Mat;m!==1?e.resize(x,M,new e.Size(0,0),m,m,e.INTER_CUBIC):x.copyTo(M);const v=new e.Mat,I=new e.Mat;e.cvtColor(w,v,e.COLOR_RGB2GRAY),e.cvtColor(M,I,e.COLOR_RGB2GRAY);const T=new e.ORB(mm),k=new e.KeyPointVector,S=new e.KeyPointVector,A=new e.Mat,N=new e.Mat,U=new e.Mat,V=[y,w,x,M,v,I,k,S,A,N,U],F=te=>{for(const ye of V)try{ye.delete()}catch{}try{T.delete()}catch{}return te};if(T.detectAndCompute(v,U,k,A),T.detectAndCompute(I,U,S,N),A.rows<8||N.rows<8)return F(null);const O=new e.BFMatcher(e.NORM_HAMMING),H=new e.DMatchVectorVector;O.knnMatch(A,N,H,2);const X=[],J=[];for(let te=0;te<H.size();te++){const ye=H.get(te);if(ye.size()===2){const Me=ye.get(0),ze=ye.get(1);if(Me.distance<Ea*ze.distance){const De=k.get(Me.queryIdx).pt,lt=S.get(Me.trainIdx).pt;X.push(De.x,De.y),J.push(lt.x,lt.y)}}}if(H.delete(),O.delete(),X.length/2<8)return F(null);const he=e.matFromArray(X.length/2,1,e.CV_32FC2,X),W=e.matFromArray(J.length/2,1,e.CV_32FC2,J),z=new e.Mat,R=e.findHomography(he,W,e.RANSAC,5,z);let B=0;for(let te=0;te<z.rows;te++)B+=z.data[te];const L=R.rows===3?[...R.data64F]:null;if(he.delete(),W.delete(),z.delete(),R.delete(),L===null||B<gm)return F(null);const G=1/m,Z=[[G,0,h],[0,G,p],[0,0,1]],ie=[0,1,2].map(te=>[0,1,2].map(ye=>Z[te][0]*L[ye]+Z[te][1]*L[3+ye]+Z[te][2]*L[6+ye]));return F({H:ie,inliers:B})}function Aa(e,t,n){if(e.length!==4||e.some(u=>!Number.isFinite(u[0])||!Number.isFinite(u[1])))return!1;let r=0;for(let u=0;u<4;u++){const[l,h]=e[u],[c,p]=e[(u+1)%4];r+=l*p-c*h}const i=Math.abs(r/2)/(t*n);if(i<R_||i>O_)return!1;const o=e.map((u,l)=>{const h=e[(l+1)%4];return Math.hypot(h[0]-u[0],h[1]-u[1])}),a=Math.min(...o);if(a<1)return!1;const s=Math.max(...o)/a;return s>=C_&&s<=A_}function Ra(e,t,n){const r=e[2][0]*t+e[2][1]*n+e[2][2];return[(e[0][0]*t+e[0][1]*n+e[0][2])/r,(e[1][0]*t+e[1][1]*n+e[1][2])/r]}function Oa(e,t,n,r){const i=n.width,o=n.height,a=Math.max(8,Math.trunc(Ca*i)),s=i+2*a,u=o+2*a;if(s*u>4e7)return null;const l=r.map(V=>[V[0],V[1],V[2]-a*(V[0]+V[1])+0]);for(let V=0;V<3;V++)l[V][2]=r[V][2]-a*r[V][0]-a*r[V][1];const h=tt(e,t),c=new e.Mat,p=e.matFromArray(3,3,e.CV_64F,l.flat());e.warpPerspective(h,c,p,new e.Size(s,u),e.WARP_INVERSE_MAP);const f=new e.Mat;e.cvtColor(c,f,e.COLOR_RGB2Lab),h.delete(),p.delete();const m=f.data,y=Math.max(4,Math.trunc(a/3)),w=[[],[],[]],b=(V,F)=>{const O=(F*s+V)*3;w[0].push(m[O]),w[1].push(m[O+1]),w[2].push(m[O+2])};for(let V=0;V<u;V++)for(let F=0;F<s;F++)(V<y||V>=u-y||F<y||F>=s-y)&&b(F,V);const x=V=>{V.sort((O,H)=>O-H);const F=V.length>>1;return V.length%2?V[F]:(V[F-1]+V[F])/2},M=[x(w[0]),x(w[1]),x(w[2])],v=(V,F)=>{const O=(F*s+V)*3,H=m[O]-M[0],X=m[O+1]-M[1],J=m[O+2]-M[2];return Math.sqrt(H*H+X*X+J*J)>z_},I=Math.max(6,Math.trunc(ym*i)),T=Math.max(6,Math.trunc(ym*o)),k=Math.max(2,Math.trunc(wm*i)),S=Math.max(2,Math.trunc(wm*o)),A=V=>{let F=0,O=0;for(const H of V)O=H?O+1:0,O>F&&(F=O);return F/Math.max(1,V.length)},N=V=>{let F,O,H,X,J;if(V==="L"?(F=a,O=a+o,H=Math.max(0,a-k-I),X=Math.max(0,a-k),J=!1):V==="R"?(F=a,O=a+o,H=a+i+k,X=Math.min(s,a+i+k+I),J=!1):(F=Math.max(0,a-S-T),O=Math.max(0,a-S),H=a,X=a+i,J=!0),O<=F||X<=H)return 0;const he=[];if(J)for(let W=H;W<X;W++){let z=0;for(let R=F;R<O;R++)v(W,R)&&z++;he.push(z/(O-F)>_m)}else for(let W=F;W<O;W++){let z=0;for(let R=H;R<X;R++)v(R,W)&&z++;he.push(z/(X-H)>_m)}return A(he)},U={L:N("L"),R:N("R"),T:N("T")};return c.delete(),f.delete(),U}const P_=6e3,D_=8,bm=.5,U_=.6;function L_(e,t,n,r){if(n.size===0)return[];const i=Math.max(t.width,t.height)<Ta?ka:1,o=tt(e,t),a=new e.Mat;i!==1?e.resize(o,a,new e.Size(0,0),i,i,e.INTER_CUBIC):o.copyTo(a);const s=new e.Mat;e.cvtColor(a,s,e.COLOR_RGB2GRAY),o.delete(),a.delete();const u=new e.ORB(P_),l=new e.Mat,h=new e.KeyPointVector,c=new e.Mat;u.detectAndCompute(s,l,h,c);const p=[],f=new e.BFMatcher(e.NORM_HAMMING);try{if(c.rows<8)return p;for(const[m,y]of n){if(r!==void 0&&Date.now()>r)break;const w=tt(e,y),b=new e.Mat;e.cvtColor(w,b,e.COLOR_RGB2GRAY);const x=new e.KeyPointVector,M=new e.Mat;u.detectAndCompute(b,l,x,M);const v=[w,x,M],I=()=>{for(const ie of v)ie.delete();b.delete()};if(M.rows<8){I();continue}const T=new e.DMatchVectorVector;f.knnMatch(M,c,T,2);const k=[],S=[];for(let ie=0;ie<T.size();ie++){const te=T.get(ie);if(te.size()===2){const ye=te.get(0);if(ye.distance<Ea*te.get(1).distance){const Me=x.get(ye.queryIdx).pt,ze=h.get(ye.trainIdx).pt;k.push(Me.x,Me.y),S.push(ze.x,ze.y)}}}if(T.delete(),k.length/2<8){I();continue}const A=e.matFromArray(k.length/2,1,e.CV_32FC2,k),N=e.matFromArray(S.length/2,1,e.CV_32FC2,S),U=new e.Mat,V=e.findHomography(A,N,e.RANSAC,5,U);let F=0;for(let ie=0;ie<U.rows;ie++)F+=U.data[ie];const O=V.rows===3?[...V.data64F]:null;if(A.delete(),N.delete(),U.delete(),V.delete(),O===null||F<D_){I();continue}const H=1/i,X=[[H*O[0],H*O[1],H*O[2]],[H*O[3],H*O[4],H*O[5]],[O[6],O[7],O[8]]],J=[[0,0],[y.width,0],[y.width,y.height],[0,y.height]].map(([ie,te])=>Ra(X,ie,te));if(!Aa(J,t.width,t.height)){I();continue}const he=tt(e,t),W=e.matFromArray(3,3,e.CV_64F,X.flat()),z=new e.Mat;e.warpPerspective(he,z,W,new e.Size(y.width,y.height),e.WARP_INVERSE_MAP);const R=new e.Mat;e.cvtColor(z,R,e.COLOR_RGB2GRAY);const B=new e.Mat;e.matchTemplate(R,b,B,e.TM_CCOEFF_NORMED);const L=B.data32F[0];if(he.delete(),W.delete(),z.delete(),R.delete(),B.delete(),L<bm){I();continue}const G=Oa(e,t,y,X),Z=Na(G);p.push({id:m,confidence:Math.max(0,L),footprint:J,built:G!==null&&Math.max(G.L,G.R,G.T)>=ii,tuckRegion:za(J,Z)}),I()}}finally{s.delete(),l.delete(),h.delete(),c.delete();try{u.delete(),f.delete()}catch{}}return p}function Na(e){return e!==null&&e.R>=ii?["R"]:[]}function za(e,t){if(e.length<4||t.length===0)return null;const n=e.map(y=>[y[0],y[1]]),r=Math.hypot(n[1][0]-n[0][0],n[1][1]-n[0][1]),i=Math.hypot(n[2][0]-n[3][0],n[2][1]-n[3][1]),o=.5*(r+i),a=Ca*o;if(!(a>0))return null;const s=n.reduce((y,w)=>y+w[0],0)/n.length,u=n.reduce((y,w)=>y+w[1],0)/n.length,l={T:[0,1],R:[1,2],L:[0,3]},h=[...n];for(const y of["L","R","T"]){if(!t.includes(y))continue;const[w,b]=l[y],x=n[w],M=n[b];let v=-(M[1]-x[1]),I=M[0]-x[0];const T=(x[0]+M[0])/2,k=(x[1]+M[1])/2;v*(T-s)+I*(k-u)<0&&(v=-v,I=-I);const S=Math.hypot(v,I);S<=1e-6||(v=v/S*a,I=I/S*a,h.push([x[0]+v,x[1]+I],[M[0]+v,M[1]+I]))}const c=h.map(y=>y[0]),p=h.map(y=>y[1]),f=Math.round(Math.min(...c)),m=Math.round(Math.min(...p));return{x:f,y:m,width:Math.round(Math.max(...c))-f,height:Math.round(Math.max(...p))-m}}function F_(e,t,n,r){const i=B_(e,n,t,r);if(i===null)return null;const a=[[0,0],[n.width,0],[n.width,n.height],[0,n.height]].map(([l,h])=>Ra(i.H,l,h));if(!Aa(a,t.width,t.height))return null;const s=Oa(e,t,n,i.H);if(s===null)return null;const u=Na(s);return{built:Math.max(s.L,s.R,s.T)>=ii,footprint:a,overflow:u,edgeScores:s,inliers:i.inliers}}const G_=.88;function xm(e,t,n,r){if(r.length!==4)return null;const i=n.width,o=n.height,a=Math.max(8,Math.trunc(Ca*i)),s=i+2*a,u=o+2*a;if(s*u>4e7)return null;const l=a+Math.trunc(i*G_),h=s-l;if(h<1)return null;const c=tt(e,t),p=e.matFromArray(4,1,e.CV_32FC2,[0,0,i,0,i,o,0,o]),f=e.matFromArray(4,1,e.CV_32FC2,[r[0][0],r[0][1],r[1][0],r[1][1],r[2][0],r[2][1],r[3][0],r[3][1]]),m=e.getPerspectiveTransform(p,f),y=[...m.data64F],w=[0,1,2].flatMap(k=>[y[k*3],y[k*3+1],y[k*3+2]-a*y[k*3]-a*y[k*3+1]]),b=e.matFromArray(3,3,e.CV_64F,w),x=new e.Mat;e.warpPerspective(c,x,b,new e.Size(s,u),e.WARP_INVERSE_MAP);const M=x.roi(new e.Rect(l,0,h,u)),v=new e.Mat;M.copyTo(v);const I=v.data,T=new Uint8ClampedArray(h*u*3);T.set(I.subarray(0,T.length));for(const k of[c,p,f,m,b,x,M,v])try{k.delete()}catch{}return{width:h,height:u,channels:3,data:T}}function W_(e,t,n,r){const[i,o,a,s]=r;if(a<8||s<8)return null;const u=Math.trunc(.06*a),l=Math.trunc(.06*s),h=Math.max(0,Math.trunc(i-u)),c=Math.min(n.width,Math.trunc(i+a+u)),p=Math.max(0,Math.trunc(o-l)),f=Math.min(n.height,Math.trunc(o+s+l));if(c-h<8||f-p<8)return null;const m=Math.max(n.width,n.height)<Ta?ka:1,y=tt(e,n),w=tt(e,t),b=y.roi(new e.Rect(h,p,c-h,f-p)),x=new e.Mat;m!==1?e.resize(b,x,new e.Size(0,0),m,m,e.INTER_CUBIC):b.copyTo(x);const M=new e.Mat,v=new e.Mat;e.cvtColor(w,M,e.COLOR_RGB2GRAY),e.cvtColor(x,v,e.COLOR_RGB2GRAY);const I=new e.ORB(mm),T=new e.KeyPointVector,k=new e.KeyPointVector,S=new e.Mat,A=new e.Mat,N=new e.Mat,U=[y,w,b,x,M,v,T,k,S,A,N],V=ie=>{for(const te of U)try{te.delete()}catch{}try{I.delete()}catch{}return ie};if(I.detectAndCompute(M,N,T,S),I.detectAndCompute(v,N,k,A),S.rows<8||A.rows<8)return V(null);const F=new e.BFMatcher(e.NORM_HAMMING),O=new e.DMatchVectorVector;F.knnMatch(S,A,O,2);const H=[],X=[];for(let ie=0;ie<O.size();ie++){const te=O.get(ie);if(te.size()===2){const ye=te.get(0),Me=te.get(1);if(ye.distance<Ea*Me.distance){const ze=T.get(ye.queryIdx).pt,De=k.get(ye.trainIdx).pt;H.push(ze.x,ze.y),X.push(De.x,De.y)}}}if(O.delete(),F.delete(),H.length/2<8)return V(null);const J=e.matFromArray(H.length/2,1,e.CV_32FC2,H),he=e.matFromArray(X.length/2,1,e.CV_32FC2,X),W=new e.Mat,z=e.findHomography(J,he,e.RANSAC,5,W);let R=0;for(let ie=0;ie<W.rows;ie++)R+=W.data[ie];const B=z.rows===3?[...z.data64F]:null;if(J.delete(),he.delete(),W.delete(),z.delete(),B===null||R<gm)return V(null);const L=1/m,G=[[L,0,h],[0,L,p],[0,0,1]],Z=[0,1,2].map(ie=>[0,1,2].map(te=>G[ie][0]*B[te]+G[ie][1]*B[3+te]+G[ie][2]*B[6+te]));return V({H:Z,inliers:R})}const q_=620;function V_(e,t){return{width:t.cols,height:t.rows,channels:3,data:new Uint8Array(t.data.slice(0,t.rows*t.cols*3))}}function $m(e,t,n,r){const i=vm(e,t,n,r);if(i!==null)return i;try{const[o,a,s,u]=r.map(I=>Math.trunc(I));if(Math.min(s,u)>=q_||s<=0||u<=0)return null;const l=Math.trunc(s*.25),h=Math.trunc(u*.25),c=Math.max(0,o-l),p=Math.max(0,a-h),f=Math.min(t.width,o+s+l),m=Math.min(t.height,a+u+h);if(f<=c||m<=p)return null;const y=tt(e,t),w=y.roi(new e.Rect(c,p,f-c,m-p)),b=new e.Mat;e.resize(w,b,new e.Size((f-c)*2,(m-p)*2),0,0,e.INTER_CUBIC);const x=V_(e,b);for(const I of[y,w,b])try{I.delete()}catch{}const M=[(o-c)*2,(a-p)*2,s*2,u*2],v=vm(e,x,n,M);return v===null?null:{...v,footprint:v.footprint.map(([I,T])=>[I*.5+c,T*.5+p])}}catch{return null}}function vm(e,t,n,r){const i=W_(e,n,t,r);if(i===null)return null;const a=[[0,0],[n.width,0],[n.width,n.height],[0,n.height]].map(([b,x])=>Ra(i.H,b,x));if(!Aa(a,t.width,t.height))return null;const s=tt(e,t),u=e.matFromArray(3,3,e.CV_64F,i.H.flat()),l=new e.Mat;e.warpPerspective(s,l,u,new e.Size(n.width,n.height),e.WARP_INVERSE_MAP);const h=tt(e,n),c=new e.Mat,p=new e.Mat;e.cvtColor(l,c,e.COLOR_RGB2GRAY),e.cvtColor(h,p,e.COLOR_RGB2GRAY);const f=new e.Mat;e.matchTemplate(c,p,f,e.TM_CCOEFF_NORMED);const m=f.data32F[0];for(const b of[s,u,l,h,c,p,f])try{b.delete()}catch{}if(m<bm)return null;const y=Oa(e,t,n,i.H);if(y===null)return null;const w=Na(y);return{built:Math.max(y.L,y.R,y.T)>=ii,footprint:a,overflow:w,edgeScores:y,inliers:i.inliers}}function H_(e,t,n,r=.03){let i=null,o=1/0;for(const a of e){const[s,u,l,h]=a;if(l<=0||h<=0)continue;const c=r*l,p=r*h;if(t>=s-c&&t<=s+l+c&&n>=u-p&&n<=u+h+p){const f=l*h;f<o&&(o=f,i=[s,u,l,h])}}return i}const j_=.3,K_=.3;function Y_(e,t){const n=e.filter(o=>o.edgeScores!==null);if(n.length===0)return[];const r=n.length>=2&&n.every(o=>{const{L:a,R:s,T:u}=o.edgeScores;return Math.min(a,s,u)>=j_}),i=[];return e.forEach((o,a)=>{if(!o.built||o.edgeScores===null)return;const{L:s,R:u,T:l}=o.edgeScores,h=Math.max(s,u,l)<K_;if(!r&&!h)return;t.some(([p,f])=>p>=o.zone.x0&&p<=o.zone.x1&&f>=o.zone.y0&&f<=o.zone.y1)||i.push(a)}),i}const Bt=128,Ba=.5;function Pa(e){const t=jn(e,Bt,Bt),n=Bt*Bt,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let o=0;o<3;o++)r[o*n+i]=t[i*3+o]/255;return r}function Sm(e){const t=e[1]??0;return{built:t>=Ba,prob:t}}const xr=120,$r=179,X_=1.3,Z_=3.6,Q_=.45,J_=6e-4,eb=.02,tb=6e3,nb=.78,rb=1.25,ib=2.4,ob=.05,ab=1.5,sb=.5,ub=.9,lb=150,cb=18,db=34,hb=90,pb=130,fb=.13,mb=.15,oi="magistrates-guild",Da="merchants-guild";function gb(e,t){const n=tt(e,t),r=new e.Mat;e.cvtColor(n,r,e.COLOR_RGB2HSV),n.delete();const i=new e.Mat(r.rows,r.cols,r.type(),[xr,30,40,0]),o=new e.Mat(r.rows,r.cols,r.type(),[$r,255,205,255]),a=new e.Mat;e.inRange(r,i,o,a),r.delete(),i.delete(),o.delete();const s=new Uint8Array(a.data),u=e.getStructuringElement(e.MORPH_RECT,new e.Size(31,31)),l=new e.Mat;e.morphologyEx(a,l,e.MORPH_CLOSE,u),a.delete(),u.delete();const h=new e.Mat,c=new e.Mat,p=new e.Mat,f=e.connectedComponentsWithStats(l,h,c,p,8);l.delete(),h.delete(),p.delete();const m=t.width*t.height,y=[];for(let w=1;w<f;w++){const b=c.intAt(w,0),x=c.intAt(w,1),M=c.intAt(w,2),v=c.intAt(w,3),I=c.intAt(w,4),T=I/m;T<J_||T>eb||I/Math.max(M*v,1)<Q_||y.push({x:b,y:x,w:M,h:v})}return c.delete(),{blobs:y,mask:s,maskWidth:t.width}}function yb(e,t,n,r,i,o,a){const s=e,u=o,l=a,h=i;if(!h.gray){const L=tt(e,r);h.gray=new s.Mat,s.cvtColor(L,h.gray,s.COLOR_RGB2GRAY),L.delete(),h.k=new s.KeyPointVector,h.d=new s.Mat;const G=new s.Mat;u.detectAndCompute(h.gray,G,h.k,h.d),G.delete()}const c=n,p=new s.Mat,f=new s.KeyPointVector,m=new s.Mat;u.detectAndCompute(c,p,f,m),p.delete();const y=L=>(f.delete(),m.delete(),L);if(h.d.rows<8||m.rows<8)return y(null);const w=new s.DMatchVectorVector;l.knnMatch(h.d,m,w,2);const b=[],x=[];for(let L=0;L<w.size();L++){const G=w.get(L);if(G.size()===2){const Z=G.get(0);if(Z.distance<nb*G.get(1).distance){const ie=h.k.get(Z.queryIdx).pt,te=f.get(Z.trainIdx).pt;b.push(ie.x,ie.y),x.push(te.x,te.y)}}}if(w.delete(),b.length/2<8)return y(null);const M=s.matFromArray(b.length/2,1,s.CV_32FC2,b),v=s.matFromArray(x.length/2,1,s.CV_32FC2,x),I=new s.Mat,T=s.findHomography(M,v,s.RANSAC,5,I);if(M.delete(),v.delete(),I.delete(),T.rows!==3)return T.delete(),y(null);const k=[...T.data64F],S=(L,G)=>{const Z=k[6]*L+k[7]*G+k[8];return[(k[0]*L+k[1]*G+k[2])/Z,(k[3]*L+k[4]*G+k[5])/Z]},A=[[0,0],[r.width,0],[r.width,r.height],[0,r.height]].map(([L,G])=>S(L,G));if(A.some(L=>!Number.isFinite(L[0])||!Number.isFinite(L[1])))return T.delete(),y(null);const N=A.map((L,G)=>{const Z=A[(G+1)%4];return Math.hypot(Z[0]-L[0],Z[1]-L[1])}),U=Math.min(...N);if(U<1)return T.delete(),y(null);const V=Math.max(...N)/U;let F=0;for(let L=0;L<4;L++){const[G,Z]=A[L],[ie,te]=A[(L+1)%4];F+=G*te-ie*Z}const O=t,H=Math.abs(F/2)/(O.rows*O.cols);if(V<rb||V>ib||H<ob||H>ab)return T.delete(),y(null);const X=new s.Mat;s.warpPerspective(O,X,T,new s.Size(r.width,r.height),s.WARP_INVERSE_MAP),T.delete();const J=new s.Mat;s.cvtColor(X,J,s.COLOR_RGB2GRAY),X.delete();const he=Math.trunc(r.height/2),W=J.roi(new s.Rect(0,0,r.width,he)),z=h.gray.roi(new s.Rect(0,0,r.width,he)),R=new s.Mat;s.matchTemplate(W,z,R,s.TM_CCOEFF_NORMED);const B=R.data32F[0];return W.delete(),z.delete(),R.delete(),J.delete(),y(B)}function wb(e,t,n){let r,i;if(n===oi)r=Da,i=fb;else if(n===Da)r=oi,i=mb;else return null;const{x:o,y:a,w:s,h:u}=t;if(s<8||u<8)return null;const l=Math.trunc(s/2);let h=0,c=null;for(const[p,f]of[[0,l],[l,s]]){let m=0,y=0;for(let b=a;b<a+u;b++)for(let x=o+p;x<o+f;x++){const M=(b*e.width+x)*e.channels,{h:v,s:I,v:T}=Ct(e.data[M],e.data[M+1],e.data[M+2]);if(v>=xr&&v<=$r&&I>=30&&I<=170&&T<=170)continue;m++,(r===Da?v>=cb&&v<=db&&I>=hb&&T>=pb:v>=95&&v<=130&&I>=80)&&y++}if(m<20)continue;const w=y/m;w>h&&(h=w,c={x:o+p,y:a,w:f-p,h:u})}return h>=i&&c!==null?{id:r,box:c}:null}const _b=1.7,bb=140,xb=170,$b=.2,vb=.1,Mm=240,Im=80,Em=60,Sb=50,Tm="scientists-guild",km="tacticians-guild",ai=["shipowners-guild","merchants-guild","builders-guild","moneylenders-guild"];function Mb(e,t,n){const{x:r,y:i,w:o,h:a}=n,s=new Float32Array(a);for(let v=0;v<a;v++){let I=0;for(let T=0;T<o;T++)e[(i+v)*t+r+T]>0&&I++;s[v]=I/o}const u=[];for(let v=0;v<a;v++)s[v]>.3&&u.push(v);if(u.length<5)return[];const l=u[0],h=u[u.length-1],c=h-l;if(c<5)return[];const p=o/c;if(p<X_||p>Z_)return[];if(p>=_b)return[{x:r,y:i+l,w:o,h:c}];const f=new Float32Array(a),m=.3*(8*.5-1)+.8,y=[];let w=0;for(let v=-4;v<=4;v++){const I=Math.exp(-(v*v)/(2*m*m));y.push(I),w+=I}for(let v=0;v<a;v++){let I=0;for(let T=-4;T<=4;T++){const k=Math.min(a-1,Math.max(0,v+T));I+=s[k]*y[T+4]}f[v]=I/w}const b=l+Math.trunc(c*.3),x=l+Math.trunc(c*.78);let M=l+Math.trunc(c/2);if(x>b){let v=1/0;for(let I=b;I<x;I++)f[I]<v&&(v=f[I],M=I)}return[{x:r,y:i+l,w:o,h:M-l},{x:r,y:i+M,w:o,h:h-M}]}function Ib(e,t){const n=Math.max(0,t.x),r=Math.max(0,t.y),i=Math.min(e.width,t.x+t.w),o=Math.min(e.height,t.y+t.h),a=Math.max(0,i-n),s=Math.max(0,o-r),u=new Uint8Array(a*s*3);for(let l=0;l<s;l++)for(let h=0;h<a;h++){const c=((r+l)*e.width+n+h)*e.channels,p=(l*a+h)*3;u[p]=e.data[c],u[p+1]=e.data[c+1],u[p+2]=e.data[c+2]}return{width:a,height:s,channels:3,data:u}}function Eb(e){let t=0,n=0;for(let r=0,i=e.width*e.height;r<i;r++){const o=r*e.channels,{h:a,s,v:u}=Ct(e.data[o],e.data[o+1],e.data[o+2]);s>=40&&u>=40&&u<=205&&(t++,a>=bb&&a<=xb&&n++)}return t===0?0:n/t}function Tb(e){let t=0;const n=e.width*e.height;for(let r=0;r<n;r++){const i=r*e.channels,{h:o,s:a,v:s}=Ct(e.data[i],e.data[i+1],e.data[i+2]);!(o>=xr&&o<=$r)&&a>=70&&s>=50&&t++}return n===0?0:t/n}function Cm(e,t){const n=tt(e,t),r=new e.Mat;e.resize(n,r,new e.Size(Mm,Im),0,0,e.INTER_AREA),n.delete();const i=new Uint8Array(r.data);return r.delete(),{width:Mm,height:Im,channels:3,data:i}}function kb(e){const t=e.width*e.height,n=[0,0,0];for(let o=0;o<t;o++){const a=o*e.channels;n[0]+=e.data[a],n[1]+=e.data[a+1],n[2]+=e.data[a+2]}n[0]/=t,n[1]/=t,n[2]/=t;const r=(n[0]+n[1]+n[2])/3,i=new Uint8Array(t*3);for(let o=0;o<t;o++){const a=o*e.channels;for(let s=0;s<3;s++){const u=n[s]>1e-6?r/n[s]:1;i[o*3+s]=Math.max(0,Math.min(255,Math.round(e.data[a+s]*u)))}}return{width:e.width,height:e.height,channels:3,data:i}}function Am(e,t){const n=kb(t),r=n.width*n.height,i=new Uint8Array(r);let o=0;for(let m=0;m<r;m++){const y=m*3,{h:w,s:b,v:x}=Ct(n.data[y],n.data[y+1],n.data[y+2]);!(w>=xr&&w<=$r&&b>=30&&b<=170&&x<=170)&&x>=40&&(i[m]=1,o++)}const a=o<20,s=tt(e,n),u=new e.Mat;e.cvtColor(s,u,e.COLOR_RGB2Lab),s.delete();const l=u.data;let h=0,c=0,p=0,f=0;for(let m=0;m<r;m++)!a&&i[m]===0||(h+=l[m*3]*100/255,c+=l[m*3+1]-128,p+=l[m*3+2]-128,f++);return u.delete(),f===0?[0,0,0]:[h/f,c/f,p/f]}function Cb(e){let t=0,n=0,r=0,i=0,o=0;const a=e.width*e.height;for(let u=0;u<a;u++){const l=u*e.channels,{h,s:c,v:p}=Ct(e.data[l],e.data[l+1],e.data[l+2]);h>=xr&&h<=$r&&c>=30&&c<=170&&p<=170||(t++,c>=70&&p>=50&&(h>=95&&h<=130?n++:h>=35&&h<=92?r++:h<=10?i++:h>=15&&h<=34&&p>=80&&o++))}const s=Math.max(t,1);return{blue:n/s,green:r/s,red:i/s,gold:o/s}}function Ab(e){const t=e.width*e.height,n={blue:0,green:0,red:0,gold:0,brown:0,grey:0};for(let r=0;r<t;r++){const i=r*e.channels,{h:o,s:a,v:s}=Ct(e.data[i],e.data[i+1],e.data[i+2]);a>=Em&&s>=Sb?(o>=95&&o<=128&&n.blue++,o>=35&&o<=85&&n.green++,(o<=8||o>=170)&&n.red++,o>=18&&o<=34&&n.gold++,o>=4&&o<=17&&s<150&&n.brown++):a<Em&&s>=70&&s<=235&&n.grey++}for(const r of Object.keys(n))n[r]/=t;return n}function Rb(e,t){let n=0,r=0;for(let s=0;s<e.length;s++)n+=e[s],r+=t[s];n/=e.length,r/=t.length;let i=0,o=0,a=0;for(let s=0;s<e.length;s++){const u=e[s]-n,l=t[s]-r;i+=u*l,o+=u*u,a+=l*l}return i/(Math.sqrt(o*a)+1e-6)}function Rm(e,t){const n=tt(e,t),r=new e.Mat;e.cvtColor(n,r,e.COLOR_RGB2GRAY),n.delete();const i=Float32Array.from(r.data);return r.delete(),i}function Ob(e,t){const n=new Map,r=new Map;for(const[i,o]of t){const a=Cm(e,o);n.set(i,Rm(e,a)),ai.includes(i)&&r.set(i,Am(e,a))}return{gray:n,warmLab:r}}function Nb(e,t,n){const r=Cm(e,t),i=Cb(r);if(i.blue>=.15&&i.blue>i.red&&i.blue>2*i.gold)return oi;if(i.green>=.08&&i.green>i.blue&&i.green>i.gold)return Tm;if(i.red>=.15&&i.red>i.blue&&i.red>1.5*i.gold)return km;const o=Ab(r),a={blue:o.blue,green:o.green,red:o.red,gold:o.gold,browngrey:o.brown+o.grey};let s="blue";for(const l of Object.keys(a))a[l]>a[s]&&(s=l);if(a[s]<=0)return"";let u;if(s==="blue")u=oi;else if(s==="green")u=Tm;else if(s==="red")u=km;else{const l=Rm(e,r);let h="",c=-2;for(const p of ai){const f=n.gray.get(p);if(f===void 0)continue;const m=Rb(l,f);m>c&&(c=m,h=p)}u=h||ai[0]}if(ai.includes(u)&&n.warmLab.size>0){const l=Am(e,r);let h=u,c=1/0;for(const[p,f]of n.warmLab){const m=Math.hypot(l[0]-f[0],l[1]-f[1],l[2]-f[2]);m<c&&(c=m,h=p)}return h}return u}function zb(e,t,n,r,i){var y;const o=[],{blobs:a,mask:s,maskWidth:u}=gb(e,t);if(a.length===0||n.size===0)return o;const l=e,h=new l.ORB(tb),c=new l.BFMatcher(l.NORM_HAMMING),p=new Map;for(const w of n.keys())p.set(w,{});const f=tt(e,t);let m=null;try{for(const w of a){if(r!==void 0&&Date.now()>r)break;const b=w.x+Math.trunc(w.w/2),x=w.y+Math.trunc(w.h/2),M=Math.max(lb,Math.trunc(ub*Math.max(w.w,w.h))),v=Math.max(0,b-M),I=Math.max(0,x-M),T=Math.min(t.width,b+M),k=Math.min(t.height,x+M);if(T-v<16||k-I<16)continue;const S=f.roi(new l.Rect(v,I,T-v,k-I)),A=new l.Mat;l.cvtColor(S,A,l.COLOR_RGB2GRAY);let N=null,U=-2;for(const[H,X]of n){if(r!==void 0&&Date.now()>r)break;const J=yb(e,S,A,X,p.get(H),h,c);J!==null&&J>U&&(U=J,N=H)}S.delete(),A.delete();const V=new Set;if(N!==null&&U>=sb){o.push({id:N,boundingBox:{x:w.x,y:w.y,width:w.w,height:w.h},confidence:1}),V.add(N);const H=wb(t,w,N);H&&(o.push({id:H.id,boundingBox:{x:H.box.x,y:H.box.y,width:H.box.w,height:H.box.h},confidence:.9}),V.add(H.id))}if(i===void 0||i.size===0)continue;const F=Mb(s,u,w);if(F.length!==2)continue;const O=F.map(H=>Ib(t,H));if(!O.some(H=>H.width*H.height===0||Tb(H)<vb))for(let H=0;H<F.length;H++){const X=O[H];if(Eb(X)<$b)continue;m===null&&(m=Ob(e,i));const J=Nb(e,X,m);if(J&&!V.has(J)){V.add(J);const he=F[H];o.push({id:J,boundingBox:{x:he.x,y:he.y,width:he.w,height:he.h},confidence:1})}}}}finally{f.delete();for(const w of p.values()){const b=w;for(const x of["gray","k","d"])try{(y=b[x])==null||y.delete()}catch{}}try{h.delete(),c.delete()}catch{}}return o}const Om=128,Bb=.56,Pb=15,Db=.58,Ub=70,Lb=50,Fb=.12,Gb=.2,Wb=.1,qb=.17,Nm=.15;function Vb(e){const t=new Map;for(const[n,r]of Object.entries(e.templates)){const i=Uint8Array.from(atob(r),o=>o.charCodeAt(0));i.length===e.size*e.size&&t.set(n,i)}return t}function zm(e,t){const{width:n,height:r,channels:i,data:o}=e,a=Math.floor(n/2),s=Math.floor(r/2),u=Math.trunc(Math.min(n,r)*.5*t);if(u<1)return e;const l=Math.max(0,a-u),h=Math.max(0,s-u),c=Math.min(n,a+u),p=Math.min(r,s+u),f=c-l,m=p-h,y=new Uint8Array(f*m*i);for(let w=0;w<m;w++){const b=((w+h)*n+l)*i;y.set(o.subarray(b,b+f*i),w*f*i)}return{width:f,height:m,channels:i,data:y}}function Hb(e){const t=zm(e,Bb),n=Pw(t),r=dm(n,Om,Om);return Dw(r)}function jb(e,t){const n=e.length;let r=0,i=0;for(let u=0;u<n;u++)r+=e[u],i+=t[u];r/=n,i/=n;let o=0,a=0,s=0;for(let u=0;u<n;u++){const l=e[u]-r,h=t[u]-i;o+=l*h,a+=l*l,s+=h*h}return o/(Math.sqrt(a*s)+1e-6)}function Kb(e){const t=new Map([["masonry",0],["strategy",0]]),n=zm(e,Db),{width:r,height:i,channels:o,data:a}=n,s=r*i||1;let u=0,l=0;for(let p=0;p<r*i;p++){const f=p*o,{h:m,s:y,v:w}=Ct(a[f],a[f+1],a[f+2]);y>=Ub&&w>=Lb&&(m>=95&&m<=130&&(u+=1),(m<=8||m>=170)&&(l+=1))}const h=u/s,c=l/s;return h>=Fb&&t.set("masonry",Nm*Math.min(1,h/Gb)),c>=Wb&&t.set("strategy",Nm*Math.min(1,c/qb)),t}function Yb(e,t){if(t.size===0||e.width===0||e.height===0)return["",0];const n=Hb(e);let r=0;for(const l of n.data)r+=l;const i=r/n.data.length,o=[];for(let l=0;l<360;l+=Pb)o.push(Fw(n,l,i));const a=new Map;for(const[l,h]of t){let c=-1/0;for(const p of o){const f=jb(p,h);f>c&&(c=f)}a.set(l,c)}for(const[l,h]of Kb(e))h>0&&a.has(l)&&a.set(l,a.get(l)+h);let s="",u=-1/0;for(const[l,h]of a)h>u&&(s=l,u=h);return[s,u]}const sn=224,Xb=512,Zb=[.485,.456,.406],Qb=[.229,.224,.225];function Jb(e){const t=atob(e.x),n=new Uint8Array(t.length);for(let i=0;i<t.length;i++)n[i]=t.charCodeAt(i);const r=new Float32Array(n.buffer);if(r.length!==e.ids.length*e.dim)throw new Error(`token_embed_index: ${r.length} floats != ${e.ids.length}x${e.dim}`);return{dim:e.dim,ids:e.ids,x:r}}function e1(e){const t=wa(e,sn,sn),n=sn*sn,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let o=0;o<3;o++)r[o*n+i]=(t[i*3+o]/255-Zb[o])/Qb[o];return r}function t1(e){const t=3*sn*sn,n=new Float32Array(4*t);for(let r=0;r<4;r++)n.set(e1(jt(e,r)),r*t);return n}function n1(e,t=Xb){const n=e.length/t,r=new Float32Array(t);for(let o=0;o<n;o++)for(let a=0;a<t;a++)r[a]+=e[o*t+a];let i=0;for(let o=0;o<t;o++)r[o]/=n,i+=r[o]*r[o];i=Math.max(Math.sqrt(i),1e-9);for(let o=0;o<t;o++)r[o]/=i;return r}function r1(e,t){let n=0,r=-2;for(let i=0;i<e.ids.length;i++){let o=0;const a=i*e.dim;for(let s=0;s<e.dim;s++)o+=e.x[a+s]*t[s];o>r&&(r=o,n=i)}return{id:e.ids[n],cosine:r}}const Yn=96,i1=["builders-guild","magistrates-guild","merchants-guild","moneylenders-guild","scientists-guild","shipowners-guild","tacticians-guild"],o1=.45;function a1(e){const t=wa(e,Yn,Yn),n=Yn*Yn,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let o=0;o<3;o++)r[o*n+i]=t[i*3+o]/255;return r}function s1(e){let t=0;for(let r=1;r<e.length;r++)e[r]>e[t]&&(t=r);const n=e[t];return{id:n>=o1?i1[t]??"":"",prob:n}}const Xn=128,u1=["circus-maximus","piraeus","the-appian-way","the-colossus","the-great-library","the-great-lighthouse","the-hanging-gardens","the-mausoleum","the-pyramids","the-sphinx","the-statue-of-zeus","the-temple-of-artemis","other"],l1=.5,c1=.9;function d1(e){const t=jn(e,Xn,Xn),n=Xn*Xn,r=new Float32Array(3*n);for(let i=0;i<n;i++)for(let o=0;o<3;o++)r[o*n+i]=t[i*3+o]/255;return r}function h1(e){const{width:t,height:n,channels:r,data:i}=e,o=new Uint8ClampedArray(t*n*r);for(let a=0;a<t;a++)for(let s=0;s<n;s++){const u=a,h=((n-1-s)*t+u)*r,c=(a*n+s)*r;for(let p=0;p<r;p++)o[c+p]=i[h+p]}return{width:n,height:t,channels:r,data:o}}function p1(e,t){let n=e;const r=(t%4+4)%4;for(let i=0;i<r;i++)n=h1(n);return n}function f1(e){let t=0;for(let n=1;n<e.length;n++)e[n]>e[t]&&(t=n);return{index:t,prob:e[t]}}async function m1(e,t){let n=0,r=-1;for(let o=0;o<4;o++){const a=o===0?e:p1(e,o),s=await t(d1(a)),u=f1(s);u.prob>r&&(r=u.prob,n=u.index)}const i=r>=l1?u1[n]??"":"";return{id:i==="other"?"":i,prob:r}}const Zn=96,g1=[1,2,3,4,5,6,7],y1=.8,w1=.99;function _1(e){const t=_a(e,e.width*2,e.height*2),n=jn({width:e.width*2,height:e.height*2,channels:3,data:t},Zn,Zn),r=Zn*Zn,i=new Float32Array(3*r);for(let o=0;o<r;o++)for(let a=0;a<3;a++)i[a*r+o]=n[o*3+a]/255;return i}function b1(e){let t=0;for(let n=1;n<e.length;n++)e[n]>e[t]&&(t=n);return{value:g1[t],prob:e[t]}}const un=128,Bm=.35,x1=["fp","laurel"],$1=.85,Qn=40;function v1(e){const r=(e.width<un&&e.height<un?_a:jn)(e,un,un),i=un*un,o=new Float32Array(3*i);for(let a=0;a<i;a++)for(let s=0;s<3;s++)o[s*i+a]=r[a*3+s]/255;return o}function S1(e){return e[x1.indexOf("fp")]}const ln=128,M1=.15,Pm=["blue","brown","green","grey","purple","red","yellow","tuile_militaire","dos_de_carte","livret_de_regles","objet_hors_jeu"],I1=7,E1=.9;function T1(e,t,n){const[r,i,o,a]=e.map(Number);if(!(o>1)||!(a>1))return null;const s=r+o/2,u=i+a/2,l=Math.max(o,a)*(1+2*M1),h=Math.max(0,ut(s-l/2)),c=Math.max(0,ut(u-l/2)),p=Math.min(t,ut(s+l/2)),f=Math.min(n,ut(u+l/2));return p-h<8||f-c<8?null:{x:h,y:c,w:p-h,h:f-c}}function k1(e){const r=(e.width<ln&&e.height<ln?_a:jn)(e,ln,ln),i=ln*ln,o=new Float32Array(3*i);for(let a=0;a<i;a++)for(let s=0;s<3;s++)o[s*i+a]=r[a*3+s]/255;return o}function C1(e){let t=0;for(let i=1;i<Pm.length;i++)e[i]>e[t]&&(t=i);const n=e[t],r=t>=I1;return{className:Pm[t],probability:n,rejected:r&&n>=E1}}const si=3,A1=2.2,R1=.3,O1=.65,N1=3,z1=1.3,B1=.77;function Dm(e,t,n){const[r,i,o,a]=e,s=[];return r<=si&&s.push("gauche"),i<=si&&s.push("haut"),r+o>=t-si&&s.push("droit"),i+a>=n-si&&s.push("bas"),s}function Um(e){const t=e[3]/Math.max(e[2],1);return t>=z1?"portrait":t<=B1?"paysage":null}function Ua(e){const t=[...e].sort((r,i)=>r-i),n=t.length;return n===0?0:n%2?t[(n-1)/2]:.5*(t[n/2-1]+t[n/2])}function P1(e,t,n){for(const[r,i,o,a]of e??[])if(Math.max(Math.abs(o-r)/Math.max(t,1),Math.abs(a-i)/Math.max(n,1))>O1)return!0;return!1}function D1(e,t,n,r,i){try{const o=[...e],a=o.filter(w=>Dm(w.box,r,i).length>0);if(a.length===0)return{kept:o,dropped:[],suspects:[]};const s=o.filter(w=>!a.includes(w)),u=w=>({kept:s,dropped:a.map(b=>({banner:b,edgeReason:w})),suspects:[]});if(P1(n,r,i))return u("photo-piste");if(s.length<N1)return t>0?u("photo-merveilles"):{kept:o,dropped:[],suspects:a.map(w=>({family:w.family,color:w.color,box:w.box,reason:"bord-sans-scene"}))};if(a.length>(s.length+a.length)/3)return u("debordement-structurel");const l=Ua(s.map(w=>w.box[2]*w.box[3])),h=Ua(s.map(w=>w.box[2])),c=Ua(s.map(w=>w.box[3])),p=new Set(s.map(w=>Um(w.box)).filter(w=>w!==null)),f=[...s],m=[],y=[];for(const w of a){const b=Dm(w.box,r,i),[,,x,M]=w.box,v=l>0?x*M/l:0,I=[];(b.includes("gauche")||b.includes("droit"))&&I.push(h>0?x/h:1),(b.includes("haut")||b.includes("bas"))&&I.push(c>0?M/c:1);const T=I.length>0?Math.min(...I):1,k=Um(w.box);v>A1?m.push({banner:w,edgeReason:"bord-grosse"}):T<R1?m.push({banner:w,edgeReason:"bord-tronquee"}):k!==null&&p.size>0&&!p.has(k)?m.push({banner:w,edgeReason:"bord-orientation-adverse"}):(f.push(w),y.push({family:w.family,color:w.color,box:w.box,reason:"tronquee-par-le-bord"}))}return{kept:f,dropped:m,suspects:y}}catch{return{kept:[...e],dropped:[],suspects:[]}}}const U1=1,L1=1.5;function F1(e){return e.length<4?[]:[[e[0],e[1]],[e[1],e[2]],[e[2],e[3]],[e[3],e[0]]]}function G1(e,t,n,r){const i=r[0]-n[0],o=r[1]-n[1],a=Math.hypot(i,o);if(a<=0)return null;const s=((e-n[0])*i+(t-n[1])*o)/(a*a);return[Math.abs((e-n[0])*o-(t-n[1])*i)/a,Math.abs(s-.5)*a]}function W1(e){if(e.length===0)return null;const t=e.map(r=>r[0]),n=e.map(r=>r[1]);return Math.max(...t)-Math.min(...t)>Math.max(...n)-Math.min(...n)}function q1(e,t,n){try{const r=Number(n);if(!(r>0)||e.length<4||t.length<4)return null;const[i,o,a,s]=t,u=i+a/2,l=o+s/2;let h=null;for(const[p,f]of F1(e)){const m=G1(u,l,p,f);m!==null&&(h===null||m[0]<h[0])&&(h=m)}if(h===null)return null;const c=W1(e);return c===null?null:{distBord:h[0]/r,decalLat:h[1]/r,perpendiculaire:c!==a>s}}catch{return null}}function V1(e,t,n,r=U1,i=L1){const o=[];for(const[a,s]of t??[]){const u=q1(e,s,n);u!==null&&u.perpendiculaire&&(u.decalLat>r||u.distBord>i||o.push([u.decalLat,a]))}return o.length===0?null:(o.sort((a,s)=>a[0]-s[0]||a[1]-s[1]),o[0][1])}const _t=64,Lm=.5,H1=[.67,1.24];function Fm(e,t,n,r){const i=Math.max(0,t-r),o=Math.max(0,n-r),a=Math.min(e.width,t+r),s=Math.min(e.height,n+r),u=a-i,l=s-o;if(u<=0||l<=0)return null;const h=e.channels,c=new Uint8ClampedArray(u*l*3),p=r*r;for(let w=0;w<l;w++){const b=o+w,x=b-n;for(let M=0;M<u;M++){const v=i+M,I=v-t,T=(w*u+M)*3;if(I*I+x*x<=p){const k=(b*e.width+v)*h;c[T]=e.data[k],c[T+1]=e.data[k+1],c[T+2]=e.data[k+2]}else c[T]=255,c[T+1]=255,c[T+2]=255}}const f=jn({width:u,height:l,channels:3,data:c},_t,_t),m=_t*_t,y=new Float32Array(3*m);for(let w=0;w<m;w++)for(let b=0;b<3;b++)y[b*m+w]=f[w*3+b]/255;return y}function j1(e){return e[1]}const ui=[1,3,6],K1=.5;function Y1(e){if(e.length!==ui.length)return null;let t=0;for(let n=1;n<e.length;n++)e[n]>e[t]&&(t=n);return{denomination:ui[t],prob:e[t]}}function X1(e,t){return e.map((n,r)=>{const i=t[r]??null;return i!==null&&ui.includes(i.denomination)&&i.prob>=K1?{value:i.denomination,source:"cnn",conf:i.prob}:{value:n,source:null,conf:null}})}const Z1=2.25,li=3,Q1=1.15,J1=.5,e2=2.5,t2=.75,n2=2.25,r2=1.3,i2=.77;function ci(e,t){const n=Math.max(0,Math.max(e[0],t[0])-Math.min(e[0]+e[2],t[0]+t[2])),r=Math.max(0,Math.max(e[1],t[1])-Math.min(e[1]+e[3],t[1]+t[3]));return Math.hypot(n,r)}function o2(e){const t=Array.from(new Map(e.map(o=>[`${o[0]},${o[1]}`,o])).values());if(t.sort((o,a)=>o[0]-a[0]||o[1]-a[1]),t.length<=2)return t;const n=(o,a,s)=>(a[0]-o[0])*(s[1]-o[1])-(a[1]-o[1])*(s[0]-o[0]),r=[];for(const o of t){for(;r.length>=2&&n(r[r.length-2],r[r.length-1],o)<=0;)r.pop();r.push(o)}const i=[];for(const o of[...t].reverse()){for(;i.length>=2&&n(i[i.length-2],i[i.length-1],o)<=0;)i.pop();i.push(o)}return[...r.slice(0,-1),...i.slice(0,-1)]}function Gm(e,t,n){let r=!1;const i=n.length;for(let o=0;o<i;o+=1){const[a,s]=n[o],[u,l]=n[(o+1)%i];if(s>t!=l>t){const h=(u-a)*(t-s)/(l-s)+a;e<h&&(r=!r)}}return r}function a2(e,t,n){if(n.length>=3&&Gm(e,t,n))return 0;let r=Number.POSITIVE_INFINITY;const i=n.length;for(let o=0;o<i;o+=1){const[a,s]=n[o],[u,l]=n[i>1?(o+1)%i:o],h=u-a,c=l-s,p=h*h+c*c,f=p===0?0:Math.max(0,Math.min(1,((e-a)*h+(t-s)*c)/p));r=Math.min(r,Math.hypot(e-(a+f*h),t-(s+f*c)))}return r}function s2(e,t,n){const r=Math.max(Math.abs(e-(n[0]+n[2]/2))-n[2]/2,0),i=Math.max(Math.abs(t-(n[1]+n[3]/2))-n[3]/2,0);return Math.hypot(r,i)}function u2(e,t,n){const[r,i]=e,o=t[0]-r,a=t[1]-i;if(o===0&&a===0)return!1;const[s,u,l,h]=n;let c=0,p=1;const f=[[-o,r-s],[o,l-r],[-a,i-u],[a,h-i]];for(const[m,y]of f){if(m===0){if(y<0)return!1;continue}const w=y/m;if(m<0?c=Math.max(c,w):p=Math.min(p,w),c>p)return!1}return c>=p?!1:c>=.1&&p<=.95||p-c>=.15}const La=e=>e.box[3]/Math.max(1,e.box[2]),Yt=e=>La(e)>Q1,Jn=e=>La(e)>=r2||La(e)<=i2;function Fa(e){const[t,n,r,i]=e.box;if(r>=i){const a=7*i;return[t,n-a,r,i+2*a]}const o=7*r;return[t-o,n,r+2*o,i]}function Ga(e,t,n,r,i){const o=new Set(t),a=[...e.map((z,R)=>({box:[z[0],z[1],z[2],z[3]],kind:o.has(R)?"card":"tucked",src:["banner",R]})),...n.map((z,R)=>({box:[z[0],z[1],z[2],z[3]],kind:"wonder",src:["wonder",R]}))],s=e.map(()=>"player"),u=n.map(()=>"player");if(a.length===0)return{bannerOwner:s,wonderOwner:u,opponentFound:!1,hulls:[],hullBoxCounts:[],pointOwner:()=>"player",pointInside:()=>"none"};const l=a.map(z=>[z.box[0]+z.box[2]/2,z.box[1]+z.box[3]/2]);let h=a.filter(z=>z.kind!=="wonder").map(z=>Math.hypot(z.box[2],z.box[3])).sort((z,R)=>z-R);h.length===0&&(h=a.map(z=>Math.hypot(z.box[2],z.box[3])).sort((z,R)=>z-R));const c=h[Math.floor(h.length/2)],p=(Z1*c)**2,f=a.map((z,R)=>R),m=z=>{let R=z;for(;f[R]!==R;)f[R]=f[f[R]],R=f[R];return R},y=a.map((z,R)=>z.kind==="card"?R:-1).filter(z=>z>=0),w=a.map((z,R)=>z.kind!=="card"?R:-1).filter(z=>z>=0);for(let z=0;z<y.length;z+=1)for(let R=z+1;R<y.length;R+=1){const B=y[z],L=y[R],G=a[B],Z=a[L];if(Jn(G)&&Jn(Z)&&Yt(G)!==Yt(Z))continue;const ie=l[B][0]-l[L][0],te=l[B][1]-l[L][1],ye=ie*ie+te*te;let Me=ye<=p;!Me&&Jn(G)&&Jn(Z)&&Yt(G)===Yt(Z)&&ye<=(4*c)**2&&(Me=ci(Fa(G),Fa(Z))<=.5*c),Me&&(f[m(B)]=m(L))}for(let z=0;z<w.length;z+=1)for(let R=z+1;R<w.length;R+=1){const B=w[z],L=w[R];ci(a[B].box,a[L].box)<=t2*c&&(f[m(B)]=m(L))}const b=new Map;for(const z of w){const R=m(z);b.set(R,[...b.get(R)??[],z])}const x=new Map;for(const z of y){const R=m(z);x.set(R,[...x.get(R)??[],z])}for(const z of b.values()){const R=z.filter(Z=>a[Z].kind==="wonder"&&Jn(a[Z])).map(Z=>Yt(a[Z])),B=R.length>0?R.filter(Boolean).length*2>R.length:null,L=[];for(const[Z,ie]of x){let te=Number.POSITIVE_INFINITY;for(const ze of z)for(const De of ie)te=Math.min(te,ci(a[ze].box,a[De].box));if(te>n2*c)continue;const Me=ie.filter(ze=>Yt(a[ze])).length/ie.length>=.5;B!==null&&Me!==B||L.push([Z,te,Me])}if(L.length===0)continue;const G=new Set(L.map(Z=>Z[2]));if(L.length>=2&&G.size===1&&B!==null){const Z=L[0][0];for(const[ie]of L.slice(1))f[m(ie)]=m(Z);f[m(z[0])]=m(Z)}else{const Z=L.reduce((ie,te)=>te[1]<ie[1]?te:ie);f[m(z[0])]=m(Z[0])}}let M=new Map;for(let z=0;z<a.length;z+=1){const R=m(z);M.set(R,[...M.get(R)??[],z])}const v=a.map((z,R)=>z.kind==="wonder"?R:-1).filter(z=>z>=0);if(v.length>0){const z=(B,L)=>{const[G,Z,ie,te]=Fa(a[B]),[ye,Me,ze,De]=a[L].box,lt=Math.max(0,Math.min(G+ie,ye+ze)-Math.max(G,ye)),Be=Math.max(0,Math.min(Z+te,Me+De)-Math.max(Z,Me));return lt*Be>=.9*a[B].box[2]*a[B].box[3]},R=new Map;for(let B=0;B<a.length;B+=1)if(!(a[B].kind!=="card"||!Jn(a[B])))for(const L of v){const G=ci(a[B].box,a[L].box);if(G<=.8*c&&Yt(a[B])!==Yt(a[L])&&z(B,L)){const Z=R.get(L);(!Z||G<Z[1])&&R.set(L,[B,G])}}for(const[B,[L]]of R){const G=m(B);for(const[Z,ie]of M){const te=ie.indexOf(L);if(te>=0&&Z!==G){ie.splice(te,1),M.set(G,[...M.get(G)??[],L]),a[L].kind="tucked";break}}}M=new Map([...M].filter(([,B])=>B.length>0))}const I=z=>z.filter(R=>a[R].kind==="card").length,T=z=>{const R=z.filter(B=>a[B].kind==="card"||a[B].kind==="wonder");return R.length===0?null:R.filter(B=>Yt(a[B])).length/R.length},k=z=>[z.reduce((R,B)=>R+l[B][0],0)/z.length,z.reduce((R,B)=>R+l[B][1],0)/z.length],S=[i[0]/2,i[1]/2],A=[...M.values()].sort((z,R)=>{const B=I(z),L=I(R);if(B!==L)return L-B;const G=Math.hypot(k(z)[0]-S[0],k(z)[1]-S[1]),Z=Math.hypot(k(R)[0]-S[0],k(R)[1]-S[1]);return G-Z}),N=k(A[0]),U=T(A[0]),V=A.map((z,R)=>{if(R===0||I(z)<li)return"player";const B=T(z),L=B!==null&&U!==null&&Math.abs(B-U)>=J1,G=k(z),Z=r.some(ie=>u2(N,G,ie));return L||Z?"opponent":"player"});if(!V.includes("opponent")){const z=B=>B.reduce((L,G)=>L+(a[G].kind==="wonder"?1:0),0);let R=V.map((B,L)=>L).filter(B=>B>0&&(I(A[B])>=li||z(A[B])>=2));if(R.reduce((B,L)=>B+z(A[L]),0)<1&&(R=[]),R.length>0&&(I(A[0])<2*li||R.reduce((B,L)=>B+I(A[L]),0)<2*li)&&(R=[]),R.length>0){const B=new Map(R.map(Z=>[Z,k(A[Z])])),L=(Z,ie)=>(Z[0]-ie[0])**2+(Z[1]-ie[1])**2;if(R.every((Z,ie)=>R.slice(ie+1).every(te=>L(B.get(Z),B.get(te))<Math.min(L(B.get(Z),N),L(B.get(te),N)))))for(const Z of R)V[Z]="opponent"}}const F=[],O=[];let H=!1;A.forEach((z,R)=>{const B=V[R];B==="opponent"&&(H=!0);const L=[],G=[];for(const Z of z){const[ie,te,ye,Me]=a[Z].box;L.push([ie,te],[ie+ye,te],[ie,te+Me],[ie+ye,te+Me]),G.push(a[Z].box);const[ze,De]=a[Z].src;ze==="banner"?s[De]=B:u[De]=B}F.push([B,o2(L)]),O.push([B,G])});const X=(z,R,B)=>Math.min(...O[B][1].map(L=>s2(z,R,L))),J=(z,R)=>F.map(([,B],L)=>B.length>=3&&Gm(z,R,B)?L:-1).filter(B=>B>=0),he=(z,R)=>{if(F.length===0)return"player";const B=c>0?e2*c:Number.POSITIVE_INFINITY,L=J(z,R);if(L.length>0){const ie=L.reduce((te,ye)=>X(z,R,ye)<X(z,R,te)?ye:te);return F[ie][0]}let G=-1,Z=Number.POSITIVE_INFINITY;return F.forEach(([,ie],te)=>{const ye=a2(z,R,ie);ye<Z&&(G=te,Z=ye)}),G>=0&&Z<=B?F[G][0]:"none"},W=(z,R)=>{if(F.length===0)return"none";const B=J(z,R);if(B.length===0)return"none";const L=B.reduce((G,Z)=>X(z,R,Z)<X(z,R,G)?Z:G);return F[L][0]};return{bannerOwner:s,wonderOwner:u,opponentFound:H,hulls:F,hullBoxCounts:O.map(([,z])=>z.length),pointOwner:he,pointInside:W}}const l2=3;function c2(e,t=l2){const n=e.length,r=Array.from({length:n},(a,s)=>s),i=a=>{for(;r[a]!==a;)r[a]=r[r[a]],a=r[a];return a};for(let a=0;a<n;a+=1)for(let s=a+1;s<n;s+=1){const u=e[a],l=e[s],h=Number(u.center[0]),c=Number(u.center[1]),p=Number(l.center[0]),f=Number(l.center[1]),m=Number(u.radius??0),y=Number(l.radius??0);![h,c,p,f,m,y].every(Number.isFinite)||m<=0||y<=0||Math.hypot(h-p,c-f)<=t*(m+y)&&(r[i(a)]=i(s))}const o=new Map;for(let a=0;a<n;a+=1){const s=i(a);o.has(s)||o.set(s,[]),o.get(s).push(a)}return[...o.values()]}function d2(e,t,n){const r=Number(n[0]),i=Number(n[1]),o=Number(n[2]),a=Number(n[3]),s=Math.max(Math.min(r,o)-e,0,e-Math.max(r,o)),u=Math.max(Math.min(i,a)-t,0,t-Math.max(i,a));return Math.hypot(s,u)}function h2(e,t,n,r){const i=()=>e.filter(o=>t.pointOwner(Number(o.center[0]),Number(o.center[1]))===n);try{const o=new Set(i());if(o.size===0)return[];const a=c2(e),s=[];for(const l of a){const h=l.map(M=>e[M]),c=h.filter(M=>o.has(M));if(c.length===0)continue;let p=0,f=0,m=0;for(const M of h){const v=Number(M.center[0]),I=Number(M.center[1]);f+=v,m+=I,t.pointInside(v,I)===n&&(p+=1)}const y=f/h.length,w=m/h.length,b=r&&r.length>0?Math.min(...r.map(M=>d2(y,w,M))):0,x=c.reduce((M,v)=>M+(Number(v.denomination??0)||0),0);s.push({miens:c,inside:p,dPiste:b,valeur:x})}return s.length===0?[]:s.length===1?s[0].miens:s.reduce((l,h)=>{const c=[l.inside>0?1:0,l.inside,l.dPiste,l.valeur],p=[h.inside>0?1:0,h.inside,h.dPiste,h.valeur];for(let f=0;f<4;f+=1){if(p[f]>c[f])return h;if(p[f]<c[f])return l}return l}).miens}catch{try{return i()}catch{return[...e]}}}const p2=1280,f2=80,m2=3,g2=3,y2=.3,w2=2.4,_2=1,b2=5.2,x2=5;function Wa(e){const t=e.filter(r=>r&&r.length>=4).map(r=>Math.min(r[2],r[3])).sort((r,i)=>r-i),n=t.length;return n===0?0:n%2?t[(n-1)/2]:.5*(t[n/2-1]+t[n/2])}function $2(e,t,n){const r=Math.min(e,t),i=Math.max(e,t);return!(n>0)||!(r>0)?!1:r/n>=y2&&r/n<=w2&&i/n>=_2&&i/n<=b2&&i/r<=x2}function v2(e,t,n){const r=Math.max(e,t);return!(r>0)||!(n>0)?!1:n*p2/r<f2}function S2(e,t){if(t.length===0)return e.slice();const n=e.map(r=>{const i=r.poly.map(s=>s[0]),o=r.poly.map(s=>s[1]),a=Math.max(1,i.length);return{hull:r,cx:i.reduce((s,u)=>s+u,0)/a,cy:o.reduce((s,u)=>s+u,0)/a,extra:[]}});if(n.length===0)return e.slice();for(const r of t){const i=Number(r[0]),o=Number(r[1]),a=Number(r[2]),s=Number(r[3]);if(![i,o,a,s].every(Number.isFinite))continue;const u=i+a/2,l=o+s/2;let h=n[0],c=1/0;for(const p of n){const f=(u-p.cx)**2+(l-p.cy)**2;f<c&&(c=f,h=p)}h.extra.push([i,o],[i+a,o+s])}return n.map(r=>r.extra.length===0?r.hull:{...r.hull,poly:[...r.hull.poly.map(i=>[i[0],i[1]]),...r.extra]})}function Wm(e,t,n,r,i=[]){const o=Wa(n);if(!v2(e,t,o))return[];const a=r.filter(l=>l.n>=g2&&l.poly.length>0).slice().sort((l,h)=>h.n-l.n).slice(0,2),s=Math.round(o*m2),u=[];for(const l of S2(a,i)){const h=l.poly.map(w=>w[0]),c=l.poly.map(w=>w[1]);if(h.length===0)continue;const p=Math.max(0,Math.trunc(Math.min(...h))-s),f=Math.max(0,Math.trunc(Math.min(...c))-s),m=Math.min(e,Math.trunc(Math.max(...h))+s),y=Math.min(t,Math.trunc(Math.max(...c))+s);m>p&&y>f&&u.push([p,f,m,y])}return u}function M2(e,t,n){if(!e||e.length<4)return null;const[r,i,o,a]=[e[0],e[1],e[2],e[3]];return $2(o,a,n)?[Math.round(r+t[0]),Math.round(i+t[1]),Math.round(o),Math.round(a)]:null}function I2(e,t,n,r,i){return Wm(e,t,n,r,i)}function E2(e,t){var s,u,l,h;const[n,r,i,o]=t,a=[];for(const c of e){const p=Number((s=c.box)==null?void 0:s[0]),f=Number((u=c.box)==null?void 0:u[1]),m=Number((l=c.box)==null?void 0:l[2]),y=Number((h=c.box)==null?void 0:h[3]);[p,f,m,y].every(Number.isFinite)&&(p+m<n||p>i||f+y<r||f>o||a.push({...c,box:[Math.round(p-n),Math.round(f-r),Math.round(m),Math.round(y)]}))}return a}function T2(e){const t=[];for(const n of e){const r=n==null?void 0:n.boundingBox;if(!r||!Number.isFinite(r.width)||!Number.isFinite(r.height))continue;const i=r.x+r.width/2,o=r.y+r.height/2;let a=!1;for(const s of t){if(n.id&&s.id===n.id){a=!0;break}const u=s.boundingBox,l=u.x+u.width/2,h=u.y+u.height/2,c=.5*Math.min(u.width,u.height);if((i-l)**2+(o-h)**2<c*c){a=!0;break}}a||t.push(n)}return t}function qm(e,t){return{x:Math.round(e.x+t[0]),y:Math.round(e.y+t[1]),width:Math.round(e.width),height:Math.round(e.height)}}const k2=1.1,C2=3.2,A2=20,R2=.5,O2=1280,N2=.18,z2=28,B2=.3;function P2(e){const t=Math.min(...e),n=Math.max(...e);let r=(t+n)/2;for(let a=0;a<30;a++){const s=e.filter(h=>h<=r),u=e.filter(h=>h>r);if(s.length===0||u.length===0)return[e.map((h,c)=>c)];const l=(s.reduce((h,c)=>h+c,0)/s.length+u.reduce((h,c)=>h+c,0)/u.length)/2;if(Math.abs(l-r)<1)break;r=l}const i=[],o=[];return e.forEach((a,s)=>(a<=r?i:o).push(s)),[i,o]}function D2(e,t,n=k2){const[r,i]=t;if(e.length<3||r<=0||i<=0)return[];const o=e.map(l=>l[0]+l[2]/2),a=e.map(l=>l[1]+l[3]/2),s=Math.max(...o)-Math.min(...o)>Math.max(...a)-Math.min(...a)?o:a,u=[];for(const l of P2(s)){if(l.length===0)continue;const h=l.map(A=>e[A]),c=h.map(A=>Math.min(A[2],A[3])).sort((A,N)=>A-N),p=c[Math.trunc(c.length/2)],f=C2*p,m=Math.max(0,Math.min(...h.map(A=>A[0]))-f),y=Math.max(0,Math.min(...h.map(A=>A[1]))-f),w=Math.min(r,Math.max(...h.map(A=>A[0]+A[2]))+f),b=Math.min(i,Math.max(...h.map(A=>A[1]+A[3]))+f),x=Math.max(w-m,b-y);if(x<=0)continue;const M=R2*p*O2/x,v=M>0?Math.max(1,Math.ceil(A2/M)):1;if(v===1){u.push([Math.trunc(m),Math.trunc(y),Math.trunc(w),Math.trunc(b)]);continue}const I=w-m>=b-y,k=(I?w-m:b-y)/v,S=k*(1+N2);for(let A=0;A<v;A++){let N=(I?m:y)+A*k-(S-k)/2;N=Math.max(I?m:y,N);const U=Math.min(I?w:b,N+S);u.push(I?[Math.trunc(N),Math.trunc(y),Math.trunc(U),Math.trunc(b)]:[Math.trunc(m),Math.trunc(N),Math.trunc(w),Math.trunc(U)])}}return u.filter(([l,h,c,p])=>Math.max(r,i)/Math.max(1,Math.max(c-l,p-h))>=n)}function U2(e,t,n,r=z2){const[i,o]=n,a=e;for(const[s,u,l,h]of t){const c=(s+l)/2+i,p=(u+h)/2+o;a.some(([m,y,w,b])=>{const x=c-(m+w)/2,M=p-(y+b)/2;return Math.hypot(x,M)<=r})||a.push([s+i,u+o,l+i,h+o])}return a}function L2(e,t,n,r=B2){for(const i of n){const o=r*Math.min(i[2],i[3]);if(i[0]-o<=e&&e<=i[0]+i[2]+o&&i[1]-o<=t&&t<=i[1]+i[3]+o)return!0}return!1}function F2(e,t,n){return n.some(([r,i,o,a])=>r<=e&&e<=o&&i<=t&&t<=a)}function G2(e,t,n,r){return n.length===0?!1:F2(e,t,n)&&!L2(e,t,r)}const Vm=4,Hm=8,di=5,Rn="base-game rule";function Pt(e,t){return{code:e,message:t,severity:"warning"}}function qa(e){const t=new Set,n=new Set;for(const r of e)t.has(r)&&n.add(r),t.add(r);return[...n].sort()}function W2(e,t=""){const n=e.filter(a=>!!a),r=t||"a player",i=[];n.length>Vm&&i.push(Pt("TOO_MANY_WONDERS",`${r}: ${n.length} wonders recognised, but a player builds at most ${Vm} (${Rn}) — at least one reading is wrong. Check the wonder list in the review; a card seen at an angle can be named as a wonder.`));const o=qa(n);return o.length>0&&i.push(Pt("DUPLICATE_WONDER",`${r}: wonder(s) counted twice — ${o.join(", ")}. Only one copy of each wonder exists (${Rn}), so one of the two readings is wrong.`)),i}function q2(e){const t=[],n=Object.entries(e).map(([i,o])=>[i,new Set(o.filter(a=>!!a))]),r=Object.values(e).reduce((i,o)=>i+o.filter(Boolean).length,0);r>Hm&&t.push(Pt("TOO_MANY_WONDERS_IN_PLAY",`${r} wonders recognised across both cities, but only ${Hm} are in play (${Rn}) — at least one reading is wrong.`));for(let i=0;i<n.length;i++){const[o,a]=n[i];for(let s=i+1;s<n.length;s++){const[u,l]=n[s],h=[...a].filter(c=>l.has(c)).sort();h.length>0&&t.push(Pt("WONDER_IN_BOTH_CITIES",`wonder(s) assigned to both cities at once (${o} and ${u}): ${h.join(", ")} — the city split misread one of them.`))}}return t}function V2(e,t=null){const n=[],r=Object.values(e).flatMap(o=>o.filter(a=>!!a));r.length>di&&n.push(Pt("TOO_MANY_TOKENS",`${r.length} Progress tokens claimed by the cities, but only ${di} are in play (${Rn}) — reserve tokens sitting on the board were probably counted as owned.`));const i=qa(r);if(i.length>0&&n.push(Pt("DUPLICATE_TOKEN",`Progress token(s) counted twice: ${i.join(", ")} — only one copy of each token exists (${Rn}).`)),t!==null){const o=t.filter(Boolean),a=r.length+o.length;a!==di&&n.push(Pt("TOKEN_COUNT_MISMATCH",`${r.length} token(s) in the cities + ${t.length} in the reserve = ${a}, but exactly ${di} are in play (${Rn}) — one is missing or one was counted twice.`));const s=new Set(o),u=[...new Set(r.filter(l=>s.has(l)))].sort();u.length>0&&n.push(Pt("TOKEN_IN_CITY_AND_RESERVE",`token(s) seen both in a city and in the reserve: ${u.join(", ")} — the board-token exclusion did not fire.`))}return n}function H2(e,t=""){const n=t||"a player",r=[],i=e.filter(a=>!a).length;i>0&&r.push(Pt("UNNAMED_GUILD",`${n}: ${i} guild(s) detected but not identified — their points cannot be computed. Name them in the review.`));const o=qa(e.filter(a=>!!a));return o.length>0&&r.push(Pt("DUPLICATE_GUILD",`${n}: guild(s) counted twice — ${o.join(", ")}. Only one copy of each guild exists (${Rn}).`)),r}const j2=[{id:"merchants-guild",name:"Merchants Guild",nameFr:"Guilde des commerçants",color:"guild",age:3,victoryPoints:0,variableScoring:"merchantsGuild",cost:{clay:1,wood:1,glass:1,papyrus:1}},{id:"shipowners-guild",name:"Shipowners Guild",nameFr:"Guilde des armateurs",color:"guild",age:3,victoryPoints:0,variableScoring:"shipownersGuild",cost:{clay:2,glass:1,papyrus:1}},{id:"builders-guild",name:"Builders Guild",nameFr:"Guilde des bâtisseurs",color:"guild",age:3,victoryPoints:0,variableScoring:"buildersGuild",cost:{stone:2,clay:1,wood:1,glass:1}},{id:"magistrates-guild",name:"Magistrates Guild",nameFr:"Guilde des magistrats",color:"guild",age:3,victoryPoints:0,variableScoring:"magistratesGuild",cost:{wood:2,clay:1,papyrus:1}},{id:"scientists-guild",name:"Scientists Guild",nameFr:"Guilde des scientifiques",color:"guild",age:3,victoryPoints:0,variableScoring:"scientistsGuild",cost:{wood:2,clay:2}},{id:"tacticians-guild",name:"Tacticians Guild",nameFr:"Guilde des tacticiens",color:"guild",age:3,victoryPoints:0,variableScoring:"tacticiansGuild",cost:{stone:2,clay:1,papyrus:1}},{id:"moneylenders-guild",name:"Moneylenders Guild",nameFr:"Guilde des usuriers",color:"guild",age:3,victoryPoints:0,variableScoring:"moneylendersGuild",cost:{stone:2,wood:2}}],K2=[{id:"lumber-yard",name:"Lumber Yard",nameFr:"Chantier",color:"raw",age:1,victoryPoints:0},{id:"logging-camp",name:"Logging Camp",nameFr:"Exploitation",color:"raw",age:1,victoryPoints:0,coinCost:1},{id:"clay-pool",name:"Clay Pool",nameFr:"Bassin argileux",color:"raw",age:1,victoryPoints:0},{id:"clay-pit",name:"Clay Pit",nameFr:"Cavité",color:"raw",age:1,victoryPoints:0,coinCost:1},{id:"quarry",name:"Quarry",nameFr:"Gisement",color:"raw",age:1,victoryPoints:0},{id:"stone-pit",name:"Stone Pit",nameFr:"Mine",color:"raw",age:1,victoryPoints:0,coinCost:1},{id:"glassworks",name:"Glassworks",nameFr:"Verrerie",color:"manufactured",age:1,victoryPoints:0,coinCost:1},{id:"press",name:"Press",nameFr:"Presse",color:"manufactured",age:1,victoryPoints:0,coinCost:1},{id:"theater",name:"Theater",nameFr:"Théâtre",color:"civilian",age:1,victoryPoints:3},{id:"altar",name:"Altar",nameFr:"Autel",color:"civilian",age:1,victoryPoints:3,providesChain:"moon"},{id:"baths",name:"Baths",nameFr:"Bains",color:"civilian",age:1,victoryPoints:3,providesChain:"drop",cost:{stone:1}},{id:"pharmacist",name:"Pharmacist",nameFr:"Officine",color:"scientific",age:1,victoryPoints:0,scienceSymbol:"mortar",providesChain:"mortar-chain",cost:{glass:2}},{id:"apothecary",name:"Apothecary",nameFr:"Apothicaire",color:"scientific",age:1,victoryPoints:1,scienceSymbol:"wheel",providesChain:"wheel-chain",cost:{glass:1}},{id:"workshop",name:"Workshop",nameFr:"Atelier",color:"scientific",age:1,victoryPoints:1,scienceSymbol:"pendulum",providesChain:"pendulum-chain",cost:{papyrus:1}},{id:"scriptorium",name:"Scriptorium",nameFr:"Scriptorium",color:"scientific",age:1,victoryPoints:0,scienceSymbol:"inkwell",providesChain:"inkwell-chain",coinCost:2},{id:"stone-reserve",name:"Stone Reserve",nameFr:"Dépôt de pierre",color:"commercial",age:1,victoryPoints:0,coinCost:3},{id:"clay-reserve",name:"Clay Reserve",nameFr:"Dépôt d'argile",color:"commercial",age:1,victoryPoints:0,coinCost:3},{id:"wood-reserve",name:"Wood Reserve",nameFr:"Dépôt de bois",color:"commercial",age:1,victoryPoints:0,coinCost:3},{id:"tavern",name:"Tavern",nameFr:"Taverne",color:"commercial",age:1,victoryPoints:0,providesChain:"jug"},{id:"guard-tower",name:"Guard Tower",nameFr:"Tour de garde",color:"military",age:1,victoryPoints:0,shields:1},{id:"stable",name:"Stable",nameFr:"Écuries",color:"military",age:1,victoryPoints:0,shields:1,providesChain:"horseshoe",cost:{wood:1}},{id:"garrison",name:"Garrison",nameFr:"Caserne",color:"military",age:1,victoryPoints:0,shields:1,providesChain:"sword",cost:{clay:1}},{id:"palisade",name:"Palisade",nameFr:"Palissade",color:"military",age:1,victoryPoints:0,shields:1,providesChain:"tower",coinCost:2}],Y2=[{id:"sawmill",name:"Sawmill",nameFr:"Scierie",color:"raw",age:2,victoryPoints:0,coinCost:2},{id:"brickyard",name:"Brickyard",nameFr:"Briqueterie",color:"raw",age:2,victoryPoints:0,coinCost:2},{id:"shelf-quarry",name:"Shelf Quarry",nameFr:"Carrière",color:"raw",age:2,victoryPoints:0,coinCost:2},{id:"glass-blower",name:"Glass-Blower",nameFr:"Soufflerie",color:"manufactured",age:2,victoryPoints:0,coinCost:2},{id:"drying-room",name:"Drying Room",nameFr:"Séchoir",color:"manufactured",age:2,victoryPoints:0,coinCost:2},{id:"courthouse",name:"Courthouse",nameFr:"Tribunal",color:"civilian",age:2,victoryPoints:5,cost:{wood:2,glass:1}},{id:"statue",name:"Statue",nameFr:"Statue",color:"civilian",age:2,victoryPoints:4,providesChain:"column",chainFrom:"moon",cost:{clay:2}},{id:"temple",name:"Temple",nameFr:"Temple",color:"civilian",age:2,victoryPoints:4,providesChain:"sun",chainFrom:"drop",cost:{wood:1,papyrus:1}},{id:"aqueduct",name:"Aqueduct",nameFr:"Aqueduc",color:"civilian",age:2,victoryPoints:5,cost:{stone:3}},{id:"rostrum",name:"Rostrum",nameFr:"Rostres",color:"civilian",age:2,victoryPoints:4,providesChain:"horseshoe",cost:{stone:1,wood:1}},{id:"school",name:"School",nameFr:"École",color:"scientific",age:2,victoryPoints:1,scienceSymbol:"wheel",providesChain:"wheel-chain-2",cost:{wood:1,papyrus:2}},{id:"laboratory",name:"Laboratory",nameFr:"Laboratoire",color:"scientific",age:2,victoryPoints:1,scienceSymbol:"pendulum",providesChain:"pendulum-chain-2",cost:{wood:1,glass:2}},{id:"library",name:"Library",nameFr:"Bibliothèque",color:"scientific",age:2,victoryPoints:2,scienceSymbol:"inkwell",chainFrom:"inkwell-chain",cost:{stone:1,wood:1,glass:1}},{id:"dispensary",name:"Dispensary",nameFr:"Dispensaire",color:"scientific",age:2,victoryPoints:2,scienceSymbol:"mortar",chainFrom:"mortar-chain",cost:{clay:2,stone:1}},{id:"forum",name:"Forum",nameFr:"Forum",color:"commercial",age:2,victoryPoints:0,providesChain:"barrel",coinCost:3,cost:{clay:1}},{id:"caravansery",name:"Caravansery",nameFr:"Caravansérail",color:"commercial",age:2,victoryPoints:0,coinCost:2,cost:{glass:1,papyrus:1}},{id:"customs-house",name:"Customs House",nameFr:"Douanes",color:"commercial",age:2,victoryPoints:0,coinCost:4},{id:"brewery",name:"Brewery",nameFr:"Brasserie",color:"commercial",age:2,victoryPoints:0,providesChain:"barrel-2"},{id:"horse-breeders",name:"Horse Breeders",nameFr:"Haras",color:"military",age:2,victoryPoints:0,shields:1,chainFrom:"horseshoe",cost:{clay:1,wood:1}},{id:"barracks",name:"Barracks",nameFr:"Baraquements",color:"military",age:2,victoryPoints:0,shields:1,chainFrom:"sword",coinCost:3},{id:"archery-range",name:"Archery Range",nameFr:"Champ de tir",color:"military",age:2,victoryPoints:0,shields:2,providesChain:"target",cost:{stone:1,wood:1,papyrus:1}},{id:"parade-ground",name:"Parade Ground",nameFr:"Place d'armes",color:"military",age:2,victoryPoints:0,shields:2,providesChain:"mask",cost:{clay:2,glass:1}},{id:"walls",name:"Walls",nameFr:"Muraille",color:"military",age:2,victoryPoints:0,shields:2,cost:{stone:2}}],X2=[{id:"pantheon",name:"Pantheon",nameFr:"Panthéon",color:"civilian",age:3,victoryPoints:6,chainFrom:"sun",cost:{clay:1,wood:1,papyrus:2}},{id:"gardens",name:"Gardens",nameFr:"Jardins",color:"civilian",age:3,victoryPoints:6,chainFrom:"column",cost:{clay:2,wood:2}},{id:"town-hall",name:"Town Hall",nameFr:"Hôtel de ville",color:"civilian",age:3,victoryPoints:7,cost:{stone:3,wood:2}},{id:"palace",name:"Palace",nameFr:"Palace",color:"civilian",age:3,victoryPoints:7,cost:{clay:1,stone:1,wood:1,glass:2}},{id:"senate",name:"Senate",nameFr:"Sénat",color:"civilian",age:3,victoryPoints:5,chainFrom:"horseshoe",cost:{clay:2,stone:1,papyrus:1}},{id:"obelisk",name:"Obelisk",nameFr:"Obélisque",color:"civilian",age:3,victoryPoints:5,cost:{stone:2,glass:1}},{id:"academy",name:"Academy",nameFr:"Académie",color:"scientific",age:3,victoryPoints:3,scienceSymbol:"sundial",cost:{stone:1,wood:1,glass:2}},{id:"study",name:"Study",nameFr:"Étude",color:"scientific",age:3,victoryPoints:3,scienceSymbol:"sundial",cost:{wood:2,glass:1,papyrus:1}},{id:"university",name:"University",nameFr:"Université",color:"scientific",age:3,victoryPoints:2,scienceSymbol:"globe",chainFrom:"wheel-chain-2",cost:{clay:1,glass:1,papyrus:1}},{id:"observatory",name:"Observatory",nameFr:"Observatoire",color:"scientific",age:3,victoryPoints:2,scienceSymbol:"globe",chainFrom:"pendulum-chain-2",cost:{stone:1,papyrus:2}},{id:"chamber-of-commerce",name:"Chamber of Commerce",nameFr:"Chambre de commerce",color:"commercial",age:3,victoryPoints:3,variableScoring:"chamberOfCommerce",cost:{papyrus:2}},{id:"port",name:"Port",nameFr:"Port",color:"commercial",age:3,victoryPoints:3,variableScoring:"port",cost:{wood:1,glass:1,papyrus:1}},{id:"armory",name:"Armory",nameFr:"Armurerie",color:"commercial",age:3,victoryPoints:3,variableScoring:"armory",cost:{stone:2,glass:1}},{id:"lighthouse",name:"Lighthouse",nameFr:"Phare",color:"commercial",age:3,victoryPoints:3,variableScoring:"lighthouse",chainFrom:"jug",cost:{clay:2,glass:1}},{id:"arena",name:"Arena",nameFr:"Arène",color:"commercial",age:3,victoryPoints:3,variableScoring:"arena",chainFrom:"barrel-2",cost:{clay:1,stone:1,wood:1}},{id:"pretorium",name:"Pretorium",nameFr:"Prétoire",color:"military",age:3,victoryPoints:0,shields:3,coinCost:8},{id:"arsenal",name:"Arsenal",nameFr:"Arsenal",color:"military",age:3,victoryPoints:0,shields:3,cost:{clay:3,wood:2}},{id:"fortifications",name:"Fortifications",nameFr:"Fortifications",color:"military",age:3,victoryPoints:0,shields:2,chainFrom:"tower",cost:{stone:2,clay:1,papyrus:1}},{id:"siege-workshop",name:"Siege Workshop",nameFr:"Atelier de siège",color:"military",age:3,victoryPoints:0,shields:2,chainFrom:"target",cost:{wood:3,glass:1}},{id:"circus",name:"Circus",nameFr:"Cirque",color:"military",age:3,victoryPoints:0,shields:2,chainFrom:"mask",cost:{clay:2,stone:2}}],Z2=[...K2,...Y2,...X2,...j2];Object.fromEntries(Z2.map(e=>[e.id,e]));const Q2=Object.fromEntries([{id:"the-appian-way",name:"The Appian Way",nameFr:"La Via Appia",victoryPoints:3,description:"The opponent loses 3 coins. Take another turn. Once built, repeated discards are not affected. Worth 3 victory points."},{id:"circus-maximus",name:"Circus Maximus",nameFr:"Le Circus Maximus",victoryPoints:3,shields:1,description:"Destroy one grey (manufactured) card the opponent has built. Provides 1 shield. Worth 3 victory points."},{id:"the-colossus",name:"The Colossus",nameFr:"Le Colosse",victoryPoints:3,shields:2,description:"Provides 2 shields. Worth 3 victory points."},{id:"the-great-library",name:"The Great Library",nameFr:"La Grande Bibliothèque",victoryPoints:4,description:"Randomly draw 3 of the Progress tokens discarded at game setup and keep one. Worth 4 victory points."},{id:"the-great-lighthouse",name:"The Great Lighthouse",nameFr:"Le Grand Phare",victoryPoints:4,description:"Once built, the owner may take any raw or manufactured good of choice each turn (production effect). Worth 4 victory points."},{id:"the-hanging-gardens",name:"The Hanging Gardens",nameFr:"Les Jardins Suspendus",victoryPoints:3,description:"Gain 6 coins. Take another turn. Worth 3 victory points."},{id:"the-mausoleum",name:"The Mausoleum",nameFr:"Le Mausolée",victoryPoints:2,description:"Build, for free, any one card from the discard pile. Worth 2 victory points."},{id:"piraeus",name:"Piraeus",nameFr:"Le Pirée",victoryPoints:2,description:"Once built, the owner may take any one manufactured good (glass or papyrus) of choice each turn. Take another turn. Worth 2 victory points."},{id:"the-pyramids",name:"The Pyramids",nameFr:"Les Pyramides",victoryPoints:9,description:"Worth 9 victory points."},{id:"the-sphinx",name:"The Sphinx",nameFr:"Le Sphinx",victoryPoints:6,description:"Take another turn. Worth 6 victory points."},{id:"the-statue-of-zeus",name:"The Statue of Zeus",nameFr:"La Statue de Zeus",victoryPoints:3,shields:1,description:"Destroy one brown (raw) card the opponent has built. Provides 1 shield. Worth 3 victory points."},{id:"the-temple-of-artemis",name:"The Temple of Artemis",nameFr:"Le Temple d'Artémis",victoryPoints:0,description:"Gain 12 coins. Take another turn. Worth 0 victory points."}].map(e=>[e.id,e]));Object.fromEntries([{id:"agriculture",name:"Agriculture",nameFr:"Agriculture",victoryPoints:4,description:"Gain 6 coins immediately. Worth 4 victory points at game end."},{id:"architecture",name:"Architecture",nameFr:"Architecture",description:"Any future Wonder constructed by the owner costs 2 fewer resources of the owner's choice."},{id:"economy",name:"Economy",nameFr:"Économie",description:"When the opponent uses the trading-cost coins (pays the bank to buy goods), the owner receives those coins instead."},{id:"law",name:"Law",nameFr:"Loi",variableScoring:"law",description:"Grants one science symbol, counting toward the six-symbol scientific victory and toward pairs of identical symbols."},{id:"masonry",name:"Masonry",nameFr:"Maçonnerie",description:"Any future blue (civilian) building constructed by the owner costs 2 fewer resources of the owner's choice."},{id:"mathematics",name:"Mathematics",nameFr:"Mathématiques",variableScoring:"mathematics",description:"Worth 3 victory points at game end for EACH Progress token the owner possesses (including this one)."},{id:"philosophy",name:"Philosophy",nameFr:"Philosophie",victoryPoints:7,description:"Worth 7 victory points at game end."},{id:"strategy",name:"Strategy",nameFr:"Stratégie",description:"Whenever the owner builds a red (military) building, it provides 1 additional shield."},{id:"theology",name:"Theology",nameFr:"Théologie",description:"Every future Wonder built by the owner grants an extra turn."},{id:"urbanism",name:"Urbanism",nameFr:"Urbanisme",description:"Gain 6 coins immediately. When the owner builds a card for free via a chain link, they also gain 4 coins."}].map(e=>[e.id,e]));const jm=.2,J2=.3,Km=.25;function ex(e,t,n){if(t.height<=0)return!1;const r=t.width/t.height;if(Math.abs(Math.log(r))<=Km)return!1;const i=e.x+e.width,o=e.y+e.height;for(const a of n){const s=a.box;if(!s||s.length<4||s[3]<=0)continue;const u=s[0]+s[2]/2,l=s[1]+s[3]/2;if(!(u>=e.x&&u<=i&&l>=e.y&&l<=o))continue;const h=s[2]/s[3];if(!(Math.abs(Math.log(h))<=Km)&&r>1==h>1)return!0}return!1}async function tx(e,t,n){const[r,i,o,a]=t;if(o<=0||a<=0)return null;const s=Math.round(o*jm),u=Math.round(a*jm),l=Math.max(0,Math.round(r-s)),h=Math.max(0,Math.round(i-u)),c=Math.min(e.width,Math.round(r+o+s)),p=Math.min(e.height,Math.round(i+a+u)),f=c-l,m=p-h;if(f<=0||m<=0)return null;const y=e.channels,w=new Uint8ClampedArray(f*m*y);for(let M=0;M<m;M++){const v=((h+M)*e.width+l)*y;w.set(e.data.subarray(v,v+f*y),M*f*y)}const b={width:f,height:m,channels:y,data:w};let x=null;for(let M=0;M<4;M++){const v=M===0?b:jt(b,M),I=v.width,T=I-Math.floor(J2*I),k=I-T;if(k<=0)continue;const S=new Uint8ClampedArray(k*v.height*v.channels);for(let F=0;F<v.height;F++){const O=(F*I+T)*v.channels;S.set(v.data.subarray(O,O+k*v.channels),F*k*v.channels)}const A={width:k,height:v.height,channels:v.channels,data:S},N=Pa(A),V=(await n.run({[n.inputNames[0]]:new qe("float32",N,[1,3,Bt,Bt])}))[n.outputNames[0]].data[1]??0;x=x===null?V:Math.max(x,V)}return x}async function Ym(e,t,n,r,i,o,a){var f;const s=(m,y,w,b)=>{const x=Math.max(0,Math.round(m)),M=Math.max(0,Math.round(y)),v=Math.min(n.width,Math.round(m+w)),I=Math.min(n.height,Math.round(y+b)),T=v-x,k=I-M;if(T<=0||k<=0)return null;const S=n.channels,A=new Uint8ClampedArray(T*k*S);for(let N=0;N<k;N++){const U=((M+N)*n.width+x)*S;A.set(n.data.subarray(U,U+T*S),N*T*S)}return{width:T,height:k,channels:S,data:A}},u=async m=>(await i.run({[i.inputNames[0]]:new qe("float32",m,[1,3,Xn,Xn])}))[i.outputNames[0]].data,l=new Map;for(const m of r){const[y,w,b,x]=m;if(b<=0||x<=0)continue;const M=s(y,w,b,x);if(M===null)continue;const{id:v,prob:I}=await m1(M,u);if(v===""||I<c1)continue;const T=l.get(v);(T===void 0||I>T.prob)&&l.set(v,{prob:I,box:m})}const h=[],c=await e.tuckClassifier(),p=await e.tuckBoxClassifier();for(const[m,{prob:y,box:w}]of l){const[b,x,M,v]=w;let I={x:Math.round(b),y:Math.round(x),width:Math.round(M),height:Math.round(v)},T=null,k=[],S=null;if(Date.now()<o)try{const X=await e.wonderRef(m);if(X!==null){const J=$m(t,n,X,w);if(J!==null){T=J.footprint,k=J.overflow;const he=T.map(B=>B[0]),W=T.map(B=>B[1]),z=Math.max(0,Math.round(Math.min(...he))),R=Math.max(0,Math.round(Math.min(...W)));if(I={x:z,y:R,width:Math.min(n.width,Math.round(Math.max(...he)))-z,height:Math.min(n.height,Math.round(Math.max(...W)))-R},c!==null)try{const B=xm(t,n,X,T);if(B!==null){const L=Pa(B),G=await c.run({[c.inputNames[0]]:new qe("float32",L,[1,3,Bt,Bt])});S=Sm(G[c.outputNames[0]].data).prob}}catch{}}}}catch(X){console.warn(`[wonders-cls] ${m} registration failed:`,X)}const A=T!==null?za(T,k):null,N=[];if(S!==null&&N.push(S>=Ba?1:0),p!==null)try{const X=await tx(n,w,p);X!==null&&N.push(X>=Ba?1:0)}catch{}const U=A??I,V=a.some(X=>{const J=X.box[0]+X.box[2]/2,he=X.box[1]+X.box[3]/2;return J>=U.x&&J<=U.x+U.width&&he>=U.y&&he<=U.y+U.height});N.push(V?1:0);let F=N.length>0&&N.reduce((X,J)=>X+J,0)*2>N.length;F&&ex(U,I,a)&&(F=!1);const O={id:m,name:((f=Q2[m])==null?void 0:f.name)??m,builtWithCardUnderneath:F,boundingBox:I,confidence:Math.round(y*1e4)/1e4,...A?{tuckRegion:A}:{}},H=A??I;h.push({obj:O,edgeScores:null,zone:{x0:H.x,y0:H.y,x1:H.x+H.width,y1:H.y+H.height},quad:T,region:A})}return h}async function Xm(e,t,n,r,i,o){const a=await e.localiseWonders(n);return a.length===0?[]:Ym(e,t,n,a,r,i,o)}function nx(e,t){const n=qm(e.obj.boundingBox,t),r=e.region===null?null:qm(e.region,t),i=r??n;return{obj:{...e.obj,boundingBox:n,...e.region===null?{}:{tuckRegion:r}},edgeScores:e.edgeScores,zone:{x0:i.x,y0:i.y,x1:i.x+i.width,y1:i.y+i.height},quad:e.quad===null?null:e.quad.map(([o,a])=>[o+t[0],a+t[1]]),region:r}}async function Zm(e){try{const t=I2(e.image.width,e.image.height,e.banners.map(a=>a.box),e.hulls,e.wonderBoxes);if(t.length===0)return[];const n=[];for(const a of t){const s=e.cropFrame(a);if(!(s.width<=0||s.height<=0))for(const u of await e.detect(s,E2(e.banners,a)))n.push(nx(u,a))}if(e.builtSeenOut)for(const a of n)a.obj.id&&a.obj.builtWithCardUnderneath===!0&&e.builtSeenOut.add(a.obj.id);if(n.length===0)return[];const r=[...e.known.map(a=>({boundingBox:a.boundingBox,id:a.id,neuf:-1})),...n.map((a,s)=>({boundingBox:a.obj.boundingBox,id:a.obj.id,neuf:s}))],i=T2(r),o=[];for(const a of i){const s=a.neuf;s>=0&&o.push(n[s])}return o}catch(t){return console.warn("[#149 wonder-rescan] skipped:",t),[]}}const Ne="/7wd-scorer/models/",Va=[];let cn=null;function rx(){Va.length=0,cn=null}function ix(e){const t=performance.now();cn!==null&&Va.push({nom:cn.nom,ms:Math.round(t-cn.debut)}),cn={nom:e,debut:t}}function ox(){const e=[...Va];cn!==null&&e.push({nom:`${cn.nom} (en cours)`,ms:Math.round(performance.now()-cn.debut)});const t=new Map;for(const n of e){const r=t.get(n.nom)??{appels:0,ms:0};r.appels+=1,r.ms+=n.ms,t.set(n.nom,r)}return[...t.entries()].map(([n,r])=>({nom:n,appels:r.appels,ms:r.ms})).sort((n,r)=>r.ms-n.ms)}function ax(){const e={};for(const t of Object.keys(ot))e[ot[t].onnx]=pi.has(t)?"wasm (repli apres echec webgpu)":"webgpu>wasm";return e}let Qm=!1;const hi=new Map;function Jm(){var e;Qm||(Fe.wasm.wasmPaths="/7wd-scorer/ort/",Fe.wasm.numThreads=globalThis.crossOriginIsolated?Math.max(1,(((e=globalThis.navigator)==null?void 0:e.hardwareConcurrency)??4)-2):1,Qm=!0)}const pi=new Set;function sx(e){Jm();let t=hi.get(e);return t===void 0&&(t=rt.create(`${Ne}${ot[e].onnx}`,{executionProviders:pi.has(e)?["wasm"]:["webgpu","wasm"]}),hi.set(e,t),t.catch(()=>hi.delete(e))),t}let Ha=null,ja=null;const ux=.75,lx=4,cx=.65,dx=3e4;let Ka=null;function vr(){return Ka===null&&(Ka=(async()=>{try{let e;return self.importScripts("/7wd-scorer/opencv/opencv.js"),e=self.cv,typeof(e==null?void 0:e.then)=="function"&&(e=await e),typeof(e==null?void 0:e.getBuildInformation)!="function"&&(e=await new Promise(t=>{e.onRuntimeInitialized=()=>t(e)})),e}catch(e){return console.warn("[wonders-reg] opencv.js load failed:",e),null}})()),Ka}const eg=new Map;function Ya(e){let t=eg.get(e);return t===void 0&&(t=(async()=>{try{const n=await fetch(`${Ne}${e}`);if(!n.ok)return null;const r=await createImageBitmap(await n.blob()),o=new OffscreenCanvas(r.width,r.height).getContext("2d");o.drawImage(r,0,0);const a=o.getImageData(0,0,r.width,r.height);return{width:r.width,height:r.height,channels:4,data:new Uint8Array(a.data.buffer)}}catch{return null}})(),eg.set(e,t)),t}function Xa(e){return Ya(`wonder-refs/${e}.jpg`)}const tg=["builders-guild","magistrates-guild","merchants-guild","moneylenders-guild","scientists-guild","shipowners-guild","tacticians-guild"];async function hx(){const e=new Map;for(const t of tg){const n=await Ya(`guild-refs/${t}.jpg`);n!==null&&e.set(t,n)}return e}async function px(){const e=new Map;for(const t of tg){const n=await Ya(`guild-band-refs/${t}.png`);n!==null&&e.set(t,n)}return e}const fx=.6,mx=12,gx=45e3;let Za=null;function ng(){return Za===null&&(Jm(),Za=(async()=>{try{const[e,t,n,r]=await Promise.all([rt.create(`${Ne}ocr/ch_PP-OCRv4_det_infer.onnx`,{executionProviders:["webgpu","wasm"]}),rt.create(`${Ne}ocr/ch_PP-OCRv4_rec_infer.onnx`,{executionProviders:["webgpu","wasm"]}),fetch(`${Ne}ocr_charset.json`).then(i=>i.ok?i.json():null),fetch(`${Ne}wonder_names.json`).then(i=>i.ok?i.json():null)]);return n===null||r===null?(console.warn("[wonders-ocr] charset/names asset missing"),null):{det:e,rec:t,charset:b_(n),catalog:r.entries}}catch(e){return console.warn("[wonders-ocr] bundle load failed:",e),null}})()),Za}async function yx(e,t){const n=Math.max(__/Kt,t.width/t.height),{tensor:r,width:i}=$_(t,n),o={[e.rec.inputNames[0]]:new qe("float32",r,[1,3,Kt,i])},a=(await e.rec.run(o))[e.rec.outputNames[0]],[s,u,l]=a.dims,h=a.data,c=new Array(u),p=new Array(u);for(let f=0;f<u;f++){let m=0,y=-1/0;const w=f*l;for(let b=0;b<l;b++){const x=h[w+b];x>y&&(y=x,m=b)}c[f]=m,p[f]=y}return x_(c,p,e.charset)}async function wx(e,t){const n=await ng();if(n===null)return{wonders:[],aborted:!1};const r=new Map,i=Date.now()+gx;let o=!1;e:for(const a of[0,1,2,3]){if(Date.now()>i){o=!0;break}t(`wonder names: rotation ${a*90}°…`,a/4);const s=jt(e,a),u=l_(s),l={[n.det.inputNames[0]]:new qe("float32",u.tensor,[1,3,u.height,u.width])},h=(await n.det.run(l))[n.det.outputNames[0]],c=m_(h.data,u,s.width,s.height).slice(0,mx);console.debug(`[wonders-ocr] rot ${a*90}: ${c.length} det boxes`,c.slice(0,5).map(p=>`${p.width}x${p.height}@${p.score.toFixed(2)}`));for(const p of c){if(Date.now()>i){o=!0;break e}const f=g_(s,p.quad);if(f.width<f.height*1.5)continue;const[m,y]=await yx(n,f);if(console.debug(`[wonders-ocr] rec "${m}" @${y.toFixed(2)}`),y<fx||m.trim().length<lx)continue;const w=k_(m,n.catalog);if(console.debug("[wonders-ocr] fuzzy",w),w===null||w.confidence<ux||w.kind!=="wonder")continue;const b=r.get(w.id);(b===void 0||w.confidence>b.confidence)&&r.set(w.id,{id:w.id,name:w.name,confidence:w.confidence,nameBox:Qa(p,a,e.width,e.height)})}}return{wonders:[...r.values()],aborted:o}}function Qa(e,t,n,r){const i=(t%4+4)%4;if(i===0)return{x:e.x,y:e.y,width:e.width,height:e.height};const o=(c,p)=>i===1?[p,r-1-c]:i===2?[n-1-c,r-1-p]:[n-1-p,c],a=[o(e.x,e.y),o(e.x+e.width,e.y+e.height)],s=a.map(c=>c[0]),u=a.map(c=>c[1]),l=Math.min(...s),h=Math.min(...u);return{x:l,y:h,width:Math.max(...s)-l,height:Math.max(...u)-h}}function _x(){return ja===null&&(ja=fetch(`${Ne}laurel_gallery.json`).then(async e=>e.ok?Zw(await e.json()):[]).catch(()=>[])),ja}function bx(e,t,n,r){return Dt(e,t-r,n-r,2*r,2*r)}function Dt(e,t,n,r,i){const o=Math.max(0,Math.round(t)),a=Math.max(0,Math.round(n)),s=Math.min(e.width,Math.round(t+r)),u=Math.min(e.height,Math.round(n+i)),l=Math.max(0,s-o),h=Math.max(0,u-a),c=new Uint8Array(l*h*3);for(let p=0;p<h;p++)for(let f=0;f<l;f++){const m=((p+a)*e.width+(f+o))*e.channels,y=(p*l+f)*3;c[y]=e.data[m],c[y+1]=e.data[m+1],c[y+2]=e.data[m+2]}return{width:l,height:h,channels:3,data:c}}function xx(){return Ha===null&&(Ha=fetch(`${Ne}token_templates.json`).then(async e=>e.ok?Vb(await e.json()):new Map).catch(()=>new Map)),Ha}let Ja=null;function $x(){return Ja===null&&(Ja=(async()=>{try{const e=await fetch(`${Ne}token_embed_index.json`);if(!e.ok)return null;const t=Jb(await e.json());return{session:await rt.create(`${Ne}token_embed.onnx`,{executionProviders:["wasm"]}),index:t}}catch{return null}})()),Ja}const vx=.92;let es=null;function Sx(){return es===null&&(es=(async()=>{try{return(await fetch(`${Ne}guild_classifier.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}guild_classifier.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),es}let ts=null;function Mx(){return ts===null&&(ts=(async()=>{try{return(await fetch(`${Ne}laurel_digit.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}laurel_digit.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ts}let ns=null,rs=null;function Ix(){return rs===null&&(rs=(async()=>{try{return(await fetch(`${Ne}banner_class.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}banner_class.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),rs}async function Ex(e,t){if(t.length===0)return t;const n=await Ix();if(n===null)return t;const r=[];for(const i of t)try{const o=T1(i.box,e.width,e.height);if(o===null){r.push(i);continue}const a=Dt(e,o.x,o.y,o.w,o.h),s=k1(a),u=await n.run({[n.inputNames[0]]:new qe("float32",s,[1,3,ln,ln])});C1(u[n.outputNames[0]].data).rejected||r.push(i)}catch{r.push(i)}return r}function Tx(){return ns===null&&(ns=(async()=>{try{return(await fetch(`${Ne}laurel_filter.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}laurel_filter.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ns}async function kx(e,t,n){let[r,i,o,a]=t,s=o-r,u=a-i;if(s<=0||u<=0)return null;if(s<Qn){const w=Math.floor((r+o)/2);r=w-Math.floor(Qn/2),o=w+Math.floor(Qn/2),s=o-r}if(u<Qn){const w=Math.floor((i+a)/2);i=w-Math.floor(Qn/2),a=w+Math.floor(Qn/2),u=a-i}const l=Math.trunc(Bm*s),h=Math.trunc(Bm*u),c=Math.max(0,r-l),p=Math.max(0,i-h),f=Math.min(e.width,o+l),m=Math.min(e.height,a+h),y=Dt(e,c,p,f-c,m-p);if(y.width<=0||y.height<=0)return null;try{const w=v1(y),b=await n.run({[n.inputNames[0]]:new qe("float32",w,[1,3,un,un])});return S1(b[n.outputNames[0]].data)}catch{return null}}let is=null;function Cx(){return is===null&&(is=(async()=>{try{return(await fetch(`${Ne}coin_filter_cnn.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}coin_filter_cnn.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),is}let os=null;function Ax(){return os===null&&(os=(async()=>{try{return(await fetch(`${Ne}coin_denom.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}coin_denom.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),os}async function Rx(e,t,n){if(t.length===0)return[];try{const r=[];for(const u of t){const l=Fm(e,Math.round(u.cx),Math.round(u.cy),Math.round(u.r));if(l===null)return null;r.push(l)}const i=new Float32Array(t.length*3*_t*_t);r.forEach((u,l)=>i.set(u,l*u.length));const a=(await n.run({[n.inputNames[0]]:new qe("float32",i,[t.length,3,_t,_t])}))[n.outputNames[0]].data,s=ui.length;return t.map((u,l)=>Y1(a.subarray(l*s,l*s+s)))}catch{return null}}async function Ox(e,t,n){if(t.length===0)return[];try{const r=async u=>{const l=[];for(let f=0;f<t.length;f++){const m=Fm(e,Math.round(t[f].cx),Math.round(t[f].cy),Math.round(u[f]));if(m===null)return null;l.push(m)}const h=new Float32Array(t.length*3*_t*_t);l.forEach((f,m)=>h.set(f,m*f.length));const p=(await n.run({[n.inputNames[0]]:new qe("float32",h,[t.length,3,_t,_t])}))[n.outputNames[0]].data;return t.map((f,m)=>j1(p.subarray(m*2,m*2+2)))},i=await r(t.map(u=>u.r));if(i===null)return null;const o=t.map(u=>u.r).sort((u,l)=>u-l),a=o.length%2===1?o[(o.length-1)/2]:(o[o.length/2-1]+o[o.length/2])/2,s=Math.trunc(a);if(s>=8){const u=await r(t.map(()=>s));if(u!==null)return i.map((l,h)=>Math.max(l,u[h]))}return i}catch{return null}}let as=null;function rg(){return as===null&&(as=(async()=>{try{return(await fetch(`${Ne}tuck_classifier.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}tuck_classifier.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),as}const ig=.1;let ss=null;function og(){return ss===null&&(ss=(async()=>{try{return(await fetch(`${Ne}track_band.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}track_band.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ss}async function ag(e,t,n){try{const r=ba(t,1280,tw(t.width,t.height,n)),i=await e.run({[e.inputNames[0]]:new qe("float32",r.tensor,[1,3,1280,1280])});return _r(i[e.outputNames[0]].data,r.params,ig)}catch{return[]}}let us=null;const Nx=.4;function zx(e,t){const n=Math.min(e.x+e.width,t.x+t.width)-Math.max(e.x,t.x),r=Math.min(e.y+e.height,t.y+t.height)-Math.max(e.y,t.y);if(n<=0||r<=0)return 0;const i=e.width*e.height;return i>0?n*r/i:0}function Bx(e,t){const n=[],r=[];for(const i of t){if(!i.builtWithCardUnderneath)continue;i.boundingBox&&n.push(i.boundingBox);const o=i.tuckRegion;o&&r.push(o)}return n.length===0&&r.length===0?e:e.filter(i=>{const o=i.boundingBox;if(!o)return!0;const a=o.x+o.width/2,s=o.y+o.height/2;for(const u of n)if(a>=u.x&&a<=u.x+u.width&&s>=u.y&&s<=u.y+u.height||zx(o,u)>=Nx)return!1;for(const u of r)if(a>=u.x&&a<=u.x+u.width&&s>=u.y&&s<=u.y+u.height)return!1;return!0})}function Px(){return us===null&&(us=(async()=>{try{return(await fetch(`${Ne}tuck_box.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}tuck_box.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),us}let ls=null;function Dx(){return ls===null&&(ls=(async()=>{try{return(await fetch(`${Ne}wonder_classifier.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}wonder_classifier.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ls}const cs={wonderRef:Xa,tuckClassifier:rg,tuckBoxClassifier:Px,localiseWonders:async e=>{try{const{rows:t,params:n}=await bt("wonder",e);return xa(t,n,ot.wonder.conf,Number.POSITIVE_INFINITY).map(r=>r.box)}catch{return[]}}};async function Ux(e,t){const n=await $x();if(n!==null)try{const r=t1(e),i=new qe("float32",r,[4,3,sn,sn]),a=(await n.session.run({image:i}))[n.session.outputNames[0]].data,{id:s,cosine:u}=r1(n.index,n1(a));return u<vx?["",-1]:[s,u]}catch{}return Yb(e,t)}async function ds(e){let t;try{t=await createImageBitmap(e)}catch(n){const r=e.name||"(sans nom)",i=e.type||"(type inconnu)",o=e.size===0?"le fichier est VIDE (0 octet) — la capture a probablement été interrompue":/heic|heif/i.test(i)||/\.hei[cf]$/i.test(r)?"format HEIC/HEIF : ce navigateur ne sait pas le décoder — régler l'appareil photo sur JPEG (« Plus compatible » sur iPhone), ou repasser par la galerie qui convertit":"le fichier n'est plus lisible : s'il vient de l'appareil photo, l'OS a pu l'invalider pendant que l'app était en arrière-plan — reprendre la photo devrait suffire";throw new Error(`Image illisible (${r}, ${i}, ${e.size} octets) : ${o}. [${n instanceof Error?n.name:String(n)}]`)}try{const r=new OffscreenCanvas(t.width,t.height).getContext("2d",{willReadFrequently:!0});if(r===null)throw new Error("OffscreenCanvas 2D context unavailable.");r.drawImage(t,0,0);const{data:i}=r.getImageData(0,0,t.width,t.height);return{width:t.width,height:t.height,channels:4,data:i}}finally{t.close()}}async function bt(e,t){const n=ot[e],{tensor:r,params:i}=ba(t,n.input),o=async()=>{const a=await sx(e),s={[a.inputNames[0]]:new qe("float32",r,[1,3,n.input,n.input])};return{rows:(await a.run(s))[a.outputNames[0]].data,params:i}};try{return await o()}catch(a){if(pi.has(e))throw a;return pi.add(e),hi.delete(e),await o()}}const Lx=6,Fx=4,Gx=5,Wx=2;async function qx(e){const t={kind:"unknown",confidence:0,banners:null,laurels:null,coins:null,pawnFound:!1},n=await ds(e),r=await bt("banner",n),i=ni(r.rows,r.params,ot.banner.conf);if(t.banners=i.length,i.length>=Lx)return{...t,kind:"player",confidence:Math.min(1,i.length/12)};const o=await bt("laurel",n),a=_r(o.rows,o.params,ot.laurel.conf);if(t.laurels=a.length,a.length>=Fx)return{...t,kind:"player",confidence:Math.min(1,a.length/8)};const s=await bt("coin",n),u=rm(s.rows,s.params,ot.coin.conf);return t.coins=u.length,u.length>=Gx?{...t,kind:"player",confidence:.5}:t.banners!==null&&t.banners<=Wx?{...t,kind:"board",confidence:.4}:t}function Vx(){return{wonders:[],guilds:[],progressTokens:[],laurels:[],cardVictoryPoints:{value:0,laurelsKept:0,laurelsUnread:0,complete:!0},cardCounts:{byFamily:{},source:"none",tuckedExcluded:0},coins:{total:0,confidence:0,source:"none",coins:[]}}}async function hs(e,t,n,r,i=()=>{},o="player",a,s=!1){const u={},l=[],h=[],c=[],p=[],f=[],m=[];let y=0,w=0,b=0,x=0,M=0;for(const A of e){M+=1;const N=`${t} photo ${M}/${e.length}`;r(`${N}: reading pixels…`,.01);const U=await ds(A);r(`${N}: card banners…`,.04);const V=await bt("banner",U);let F=ni(V.rows,V.params,ot.banner.conf);F=await Ex(U,F),r(`${N}: progress tokens…`,.08);let O=[];const H=await og();H!==null&&(O=await ag(H,U,F)),O.length>0&&F.length>0&&(F=F.filter(D=>{const j=D.box[0]+D.box[2]/2,Q=D.box[1]+D.box[3]/2;return!O.some(([ne,ae,ue,Te])=>Math.min(ne,ue)<=j&&j<=Math.max(ne,ue)&&Math.min(ae,Te)<=Q&&Q<=Math.max(ae,Te))}));const X=await bt("token",U),J=await xx(),he=c.length,W=[];for(const D of hw(X.rows,X.params,ot.token.conf)){if(W.push({cx:D.cx,cy:D.cy,r:D.r}),O.some(([ne,ae,ue,Te])=>D.cx>=ne&&D.cx<=ue&&D.cy>=ae&&D.cy<=Te))continue;const[j,Q]=await Ux(cm(U,D),J);j===""&&Q<0?W.pop():j===""?w+=1:c.some(ne=>ne.id===j)||c.push({id:j,center:[D.cx,D.cy],radius:D.r,confidence:Math.round(Q*1e4)/1e4})}r(`${N}: coins…`,.14);const z=await bt("coin",U),R=rm(z.rows,z.params,ot.coin.conf).filter(D=>!W.some(j=>(D.cx-j.cx)**2+(D.cy-j.cy)**2<=D.r*D.r)),B=await Cx(),L=B!==null?await Ox(U,R,B):null,G=(L!==null?R.filter((D,j)=>L[j]>=Lm).map(D=>D.r):[]).sort((D,j)=>D-j),Z=G.length>0?G.length%2===1?G[(G.length-1)/2]:(G[G.length/2-1]+G[G.length/2])/2:null,[ie,te]=H1,ye=R.map((D,j)=>{const Q=L!==null?L[j]:null;return Q===null||Q>=Lm?"keep":Z!==null&&Z>0&&D.r/Z>=ie&&D.r/Z<=te?"suspect":"drop"}),Me=R.filter((D,j)=>ye[j]==="keep"),ze=Bw(U,Me),De=await Ax(),lt=De!==null?await Rx(U,Me,De):null,Be=X1(ze,lt??ze.map(()=>null));Be.map(D=>D.value);const ge=[];let ct=0;if(R.forEach((D,j)=>{if(ye[j]==="drop")return;if(ye[j]==="suspect"){const ne=L[j];ge.push({denomination:null,center:[D.cx,D.cy],radius:D.r,suspect:!0,suspectReason:`content rejected as non-coin (P=${ne.toFixed(2)}) but the size matches this photo's confirmed coins — glare-blinded real coin OR a look-alike object; confirm or remove (a busy table warrants a cleaner photo)`});return}const Q=Be[ct++];ge.push({denomination:Q.value,center:[D.cx,D.cy],radius:D.r,denomSource:Q.source??"colour"})}),R.length>0&&ge.length===0&&n.push({code:"LOW_CONFIDENCE",message:`${t}: ${R.length} disque(s) rond(s) détecté(s) mais tous rejetés comme non-pièces (0 pièce comptée) — vérifie, ou reprends une photo plus nette.`}),ge.length>=2){const D=ge.map(Q=>Q.radius).sort((Q,ne)=>Q-ne),j=D.length%2===1?D[(D.length-1)/2]:(D[D.length/2-1]+D[D.length/2])/2;if(j>0)for(const Q of ge)Q.radius/j>2&&(Q.suspect=!0,Q.suspectReason=`radius ${Q.radius}px is ${(Q.radius/j).toFixed(1)}x the photo's median coin radius — probably not a coin`)}if(ge.length>=2)for(let D=0;D<ge.length;D+=1)for(let j=D+1;j<ge.length;j+=1){const Q=ge[D],ne=ge[j],ae=Math.hypot(Q.center[0]-ne.center[0],Q.center[1]-ne.center[1]);if(ae<1.1*Math.min(Q.radius,ne.radius))for(const ue of[Q,ne])ue.suspect||(ue.suspect=!0,ue.suspectReason=`almost concentric with another coin (${ae.toFixed(0)}px apart) — either a pile of two coins or a duplicate read of one; confirm which`)}const Xt=p.length,Qe=[],dt=[],pt=Date.now()+dx;let Ye=null,Rt=null;const Sr=()=>(Rt===null&&(Rt=(async()=>{try{const{rows:D,params:j}=await bt("wonder",U);return xa(D,j,ot.wonder.conf,Number.POSITIVE_INFINITY).map(Q=>Q.box)}catch{return[]}})()),Rt),Zt=[];let On=!1;const Qt=await Dx();if(Qt!==null){const D=await Sr();if(D.length>0&&(Ye=await vr(),Ye!==null)){r(`${N}: identifying wonders…`,.35);const j=await Ym(cs,Ye,U,D,Qt,pt,F);for(const Q of j)p.some(ne=>ne.id===Q.obj.id)||(p.push(Q.obj),Zt.push({obj:Q.obj,edgeScores:Q.edgeScores,zone:Q.zone}),Qe.push(Q.zone),dt.push({quad:Q.quad,region:Q.region}));On=j.length>0}}On||r(`${N}: wonder names…`,.2);const ft=On?{wonders:[],aborted:!1}:await wx(U,(D,j)=>r(`${N}: ${D}`,.2+.35*(j??0)));Ye===null&&(Ye=ft.wonders.length>0?await vr():null);for(const D of ft.wonders){let j=null;if(Ye!==null&&Date.now()<pt){r(`${N}: registering ${D.name}…`,.6);try{const Q=await Xa(D.id);if(Q!==null){let ne=F_(Ye,U,Q,[[D.nameBox.x,D.nameBox.y],[D.nameBox.x+D.nameBox.width,D.nameBox.y],[D.nameBox.x+D.nameBox.width,D.nameBox.y+D.nameBox.height],[D.nameBox.x,D.nameBox.y+D.nameBox.height]]);if(ne===null){const ae=await Sr(),ue=H_(ae,D.nameBox.x+D.nameBox.width/2,D.nameBox.y+D.nameBox.height/2);ue!==null&&(ne=$m(Ye,U,Q,ue))}if(ne!==null){let ae=ne.built,ue=!1;const Te=await rg();if(Te!==null)try{const me=xm(Ye,U,Q,ne.footprint);if(me!==null){const be=Pa(me),We=await Te.run({[Te.inputNames[0]]:new qe("float32",be,[1,3,Bt,Bt])});ae=Sm(We[Te.outputNames[0]].data).built,ue=!0}}catch{}const ke=ne.footprint.map(me=>me[0]),ce=ne.footprint.map(me=>me[1]),oe=Math.max(0,Math.round(Math.min(...ke))),le=Math.max(0,Math.round(Math.min(...ce)));j={built:ae,boundingBox:{x:oe,y:le,width:Math.min(U.width,Math.round(Math.max(...ke)))-oe,height:Math.min(U.height,Math.round(Math.max(...ce)))-le},tuckRegion:za(ne.footprint,ne.overflow),footprint:ne.footprint,edgeScores:ne.edgeScores,builtByTuck:ue}}}}catch(Q){console.warn(`[wonders-reg] ${D.id} failed:`,Q)}}if(j!==null){const Q=j.tuckRegion??j.boundingBox;Qe.push({x0:Q.x,y0:Q.y,x1:Q.x+Q.width,y1:Q.y+Q.height}),dt.push({quad:j.footprint,region:j.tuckRegion})}else{const Q=Math.max(8,D.nameBox.height),ne=Math.round(D.nameBox.width*.15);Qe.push({x0:D.nameBox.x-ne,y0:D.nameBox.y-Q*2.5,x1:D.nameBox.x+D.nameBox.width+ne,y1:D.nameBox.y+D.nameBox.height+Q*2.5}),dt.push({quad:null,region:null})}if(!p.some(Q=>Q.id===D.id)){const Q=(j==null?void 0:j.builtByTuck)===!0,ne=Q?j.built:!1,ae=!Q&&(j==null?void 0:j.built)===!0,ue={id:D.id,name:D.name,builtWithCardUnderneath:ne,boundingBox:(j==null?void 0:j.boundingBox)??{x:0,y:0,width:0,height:0},...j!=null&&j.tuckRegion?{tuckRegion:j.tuckRegion}:{},confidence:D.confidence,...ae?{suspect:!0,suspectReason:"built-unconfirmed"}:{}};p.push(ue),Zt.push({obj:ue,edgeScores:j&&!j.builtByTuck?j.edgeScores:null,zone:Qe[Qe.length-1]})}}if(!On){const D=Y_(Zt.map(j=>({built:j.obj.builtWithCardUnderneath,edgeScores:j.edgeScores,zone:j.zone})),F.map(j=>[j.box[0]+j.box[2]/2,j.box[1]+j.box[3]/2]));for(const j of D){const Q=Zt[j];Q.obj.builtWithCardUnderneath=!1,n.push({code:"INCONSISTENT_STATE",message:`${t}: wonder '${Q.obj.id}' was NOT marked built — the card-under-wonder signal saturated on this surface and no tucked card banner supports it. Tick it in the review if it really was built.`})}if(F.length>0){const j=new Set(D);for(let Q=0;Q<Zt.length;Q++){const ne=Zt[Q];if(j.has(Q)||!ne.obj.builtWithCardUnderneath)continue;const ae=ne.obj.tuckRegion;if(ae===void 0)continue;if(!F.some(Te=>{const ke=Te.box[0]+Te.box[2]/2,ce=Te.box[1]+Te.box[3]/2;return ke>=ae.x&&ke<=ae.x+ae.width&&ce>=ae.y&&ce<=ae.y+ae.height})){const Te=ne.obj;Te.builtWithCardUnderneath=!1,Te.suspect=!0,Te.suspectReason="built-unconfirmed"}}}}if(ft.aborted&&n.push({code:"LOW_CONFIDENCE",message:`${N}: the wonder-name read ran out of its time budget on this device — ${ft.wonders.length} wonder(s) read before the cutoff; check the built-wonders list.`}),Ye!==null&&ft.wonders.length>0&&Date.now()<pt)try{const D=await ng(),j=(D==null?void 0:D.catalog.filter(ne=>ne.kind==="wonder").map(ne=>ne.id))??[],Q=new Map;for(const ne of j)if(!p.some(ae=>ae.id===ne)){const ae=await Xa(ne);ae!==null&&Q.set(ne,ae)}if(Q.size>0){r(`${N}: searching occluded wonders…`,.7);const ne=L_(Ye,U,Q,pt);for(const ae of ne){const ue=ae.footprint.map(We=>We[0]),Te=ae.footprint.map(We=>We[1]),ke=Math.max(0,Math.round(Math.min(...ue))),ce=Math.max(0,Math.round(Math.min(...Te))),oe={x:ke,y:ce,width:Math.min(U.width,Math.round(Math.max(...ue)))-ke,height:Math.min(U.height,Math.round(Math.max(...Te)))-ce};if(p.some(We=>{const Ue=We.boundingBox,Pn=Math.max(0,Math.min(Ue.x+Ue.width,oe.x+oe.width)-Math.max(Ue.x,oe.x)),nt=Math.max(0,Math.min(Ue.y+Ue.height,oe.y+oe.height)-Math.max(Ue.y,oe.y)),Pe=Pn*nt,He=Ue.width*Ue.height+oe.width*oe.height-Pe;return He>0&&Pe/He>U_}))continue;const me=D==null?void 0:D.catalog.find(We=>We.id===ae.id);p.push({id:ae.id,name:(me==null?void 0:me.nameFr)??(me==null?void 0:me.name)??ae.id,builtWithCardUnderneath:ae.built,boundingBox:oe,...ae.tuckRegion?{tuckRegion:ae.tuckRegion}:{},confidence:Math.round(ae.confidence*1e4)/1e4});const be=ae.tuckRegion??oe;Qe.push({x0:be.x,y0:be.y,x1:be.x+be.width,y1:be.y+be.height}),dt.push({quad:ae.footprint.map(([We,Ue])=>[We,Ue]),region:ae.tuckRegion??null})}}}catch(D){console.warn("[wonders-reg] discovery failed:",D)}const dn=o==="opponent";let hn=(D,j)=>!dn,Nn=(D,j)=>!dn,Mr=null;try{let D=p.slice(Xt);const j=[];F.forEach((ce,oe)=>{const le=ce.box[0]+ce.box[2]/2,me=ce.box[1]+ce.box[3]/2;Qe.some(be=>le>=be.x0&&le<=be.x1&&me>=be.y0&&me<=be.y1)||j.push(oe)});const Q=[],ne=[];D.forEach((ce,oe)=>{const le=ce.boundingBox;le&&le.width>0&&(Q.push(oe),ne.push([le.x,le.y,le.width,le.height]))});const ae=ce=>{const oe=[];return ce.forEach((le,me)=>{const be=le.box[0]+le.box[2]/2,We=le.box[1]+le.box[3]/2;Qe.some(Ue=>be>=Ue.x0&&be<=Ue.x1&&We>=Ue.y0&&We<=Ue.y1)||oe.push(me)}),oe};let ue=Ga(F.map(ce=>ce.box),j,ne,O,[U.width,U.height]);if(Qt!==null){r(`${N}: seconde passe merveilles (crop de cité)…`,.42);const oe=(await Zm({image:U,banners:F,hulls:ue.hulls.map(([le,me],be)=>({owner:le,poly:me,n:ue.hullBoxCounts[be]??0})),wonderBoxes:ne,known:D,cropFrame:([le,me,be,We])=>Dt(U,le,me,be-le,We-me),detect:async(le,me)=>(Ye===null&&(Ye=await vr()),Ye===null?[]:Xm(cs,Ye,le,Qt,pt,me))})).filter(le=>!p.some(me=>me.id===le.obj.id));if(oe.length>0){for(const le of oe)p.push(le.obj),Qe.push(le.zone),dt.push({quad:le.quad,region:le.region});D=p.slice(Xt),Q.length=0,ne.length=0,D.forEach((le,me)=>{const be=le.boundingBox;be&&be.width>0&&(Q.push(me),ne.push([be.x,be.y,be.width,be.height]))}),ue=Ga(F.map(le=>le.box),ae(F),ne,O,[U.width,U.height])}}try{const ce=Wm(U.width,U.height,F.map(oe=>oe.box),ue.hulls.map(([oe,le],me)=>({owner:oe,poly:le,n:ue.hullBoxCounts[me]??0})),ne);if(ce.length>0){const oe=Wa(F.map(me=>me.box)),le=[];for(const me of ce){const[be,We,Ue,Pn]=me,nt=Dt(U,be,We,Ue-be,Pn-We);if(nt.width<=0||nt.height<=0)continue;const Pe=await bt("banner",nt);for(const He of ni(Pe.rows,Pe.params,ot.banner.conf)){const Xe=M2(He.box,me,oe);Xe&&le.push({...He,box:Xe})}}if(le.length>0){const me=am([...F,...le]);me.length>F.length&&(F=me,ue=Ga(F.map(be=>be.box),ae(F),ne,O,[U.width,U.height]))}}}catch(ce){console.warn("[#129 city-rescan] skipped:",ce)}if(Qt!==null&&D.some(ce=>ce.builtWithCardUnderneath!==!0)){r(`${N}: revote built (crop de cité)…`,.47);const ce=new Set;await Zm({builtSeenOut:ce,image:U,banners:F,hulls:ue.hulls.map(([oe,le],me)=>({owner:oe,poly:le,n:ue.hullBoxCounts[me]??0})),wonderBoxes:ne,known:D,cropFrame:([oe,le,me,be])=>Dt(U,oe,le,me-oe,be-le),detect:async(oe,le)=>(Ye===null&&(Ye=await vr()),Ye===null?[]:Xm(cs,Ye,oe,Qt,pt,le))});for(const oe of D)oe.id&&ce.has(oe.id)&&oe.builtWithCardUnderneath!==!0&&(oe.builtWithCardUnderneath=!0,oe.builtByCityCrop=!0)}a!==void 0&&(a.hulls=ue.hulls.map(([ce,oe],le)=>({owner:ce,poly:oe,n:ue.hullBoxCounts[le]??0})),a.bandBoxes=O,a.image=U),hn=(ce,oe)=>ue.pointOwner(ce,oe)==="opponent"===dn;const Te=dn?"opponent":"player";if(Nn=(ce,oe)=>ue.pointOwner(ce,oe)===Te,s){const ce=ue;Mr=oe=>new Set(h2(oe,ce,Te,O))}F=F.filter((ce,oe)=>ue.bannerOwner[oe]==="opponent"===dn);const ke=D.map(()=>"player");Q.forEach((ce,oe)=>{ke[ce]=ue.wonderOwner[oe]});for(let ce=D.length-1;ce>=0;ce-=1)ke[ce]==="opponent"!==dn&&p.splice(Xt+ce,1);Qe.length=0;for(const ce of p.slice(Xt)){const oe=ce.tuckRegion??ce.boundingBox;oe&&Qe.push({x0:oe.x,y0:oe.y,x1:oe.x+oe.width,y1:oe.y+oe.height})}for(let ce=c.length-1;ce>=he;ce-=1){const[oe,le]=c[ce].center;hn(oe,le)||c.splice(ce,1)}}catch(D){console.warn("[city-split] failed (side unfiltered):",D)}const Jt=Mr!==null?Mr(ge):null;for(const D of ge)(Jt!==null?!Jt.has(D):!Nn(D.center[0],D.center[1]))||(y+=D.denomination??0,h.push(D));const mi=new Set,Ir=[],gi=Wa(F.map(D=>D.box));dt.forEach((D,j)=>{if(D.quad===null||D.region===null){const ue=Qe[j];ue&&Ir.push(ue);return}const Q=D.region,ne=[];F.forEach((ue,Te)=>{const ke=ue.box[0]+ue.box[2]/2,ce=ue.box[1]+ue.box[3]/2;ke>=Q.x&&ke<=Q.x+Q.width&&ce>=Q.y&&ce<=Q.y+Q.height&&ne.push([Te,ue.box])});const ae=V1(D.quad,ne,gi);ae!==null&&mi.add(ae)});let xt=[],zn=0;F.forEach((D,j)=>{if(mi.has(j)){x+=1,zn+=1;return}const Q=D.box[0]+D.box[2]/2,ne=D.box[1]+D.box[3]/2;if(Ir.some(ae=>Q>=ae.x0&&Q<=ae.x1&&ne>=ae.y0&&ne<=ae.y1)){x+=1,zn+=1;return}xt.push(D)});const yi=D1(xt,zn,O,U.width,U.height);xt=yi.kept;for(const D of xt)u[D.family]=(u[D.family]??0)+1,b+=1;const er=bw(xt),tr=new Set(er.map(D=>D.box.join(",")));for(const D of $w(xt))tr.has(D.box.join(","))||(er.push(D),tr.add(D.box.join(",")));for(const D of yi.suspects)tr.has(D.box.join(","))||(er.push(D),tr.add(D.box.join(",")));for(const D of er)m.push(D);if(xt.some(D=>D.family==="guild")){const D=await Sx();if(D!==null){r(`${N}: identifying guilds…`,.75);for(const j of xt)if(j.family==="guild")try{const[Q,ne,ae,ue]=j.box,Te=Dt(U,Q,ne,ae,ue),ke=a1(Te),ce={[D.inputNames[0]]:new qe("float32",ke,[1,3,Yn,Yn])},le=(await D.run(ce))[D.outputNames[0]].data,{id:me,prob:be}=s1(le);me!==""&&!f.some(We=>We.id===me)&&f.push({id:me,boundingBox:{x:Q,y:ne,width:ae,height:ue},confidence:Math.round(be*1e4)/1e4})}catch(Q){console.warn("[guild-cls] failed:",Q)}}else if(Date.now()<pt)try{const j=Ye??await vr();if(j!==null){const Q=await hx();if(Q.size>0){r(`${N}: identifying guilds…`,.75);const ne=await px();for(const ae of zb(j,U,Q,pt,ne))f.some(ue=>ue.id===ae.id)||f.push(ae)}}}catch(j){console.warn("[guilds-reg] failed:",j)}}r(`${N}: laurels…`,.8);const ms=await _x(),wi=[];for(const D of[0]){const j=D===0?U:jt(U,D),Q=await bt("laurel",j);for(const[ne,ae,ue,Te]of _r(Q.rows,Q.params,ot.laurel.conf)){const ke=Qa({x:ne,y:ae,width:ue-ne,height:Te-ae},D,U.width,U.height);wi.push([ke.x,ke.y,ke.x+ke.width,ke.y+ke.height])}}let pn=im(wi);const Er=[];try{const D=D2(F.map(j=>j.box),[U.width,U.height]);for(const[j,Q,ne,ae]of D){const ue=Dt(U,j,Q,ne-j,ae-Q);if(ue.width<=0||ue.height<=0)continue;const Te=[];for(const ke of[0]){const ce=ke===0?ue:jt(ue,ke),oe=await bt("laurel",ce);for(const[le,me,be,We]of _r(oe.rows,oe.params,ot.laurel.conf)){const Ue=Qa({x:le,y:me,width:be-le,height:We-me},ke,ue.width,ue.height);Te.push([Ue.x,Ue.y,Ue.x+Ue.width,Ue.y+Ue.height])}}if(pn=U2(pn,im(Te),[j,Q]),H!==null)try{const ke=ba(ue,1280,wr),ce=await H.run({[H.inputNames[0]]:new qe("float32",ke.tensor,[1,3,1280,1280])});for(const[oe,le,me,be]of _r(ce[H.outputNames[0]].data,ke.params,ig))Er.push([oe+j,le+Q,me+j,be+Q])}catch{}}}catch(D){console.warn("[laurel-containers] failed:",D)}const gs=[...O,...Er];pn=pn.filter(([D,j,Q,ne])=>!G2((D+Q)/2,(j+ne)/2,gs,F.map(ae=>ae.box)));const Bn=await Mx(),Tr=await Tx();for(const[D,j,Q,ne]of pn){const ae=Math.trunc((D+Q)/2),ue=Math.trunc((j+ne)/2);if([...W,...R].some(Pe=>(ae-Pe.cx)**2+(ue-Pe.cy)**2<=Pe.r*Pe.r)||!hn(ae,ue))continue;if(Tr!==null){const Pe=await kx(U,[Math.trunc(D),Math.trunc(j),Math.trunc(Q),Math.trunc(ne)],Tr);if(Pe!==null&&Pe>=$1)continue}const ke=Math.min(Math.trunc(Q-D),Math.trunc(ne-j)),ce=Math.max(6,Math.trunc(Math.max(Q-D,ne-j)*Gw)),oe=bx(U,ae,ue,ce);let le=null,me=0;const be=new Map;if(ke>=6)for(const Pe of[0,1,2,3]){const He=Pe===0?oe:jt(oe,Pe),[Xe,at]=n_(He,ms);Xe!==null&&(be.set(Xe,Math.max(be.get(Xe)??0,at)),at>me&&(le=Xe,me=at))}le!==null&&me<cx&&(le=null);const We=me;if(Bn!==null&&ke>=6){const Pe=Dt(U,Math.trunc(D),Math.trunc(j),Math.trunc(Q-D),Math.trunc(ne-j));let He=null,Xe=0;for(const at of[0,1,2,3]){const _i=at===0?Pe:jt(Pe,at),ys=_1(_i),ws=await Bn.run({[Bn.inputNames[0]]:new qe("float32",ys,[1,3,Zn,Zn])}),{value:bi,prob:Ut}=b1(ws[Bn.outputNames[0]].data);if(Ut>Xe&&(He=bi,Xe=Ut),He!==null&&Xe>=w1)break}He!==null&&Xe>=y1&&(le=He,me=Xe)}const Ue=le!==null&&[...be.entries()].some(([Pe,He])=>Pe!==le&&He>=We-.1),Pn=Qe.some(Pe=>ae>=Pe.x0&&ae<=Pe.x1&&ue>=Pe.y0&&ue<=Pe.y1),nt=f.some(Pe=>{const He=Pe.boundingBox;return He!==void 0&&ae>=He.x&&ae<=He.x+He.width&&ue>=He.y&&ue<=He.y+He.height});l.push({value:le,valueRead:le!==null,center:[Math.round((D+Q)/2),Math.round((j+ne)/2)],boundingBox:{x:Math.trunc(D),y:Math.trunc(j),width:Math.trunc(Q-D),height:Math.trunc(ne-j)},confidence:Math.round(me*1e4)/1e4,excluded:Pn||nt,photoIndex:M-1,...Ue?{suspect:!0,suspectReason:"orientation-ambiguous"}:{}})}i()}x>0?n.push({code:"OVERLAPPING_OBJECTS",message:`${t}: ${x} banner(s) near a wonder were excluded as tucked/consumed (estimated footprint — the server uses the real card box); verify the per-colour counts.`}):b>0&&p.length===0&&n.push({code:"OVERLAPPING_OBJECTS",message:`${t}: no wonder was located on this photo, so a card tucked under a wonder may still be counted — verify the per-colour counts.`});const v=u.guild??0;v!==f.length?n.push({code:"INCONSISTENT_STATE",message:`${t}: ${v} purple banner(s) counted but ${f.length} guild(s) identified — reconcile in the review (stacked guilds or a missed identification).`}):f.length>0&&n.push({code:"LOW_CONFIDENCE",message:`${t}: guild(s) identified by their card art: `+f.map(A=>A.id).join(", ")+" — confirm in the review."});const I=p.filter(A=>A.boundingBox.width===0);if(I.length>0?n.push({code:"LOW_CONFIDENCE",message:`${t}: wonder(s) identified by name but NOT registered against their reference (${I.map(A=>A.name).join(", ")}) — their BUILT flag is a suggestion: unselect any that was not built.`}):p.length>0&&n.push({code:"LOW_CONFIDENCE",message:`${t}: ${p.length} wonder(s) registered — the BUILT flags were measured (card protruding underneath); confirm in the review.`}),w>0&&n.push({code:"UNRECOGNIZED_OBJECT",message:`${t}: ${w} token disc(s) found but not identified — pick them in the review below.`}),c.length>0&&n.push({code:"LOW_CONFIDENCE",message:`${t}: progress token(s) identified on-device: `+c.map(A=>A.id).join(", ")+" — confirm in the review."}),h.length>0){const A=h.filter(U=>U.denomSource==="cnn").length,N=h.length-A;n.push({code:"LOW_CONFIDENCE",message:N===0?`${t}: coins read as ${y} from ${h.length} tile(s) by the learned denomination model — confirm the total.`:`${t}: coins read as ${y} from ${h.length} tile(s) — ${A} by the learned model, ${N} by metal COLOUR alone (the model abstained); confirm the total.`})}const T=Bx(f,p);for(const A of[...W2(p.map(N=>N.id),t),...H2(T.map(N=>N.id),t)])n.push({code:"INCONSISTENT_STATE",message:A.message});const k=l.filter(A=>!A.excluded),S=k.filter(A=>A.valueRead);return{...Vx(),wonders:p,guilds:T,progressTokens:c,laurels:l,cardVictoryPoints:{value:S.reduce((A,N)=>A+(N.value??0),0),laurelsKept:k.length,laurelsUnread:k.length-S.length,complete:k.length===S.length},cardCounts:{byFamily:u,source:b>0?"yolo":"none",tuckedExcluded:x,...m.length>0?{suspects:m}:{}},coins:{total:y,confidence:h.length>0?.5:0,source:h.length===0?"none":h.some(A=>A.denomSource==="cnn")?"local-cnn":"local-colour",coins:h}}}const At=1280,Hx=.3,fi=9;let ps=null;function sg(){return ps===null&&(ps=(async()=>{try{return(await fetch(`${Ne}pawn_ends.onnx`,{method:"HEAD"})).ok?await rt.create(`${Ne}pawn_ends.onnx`,{executionProviders:["wasm"]}):null}catch{return null}})()),ps}function jx(e){const t=At/Math.max(e.width,e.height),n=Math.round(e.width*t),r=Math.round(e.height*t),i=new OffscreenCanvas(e.width,e.height);i.getContext("2d",{willReadFrequently:!0}).putImageData(new ImageData(new Uint8ClampedArray(e.data),e.width,e.height),0,0);const s=new OffscreenCanvas(At,At).getContext("2d",{willReadFrequently:!0});s.fillStyle="rgb(114,114,114)",s.fillRect(0,0,At,At),s.drawImage(i,0,0,e.width,e.height,0,0,n,r);const{data:u}=s.getImageData(0,0,At,At),l=At*At,h=new Float32Array(3*l);for(let c=0;c<l;c+=1)h[c]=u[c*4]/255,h[l+c]=u[c*4+1]/255,h[2*l+c]=u[c*4+2]/255;return{tensor:h,r:t}}async function Kx(e,t){const{tensor:n,r}=jx(t),o=(await e.run({[e.inputNames[0]]:new qe("float32",n,[1,3,At,At])}))[e.outputNames[0]].data,a=new Map;for(let s=0;s+5<o.length;s+=6){const u=o[s+4];if(u<Hx)continue;const l=Math.round(o[s+5]),h=a.get(l);if(h===void 0||u>h.conf){const c=(o[s]+o[s+2])/2/r,p=(o[s+1]+o[s+3])/2/r;a.set(l,{conf:u,cx:c,cy:p})}}return a}async function fs(e,t){let n=null;for(let w=0;w<4;w+=1){const b=w===0?t:jt(t,w),x=await Kx(e,b);if(x.has(0)&&x.has(1)&&x.has(2)){const M=x.get(0).conf+x.get(1).conf+x.get(2).conf;(n===null||M>n.score)&&(n={score:M,det:x,k:w})}}if(n===null)return null;const r=n.det.get(0),i=n.det.get(1),o=n.det.get(2),a=o.cx-i.cx,s=o.cy-i.cy,u=(i.cx+o.cx)/2,l=(i.cy+o.cy)/2,h=a*a+s*s;if(h<=0)return null;const c=((r.cx-u)*a+(r.cy-l)*s)/h*(2*fi),p=Math.min(fi,Math.max(-fi,ut(c))),f=Math.min(r.conf,i.conf,o.conf),m=(w,b)=>{const x=n.k%4;return x===0?[w,b]:x===1?[b,t.height-1-w]:x===2?[t.width-1-w,t.height-1-b]:[t.width-1-b,w]},y=[i,o].map(w=>{const[b,x]=m(w.cx,w.cy);return[ut(b),ut(x)]});return{position:p,confidence:Math.round(f*1e4)/1e4,ends:y}}async function ug(e,t,n){let r=null;for(const i of n){const o=nw(t.width,t.height,i);if(o===null)continue;const a=Dt(t,o.x,o.y,o.width,o.height);if(a.width===0||a.height===0)continue;const s=await fs(e,a);s!==null&&(r===null||s.confidence>r.confidence)&&(r={...s,ends:s.ends.map(([u,l])=>[u+o.x,l+o.y])})}return r}async function Yx(e,t){const n=[{code:"LOW_CONFIDENCE",message:"On-device mode: everything is recognised locally — card counts, laurel values, wonders, guilds and token identities. The one gap is COIN DENOMINATIONS: the phone decides them by colour alone (the server uses a learned model that coin_denom.onnx would provide, and it is not staged for the browser), so check the coin totals."}],r={left:null,right:null},i=e.left.length+e.right.length+(e.both!==void 0?2:0);let o=0;const a=(f,m=0)=>{t(f,i>0?Math.min(.99,(o+m)/i):void 0)},s=()=>{o+=1};for(const f of["left","right"]){const m=e[f];m.length>0&&(r[f]=await hs(m,f,n,a,s))}let u=null,l=null;if(e.both!==void 0){const f={},m={player:await hs([e.both],"left",n,a,s,"player",f,!0),opponent:await hs([e.both],"right",n,a,s,"opponent",void 0,!0)};if(f.image!==void 0)try{const w=await sg();w!==null&&(u=await fs(w,f.image),u===null&&f.bandBoxes!==void 0&&f.bandBoxes.length>0&&(u=await ug(w,f.image,f.bandBoxes)))}catch(w){console.warn("[#125] both-photo pawn read failed:",w)}u!==null&&(l=sw(u.ends,f.hulls??[],u.position));const y=l!==null&&!l.ambiguous?uw(l):null;y!==null?(r.left=m[y.left],r.right=m[y.right],n.push({code:"AMBIGUOUS_OWNER",message:`Both-players photo: LEFT and RIGHT were derived from the MILITARY BOARD geometry (each track end paired with the city it is the capital of), which overrides the cluster-dominance guess — favored ${l.favoredOwner}, pawn at ${u.position}. Swap them in the review only if this is wrong.`})):(r.left=m.player,r.right=m.opponent,n.push({code:"AMBIGUOUS_OWNER",message:"Both-players photo: the DOMINANT city was assigned to the left player and the opposing city to the right — swap them in the review if the seating is the other way around."}))}{const f={},m={};for(const y of["left","right"]){const w=r[y];w!=null&&(f[y]=w.wonders.map(b=>b.id),m[y]=w.progressTokens.map(b=>b.id))}for(const y of[...q2(f),...V2(m)])n.push({code:"INCONSISTENT_STATE",message:y.message})}let h={conflictPawnPosition:0,found:!1,confidence:0};if(e.board!==void 0){try{const f=await ds(e.board),m=await sg();if(m!==null){let y=await fs(m,f);if(y===null){const w=await og();if(w!==null){const b=await bt("banner",f),x=ni(b.rows,b.params,ot.banner.conf),M=await ag(w,f,x);y=await ug(m,f,M)}}y!==null&&(h={conflictPawnPosition:y.position,found:!0,confidence:y.confidence},n.push({code:"AMBIGUOUS_OWNER",message:`Conflict pawn read at position ${y.position} — confirm which player it favours (the sign is a convention, not read from the photo).`}))}}catch(f){console.warn("[pawn] on-device read failed:",f)}h.found||n.push({code:"MILITARY_PAWN_NOT_FOUND",message:"On-device mode could not read the conflict pawn — set its position below."})}else u!==null&&l!==null&&(h={conflictPawnPosition:u.position,found:!0,confidence:u.confidence});const c=h.conflictPawnPosition,p=Math.abs(c)>=fi?{type:"military",winner:c>0?"left":"right"}:{type:"civilian"};return{imageId:e.imageId,players:r,militaryTrack:h,outcome:p,confidence:.5,warnings:n}}self.onmessage=e=>{const{id:t,kind:n}=e.data,r=(i,o)=>{ix(i),self.postMessage({id:t,progress:i,...o!==void 0?{fraction:o}:{}})};(async()=>{try{n==="recognize"&&r("starting the on-device engine…",0),rx();const i=performance.now(),o=n==="classify"?await qx(e.data.file):await Yx(e.data.payload,r);self.postMessage({id:t,ok:!0,result:o,perf:{etapes:ox(),providers:ax(),totalMs:Math.round(performance.now()-i)}})}catch(i){self.postMessage({id:t,ok:!1,error:String(i)})}})()}})();
