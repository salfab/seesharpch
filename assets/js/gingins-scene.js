...
(function () {
  ...
  scene.fog = new THREE.FogExp2(0x0b0c0e, 0.0015);
  ...
  var hemiLight = new THREE.HemisphereLight(0x88aacc, 0x445533, 0.3);
  scene.add(hemiLight);
  ...
  // Vegetation (surface - terrain = tree height, render as points above threshold)
  Promise.all([
    ...
    var terrain = results[0], surface = results[1];
    var tw = terrain.width, th = terrain.height;
    var sw = surface.width, sh = surface.height;
    // Collect tree points where surface > terrain + 3m
    var points = [];
    var step = 2; // sample every 2 pixels (1m) to reduce density
    for (var sy = 0; sy < sh; sy += step) {
      for (var sx = 0; sx < sw; sx += step) {
        var surfElev = surface.surface[sy * sw + sx];
        // Map surface pixel to terrain pixel
    ...
    if (points.length === 0) { checkReady(); return; }
    var geo = new THREE.BufferGeometry();
    geo.setAttribute('position', new THREE.Float32BufferAttribute(points, 3));
    var mat = new THREE.PointsMaterial({
      color: 0x2d6b2d, size: 2.5, sizeAttenuation: true, opacity: 0.7, transparent: true
    });
    vegMesh = new THREE.Points(geo, mat);
    scene.add(vegMesh);
    checkReady();
  });
